...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log(`🔍 Validation result: ${this.NOTES_SHARP[validatedKey.root]}${validatedKey.minor ? 'm' : ''} (was: ${this.NOTES_SHARP[key.root]}${key.minor ? 'm' : ''})`);
    
    if (validatedKey.root !== key.root || validatedKey.minor !== key.minor) {
      console.log(`🔄 KEY CHANGED! Re-running with ${this.NOTES_SHARP[validatedKey.root]}${validatedKey.minor ? 'm' : ''}`);
      key = validatedKey;
      timeline = this.chordTrackingHMMHybrid(feats, key, opts.bassMultiplier, true);
      timeline = this.finalizeTimeline(timeline, key, audioData.bpm, feats);
      console.log(`✅ Timeline rebuilt with correct key`);
    }
  ...
  computeDynamicIntroSkip(frameE, hop, sr) {
    // 🎵 v14.9: More aggressive skip for noisy intros
    const thr = this.percentile(frameE, 75); // Was 70, now 75 - stronger threshold
    let stable = 0, i = 0;
    ...
    for (; i < frameE.length; i++) {
      ...
      // 🆕 v14.9: Require LONGER stability for noisy starts (1.0s instead of 0.5s)
      const requiredStability = (i < 50 && frameE[i] < maxEnergy * 0.5) ? 1.0 : 0.5;
      
      if (stable * hop / sr >= requiredStability) break;
    }
    
    // 🆕 v14.9: Minimum skip - always skip first 1 second if energy is low
    const minSkipSec = 1.0;
    const minSkipFrames = Math.floor(minSkipSec * sr / hop);
    
    // Check if first second is weak
    let firstSecWeak = true;
    ...
    // If first second is 40% weaker than rest → skip it
    if (avgFirstSec < avgRest * 0.6) {
      i = Math.max(i, minSkipFrames);
      console.log(`⚠️ Weak intro detected (${avgFirstSec.toFixed(4)} vs ${avgRest.toFixed(4)}), forcing min ${minSkipSec}s skip`);
    }
    
    // Hard cap at 10 seconds (was 8)
  ...
  detectTonicFromBass(feats) {
    const { bassPc, frameE, hop, sr, chromaPeak } = feats;
    ...
    const harmonicThr = chromaPeak ? this.percentile(chromaPeak.filter(v => v > 0), 70) : 0;
    
    // 🆕 v14.5: Dynamic intro skip instead of hard 15%
    const skipFrames = this.computeDynamicIntroSkip(frameE, hop, sr);
    console.log(`🎸 Bass analysis: skip=${skipFrames}, E_thr=${threshold.toFixed(4)}, H_thr=${(harmonicThr||0).toFixed(3)}`);
    
    for (let i = skipFrames; i < bassPc.length; i++) {
      const bp = bassPc[i];
      // 🆕 v14.12: Also check harmonic content - skip percussion-only frames
      if (bp >= 0 && frameE[i] >= threshold && (!chromaPeak || chromaPeak[i] >= harmonicThr)) {
  ...
  detectKeyEnhanced(feats, duration) {
    ...
    if (bassTonic.confidence > 0.30) {
      ...
      const harmonicThr = feats.chromaPeak ? this.percentile(feats.chromaPeak.filter(v => v > 0), 70) : 0;
      ...
      for (let i = skipFrames; i < chroma.length; i++) {
        // 🆕 v14.12: Only use frames with strong energy AND harmonic content
        if (feats.frameE[i] >= threshold && (!feats.chromaPeak || feats.chromaPeak[i] >= harmonicThr)) {
  ...
  validateKeyFromChords(timeline, currentKey, feats) {
    // 🆕 v14.13: FORCE re-detection from actual chords (ignore initial detection)
    if (!timeline || timeline.length < 3) {
      return currentKey;
    }
    
    console.log(`🔍 Key validation: analyzing ${timeline.length} chords (current key: ${this.NOTES_SHARP[currentKey.root]}${currentKey.minor ? 'm' : ''})`);
    ...
    for (let keyRoot = 0; keyRoot < 12; keyRoot++) {
      for (let keyMinor of [false, true]) {
        ...
        const matchRatio = matchCount / totalChords;
        
        // 🆕 v14.13: Lower threshold - accept if 60%+ match
        if (matchRatio >= 0.60) {
          candidates.push({
            root: keyRoot,
            minor: keyMinor,
    ...
    if (candidates.length === 0) {
      console.warn('⚠️ No valid key candidates found!');
      return currentKey;
    }
  ...
  finalizeTimeline(timeline, key, bpm, feats) {
    ...
    // 🆕 v14.13: EARLY FILTERING - Remove percussion-only/silent frames
    console.log('🎵 Early filtering: removing frames with no harmonic content...');
    const preFiltered = [];
    
    if (feats.chromaSum && feats.chromaSum.length > 0) {
      // Calculate threshold - only keep frames with meaningful harmonic content
      ...
      if (validSums.length > 0) {
        ...
        const minHarmonicThreshold = medianSum * 0.15; // Must have at least 15% of median
        ...
        for (const ev of timeline) {
          ...
          // Skip frames with almost no harmonic content (metronome/percussion only)
          if (chromaSum < minHarmonicThreshold) {
            console.log(`🥁 Removing metronome/percussion at ${ev.t.toFixed(1)}s (chromaSum: ${chromaSum.toFixed(3)} < ${minHarmonicThreshold.toFixed(3)})`);
            continue;
          }
        ...
        console.log(`✅ Early filter: ${timeline.length} → ${preFiltered.length} chords (removed ${timeline.length - preFiltered.length} percussion-only)`);
      } else {
        // No valid chromaSum - keep all
        preFiltered.push(...timeline);
      }
    } else {
      // No chromaSum available - keep all
      preFiltered.push(...timeline);
    }
    
    if (preFiltered.length === 0) {
      console.warn('⚠️ No chords after early filtering!');
      return [];
    }
