...
class ChordEngineUltimate {
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    if (best.score < 50) return null; // היה 45
    if (best.score < 60 && durSec < 0.50) return null; // אקורד חלש וקצר = רעש
    ...
    // v16.18: אם עדיין לא דיאטוני ואקורד קצר - דחה
    if (!diatonic.pcs.includes(best.root) && durSec < 0.7) {
