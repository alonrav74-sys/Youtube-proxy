...
class ChordEngineUltimate {
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    const MIN_DURATION_SEC = 0.8; // v16.18: עוד יותר ארוך (היה 0.6)
    const CHROMA_CHANGE_THRESHOLD = 0.45; // v16.18: דורש יותר שינוי (היה 0.35)
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    // v16.18: סף גבוה יותר - מבוסס v14.36
    // v16.18: סף גבוה יותר - פחות אקורדים, יותר דיוק
    if (best.score < 55) return null; // מינימום סף
    if (best.score < 70 && durSec < 0.60) return null; // אקורד חלש וקצר = רעש
    if (best.score < 80 && durSec < 0.40) return null; // אקורד ממש קצר צריך להיות ממש חזק
    
    // 🎯 v16.18: תיקון שורש "שלישית" - מבוסס v14.36
    // אם השורש שזוהה הוא כרומטי אבל הוא שלישית של אקורד דיאטוני - נעדיף את הדיאטוני
    if (!diatonic.pcs.includes(best.root)) {
      for (const dc of diatonic.chords) {
        ...
        if (best.root === majorThird || best.root === minorThird) {
          // חישוב חוזק טריאדות
          const diaRoot = avg[dc.root] || 0;
          const diaThird = avg[best.root === majorThird ? majorThird : minorThird] || 0;
          const diaFifth = avg[this.toPc(dc.root + 7)] || 0;
          const triadDiatonic = diaRoot + diaThird + diaFifth;
          
          const chrRoot = avg[best.root] || 0;
          const chrThird = avg[this.toPc(best.root + 4)] || 0;
          const chrFifth = avg[this.toPc(best.root + 7)] || 0;
          const triadChromatic = chrRoot + chrThird + chrFifth;
          
          // אם הטריאדה על השורש הדיאטוני חזקה מספיק - נעדיף אותה
          if (triadDiatonic >= triadChromatic * 0.75 && diaRoot > 0.05) {
            console.log(`🎵 Re-rooting ${this.NOTES_SHARP[best.root]} → ${this.NOTES_SHARP[dc.root]} (diatonic triad: ${triadDiatonic.toFixed(3)} vs chromatic: ${triadChromatic.toFixed(3)})`);
            best.root = dc.root;
            best.isMinor = dc.minor;
            best.chordType = 'diatonic_rerooted';
            break;
          }
        }
      }
    ...
    // v16.18: סינון אגרסיבי לאקורדים לא דיאטוניים
    if (!diatonic.pcs.includes(best.root)) {
      if (durSec < 1.0) {
        console.log(`🔒 Rejecting short non-diatonic ${this.NOTES_SHARP[best.root]} (${durSec.toFixed(2)}s)`);
        return null;
      }
      // גם אם ארוך, צריך להיות ממש חזק
      if (best.score < 75) {
        console.log(`🔒 Rejecting weak non-diatonic ${this.NOTES_SHARP[best.root]} (score: ${best.score})`);
        return null;
      }
    }
    ...
    // v16.18: הקוד הישן של forcedSubstitutions הוסר
    // ה-re-rooting למעלה מטפל בזה בצורה אוניברסלית לכל סולם
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale) {
    ...
    // v16.18: העדפה חזקה מאוד לדיאטוני - מבוסס v14.36
    if (inScale) {
      score += 20; // בונוס גדול לדיאטוני
    } else {
      score -= 15; // קנס כבד ללא דיאטוני
    }
  ...
  finalizeTimeline(timeline, bpm, features) {
    ...
    const minDuration = 0.6 * spb; // v16.18: עוד יותר ארוך (היה 0.5)
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      const isDiatonic = ev.inScale;
      
      // v16.18: סינון אגרסיבי יותר - מבוסס v14.36
      // אקורד ראשון תמיד נשמר
      if (i === 0) {
        filtered.push(ev);
        continue;
      }
      
      // אקורד קצר מאוד - רק אם הוא דיאטוני וחזק מאוד
      if (isVeryShort) {
        if (isDiatonic && isStrong && isTheoryBacked) {
          filtered.push(ev);
        }
        continue;
      }
      
      // אקורד קצר - רק אם חזק ומגובה תיאורטית
      if (isShort) {
        if (isStrong && isTheoryBacked) {
          filtered.push(ev);
        }
        continue;
      }
