...
class ChordEngineUltimate {
    ...
    for (let i = 0; i < timeline.length - 1; i++) {
      ...
      if (i < timeline.length - 2) {
        ...
        if (third && third.root != null) {
          ...
          if (int1 === 5 && int2 === 5) {
            cadenceTargets[third.root] += 20; // IV–I–IV pattern וכו'
          }
        }
      }
    }
  ...
  detectTonicHybrid(features, startFrame) {
    ...
    // 3. אקורד ראשון חזק (לפי כרומה + באס)
    ...
    if (firstChord) {
      console.log(
        `  First chord: ${this.NOTES_SHARP[firstChord.root]}${firstChord.isMinor ? 'm' : ''} ` +
        `(score=${firstChord.score.toFixed(2)})`
      );
    }

    // 🗳️ הצבעה בין שלושת המקורות
    const votes = new Array(12).fill(0);

    // הבס – חזק אבל לא דיקטטור
    votes[bassTonic.root] += (bassTonic.confidence >= 80 ? 3 : 2);

    // KS – הכי חלש
    votes[ksTonic.root] += (ksTonic.confidence >= 80 ? 2 : 1);

    // האקורד הראשון – עוגן חזק מאוד בפופ/רוק
    if (firstChord) {
      const w = firstChord.score >= 0.45 ? 3 : 2;
      votes[firstChord.root] += w;
    }

    // מציאת הטוניקה בעלת מספר הקולות הגבוה ביותר
    let bestRoot = 0;
    let bestVotes = -1;
    for (let i = 0; i < 12; i++) {
    ...
    // אם הבס בטוח מאד *וגם* מסכים עם התוצאה – נשארים איתו
    if (bassTonic.confidence >= 75 && bestRoot === bassTonic.root) {
      return {
        root: bassTonic.root,
    ...
    // אם שניים מתוך שלושה מסכימים על אותו שורש – זה מנצח
    const agreeWithBest = [
      bassTonic.root === bestRoot,
      ksTonic.root === bestRoot,
      firstChord ? firstChord.root === bestRoot : false
    ].filter(Boolean).length;

    if (agreeWithBest >= 2) {
      // confidence משוקלל
      let conf = 0;
      if (bassTonic.root === bestRoot) conf += bassTonic.confidence * 0.5;
      if (ksTonic.root === bestRoot) conf += ksTonic.confidence * 0.2;
      if (firstChord && firstChord.root === bestRoot) conf += 30; // בונוס על אקורד ראשון
      conf = Math.min(98, Math.max(60, Math.round(conf)));

      return {
        root: bestRoot,
        confidence: conf,
        method: 'vote_bass_KS_first'
      };
    }

    // אחרת – נשארים במודל הישן: הבס מחליט, KS רק מוריד/מעלה confidence
  ...
  finalizeTimeline(timeline, bpm, features) {
    ...
    const spb = 60 / Math.max(60, Math.min(200, bpm));
    const minDuration = 0.5 * spb; // v16.36: Longer minimum (was 0.4)
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      const isStrong = ev.confidence >= 90; // v16.36: Higher threshold (was 85)
