/**
 * ChordEngine v16.36 - Conservative Filtering
 * 
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log('🎵 ChordEngine v16.36 (Conservative - Fewer but Solid Chords)');
    ...
    // v16.36: TONIC VALIDATION after timeline (section 6)
    ...
    // v16.36: Enhanced slash chord detection (section 6)
    ...
    // v16.36: ENFORCE EARLY DIATONIC (from v14.36) - clean intro
    timeline = this.enforceEarlyDiatonic(timeline, key, features, audio.bpm);

    // v16.36: AUTOMATIC STYLE PROFILE DETECTION
    ...
    // v16.36: Pattern memory V2 (section 6 - fixes anomalies in repeated patterns)
    ...
    return {
      ...
      mode: modeResult,
      styleProfile,  // v16.36: easy_pop / colorful_pop / jazz_like
  ...
  processAudio(audioBuffer) {
    ...
    // v16.36: Clean signal before processing
  ...
  // v16.36: Clean signal - pre-emphasis + noise gate + smoothing
  cleanSignal(x, sr) {
  ...
  extractFeatures(audio) {
    ...
    for (let start = 0; start + win <= x.length; start += hop) {
      ...
      // v16.36: Improved chroma with harmonic weighting
      ...
      for (let bin = 1; bin < mags.length; bin++) {
        ...
        // v16.36: Weight by harmonic clarity
    ...
    // v16.36: Compute spectral flux for chord change detection
    ...
    return {
      ...
      flux, fluxP80, // v16.36: Added flux
  ...
  // v16.36: Spectral flux - detects changes in chroma
  ...
  // v16.36: Find candidate frames for chord changes
  ...
  detectBassNote(mags, sr, N) {
    const fmin = 40, fmax = 300; // v16.36: Extended range (was 250)
    ...
    for (let bin = 1; bin < mags.length; bin++) {
      ...
      if (freq >= fmin) {
        ...
        const weight = freq < 150 ? 1.5 : 1.0; // v16.36: Boost very low freqs
    ...
    // v16.36: Stricter threshold (was 0.25)
  ...
  // v16.36: Improved - also detect clear pitch without bass (guitar/piano intros)
  findMusicStart(features) {
    ...
    for (let i = 0; i < energy.length; i++) {
      ...
      // v16.36: Check for clear musical pitch in chroma (for guitar/piano intros)
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.36: SONG PROFILE ANALYSIS (Dynamic thresholds) - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.36: TONIC VALIDATION FROM CHORDS - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.36: BETTER SLASH CHORD DETECTION - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.36: IMPROVED PATTERN MEMORY (3+4 chord patterns) - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 v16.36: STABLE BASS CHORD BUILDING with Beat-Aware Detection
  // ═══════════════════════════════════════════════════════════
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    // v16.36: MUCH STRICTER thresholds to avoid noise
    ...
    // v16.36: Get chord change candidate frames (beat-aligned + flux + bass changes)
    const changeFrames = this.getChordChangeCandidates(features, bpm, startFrame);
    console.log(`🎵 v16.36: ${changeFrames.length} chord change candidates (beat-aligned)`);
  ...
  getBorrowedChords(key) {
    ...
    if (!key.minor) {
      ...
      // v16.36: REMOVED bVI (Ab/G#) - causes confusion with E
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    if (candidates.length === 0) {
      // v16.36: Chromatic fallback - only if VERY clear
      for (const isMinor of [false, true]) {
        const score = this.scoreChordCandidate(avg, bassNote, isMinor, bassNote, false);
        if (score > 60) { // v16.36: Higher threshold (was 40)
          ...
          if (isReasonable || score > 80) { // Only if reasonable OR super strong
            candidates.push({
              ...
              inversionBass: null,
              chordType: 'chromatic',
              score: score - 30, // v16.36: Bigger penalty (was -20)
              priority: 20        // v16.36: Lower priority (was 30)
            });
          }
        }
      }
    ...
    if (best.score < 45 && durSec < 0.40) return null; // v16.36: Stricter threshold
    
    // v16.36: Enharmonic substitution for unlikely chromatics
    ...
    // v16.36: AUTO-CORRECT quality based on actual third (use finalRoot!)
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale) {
    ...
    // v16.36: AUTO-DETECT which third is actually present!
    ...
    else if (bassNote === actualThird) score += 10; // v16.36: Slightly lower for first inversion
    else if (bassNote === fifth) score += 8;        // v16.36: Slightly lower for second inversion
  ...
  // v16.36: Check if chromatic chord makes harmonic sense
  isReasonableChromaticChord(root, key, prevChord) {
    ...
    if (!key.minor) {
      ...
      // v16.36: REMOVED bVI (Ab/G#) - too rare and often confused with E
  ...
  validateWithCircleOfFifths(timeline, key, features) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.36: STRICT chromatic filtering
  ...
  addExtensions(timeline, features, key, opts) {
    ...
    return timeline.map(ev => {
      ...
      // v16.36: Check all chord tones properly
      ...
      // 3. Check for 7th chords - v16.36: More precise detection
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
        ...
        if (!isMinor && M7 > 0.10 && M7 > b7 * 1.5 && M3 > 0.08) {
          // v16.36: Also check that M7 is significant relative to root
          if (M7 > r * 0.15) {
        ...
        // Dominant 7th (C7, G7) - only for major chords with clear b7
          ...
          // v16.36: b7 must be prominent enough
          if (b7 > r * 0.12) {
            label += '7';
          }
        }
      ...
      // 4. Check for sus chords - v16.36: More precise
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
      ...
      // 5. Add 6 if present (C6, Am6) - v16.36: Stricter to avoid melodic 6ths
  ...
  finalizeTimeline(timeline, bpm, features) {
    ...
    // v16.36: MUCH STRICTER filtering like v14.36
    const minDuration = 0.6 * spb;  // Increased from 0.5
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      const isWeak = energy < energyMedian * 0.85;  // v14.36 style
      const isShort = duration < minDuration;
      const isVeryShort = duration < minDuration * 0.5;
      
      const isStrong = (ev.confidence || 0) >= 85;
      const isTheoryBacked = ev.chordType && !ev.chordType.startsWith('chromatic');
      const isDiatonic = ev.inScale === true;
      
      // v14.36 style: aggressive filtering
      // 1. Remove very short chords regardless
      if (isVeryShort) continue;
      
      // 2. Remove short + weak chords
      if (isShort && isWeak) continue;
      
      // 3. Remove short non-diatonic chords unless very strong
      if (isShort && !isDiatonic && !isStrong) continue;
      
      // 4. Keep if: long enough, OR (strong AND theory-backed)
    ...
    // v16.36: Second pass - remove any remaining short chords after merging
    const final = [];
    for (let i = 0; i < merged.length; i++) {
      ...
      // After merging, still filter out very short results
      if (duration >= minDuration * 0.7 || final.length === 0) {
        final.push(ev);
      } else if (final.length > 0) {
        // Extend previous chord instead
        // (implicit - just don't add this chord)
      }
    }
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.36: ENFORCE EARLY DIATONIC (from v14.36)
  ...
  // v16.36: Detect if song is "easy rock" (mostly diatonic)
  ...
  // v16.36: Force non-diatonic chords to nearest diatonic
  ...
  // v16.36: Remove isolated chromatic chords between identical neighbors
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.36: HMM ENGINE (parallel to Bass engine)
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.36: MERGE ENGINES - Compare and choose best
  // ═══════════════════════════════════════════════════════════
