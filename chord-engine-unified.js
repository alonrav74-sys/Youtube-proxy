...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // 🎯 STEP 1: Detect key from chroma
    let key = this.detectKeyEnhanced(feats, audioData.duration);
    
    // 🎯 STEP 2: Run HMM with ALL chords (diatonic get bonus)
    let timeline = this.chordTrackingHMMHybrid(feats, key, opts.bassMultiplier);
    ...
    if (validatedKey.root !== key.root || validatedKey.minor !== key.minor) {
      key = validatedKey;
      timeline = this.chordTrackingHMMHybrid(feats, key, opts.bassMultiplier);
      timeline = this.finalizeTimeline(timeline, key, audioData.bpm, feats);
    }
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier) {
    const { chroma, bassPc, hop, sr, frameE } = feats;
    
    // Build diatonic set for bonus
    ...
    for (let i = 0; i < diatonicPcs.length; i++) {
      ...
      const label = this.nameSharp(r) + qual;
      diatonicSet.add(label);
      diatonicList.push({ root: r, label, quality: qual });
    }
    
    // OPTIMIZED: Start with only diatonic + close relatives
    // Total ~20 candidates instead of 38
    const candidates = [];
    
    // 1. All diatonic chords (7)
    for (const dc of diatonicList) {
      candidates.push({
        root: dc.root,
        label: dc.label,
        type: dc.quality === 'm' ? 'minor' : (dc.quality === 'dim' ? 'dim' : 'major'),
        isDiatonic: true
      });
    }
    
    // 2. Common non-diatonic: relative major/minor (1)
    const relativeRoot = key.minor ? this.toPc(key.root + 3) : this.toPc(key.root - 3);
    const relativeLabel = key.minor ? this.nameSharp(relativeRoot) : this.nameSharp(relativeRoot) + 'm';
    if (!diatonicSet.has(relativeLabel)) {
      candidates.push({
        root: relativeRoot,
        label: relativeLabel,
        type: key.minor ? 'major' : 'minor',
        isDiatonic: false
      });
    }
    
    // 3. Common borrowings: bVI, bVII for major (2)
    if (!key.minor) {
    ...
    // 4. V major for minor keys (harmonic minor) (1)
    if (key.minor) {
      const V = this.toPc(key.root + 7);
      if (!diatonicSet.has(this.nameSharp(V))) {
        candidates.push({ root: V, label: this.nameSharp(V), type: 'major', isDiatonic: false });
      }
    }
    
    // 5. Sus variants for tonic + dominant only (4)
    ...
    console.log(`HMM: ${uniqueCandidates.length} candidates, ${chroma.length} frames = ${(uniqueCandidates.length * uniqueCandidates.length * chroma.length / 1e6).toFixed(1)}M ops`);
