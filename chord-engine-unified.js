  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    if (useFullMode) {
      ...
      if (!key.minor) {
        ...
        candidates.push({ root: bVI, label: this.NOTES_SHARP[bVI], type: 'major', isDiatonic: false });
        candidates.push({ root: bVII, label: this.NOTES_SHARP[bVII], type: 'major', isDiatonic: false });
        
        // V/V (secondary dominant)
        const V_of_V = this.toPc(key.root + 2);
        candidates.push({ root: V_of_V, label: this.NOTES_SHARP[V_of_V], type: 'major', isDiatonic: false });
      } else {
        const V = this.toPc(key.root + 7);
        const VII = this.toPc(key.root + 11);
        candidates.push({ root: V, label: this.NOTES_SHARP[V], type: 'major', isDiatonic: false });
        candidates.push({ root: VII, label: this.NOTES_SHARP[VII], type: 'major', isDiatonic: false });
      }
      
      // 3. Sus variants
      const tonic = key.root;
      const dom = this.toPc(key.root + 7);
      candidates.push({ root: tonic, label: this.NOTES_SHARP[tonic] + 'sus4', type: 'sus4', isDiatonic: false });
      candidates.push({ root: tonic, label: this.NOTES_SHARP[tonic] + 'sus2', type: 'sus2', isDiatonic: false });
      candidates.push({ root: dom, label: this.NOTES_SHARP[dom] + 'sus4', type: 'sus4', isDiatonic: false });
      candidates.push({ root: dom, label: this.NOTES_SHARP[dom] + 'sus2', type: 'sus2', isDiatonic: false });
    }
