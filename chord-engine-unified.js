...
class ChordEngine {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // 1. Krumhansl-Schmuckler (baseline - conservative weight)
    tonicScores[krumhanslTonic] += 25;
    console.log(`📊 KS suggests: ${this.nameSharp(krumhanslTonic)} (+25)`);
    
    // 2. Bass histogram (STRONG evidence)
    if (maxBassPc >= 0) {
      const bassPoints = Math.floor(bassConfidence * 120); // Up to 120 points!
    ...
    // 3. Final bass note (songs END on tonic - VERY STRONG)
    if (finalBassPc >= 0) {
      tonicScores[finalBassPc] += 45;
      console.log(`📊 Final chord: ${this.nameSharp(finalBassPc)} (+45)`);
    }
    
    // 4. First chord (songs START on tonic - VERY STRONG)
    if (firstChordRoot >= 0) {
      tonicScores[firstChordRoot] += 45;
      console.log(`📊 First chord: ${this.nameSharp(firstChordRoot)} (+45)`);
    }
    
    // 5. Cadence analysis (V→I resolutions - STRONG)
    if (cadenceTonic >= 0 && maxCadence > 0) {
      const cadencePoints = Math.floor(maxCadence * 25); // Up to 100+ points
    ...
    // 6. SUPER BOOST: First AND Last agree (MOST RELIABLE!)
    if (firstChordRoot >= 0 && lastChordRoot >= 0 && firstChordRoot === lastChordRoot) {
      tonicScores[firstChordRoot] += 60; // MASSIVE bonus!
      console.log(`📊 First & Last AGREE: ${this.nameSharp(firstChordRoot)} (+60 SUPER BOOST!)`);
    }
    ...
    // 🎵 MAJOR vs MINOR decision (check after tonic is set)
    // For relative keys (Am vs C, Em vs G), check which 3rd is stronger
    const relToKS = this.toPc(krumhanslTonic - bestTonic);
    const isRelativeKey = (relToKS === 9 || relToKS === 3);
    
    if (isRelativeKey && bassConfidence > 0.12) {
      ...
      console.log(`🎵 3rd analysis: minor=${minorStrength.toFixed(3)}, major=${majorStrength.toFixed(3)}`);
      
      // Decide major vs minor based on 3rd
      if (relToKS === 9 && !key.minor && minorStrength > majorStrength * 1.08) {
        // Was major, but minor 3rd is stronger → switch to minor
        console.log(`✅ Mode corrected: ${this.nameSharp(bestTonic)} → ${this.nameSharp(bestTonic)}m (minor 3rd dominant)`);
        key.minor = true;
      } else if (relToKS === 3 && key.minor && majorStrength > minorStrength * 1.08) {
        // Was minor, but major 3rd is stronger → switch to major
        console.log(`✅ Mode corrected: ${this.nameSharp(bestTonic)}m → ${this.nameSharp(bestTonic)} (major 3rd dominant)`);
        key.minor = false;
      } else {
        console.log(`✅ Mode confirmed: ${this.nameSharp(bestTonic)}${key.minor?'m':''} (3rd analysis agrees)`);
      }
    }
