...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // Simple mode (7 candidates) is 4× faster!
    // Use full mode only when very confident about key
    ...
    // 🎯 If tonic detection is VERY confident (95%+), trust it and re-run!
    // For speed: only re-run if confidence >= 98% (configurable)
    const tonicThreshold = opts.tonicRerunThreshold !== undefined ? opts.tonicRerunThreshold : 95;
    
    if (tonic.root !== key.root && tonic.confidence >= tonicThreshold) {
      // Update key
      key.root = tonic.root;
      
      // 🔄 RE-RUN entire chord detection with corrected key!
      if (opts.progressCallback) {
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    // Build diatonic pitch classes
    const scale = key.minor ? this.MINOR_SCALE : this.MAJOR_SCALE;
    const diatonicPcs = scale.map(s => this.toPc(key.root + s));
    
    // V10 APPROACH: Both major AND minor for each diatonic note!
    // Let the data decide which quality fits better via cosine similarity
    const candidates = [];
    ...
    // Total: 14 candidates (7 × 2)
    // Much faster than complex candidate sets!
    
    // V10 APPROACH: Simple cosine similarity!
    // Build chord mask (1 for chord tones, 0 otherwise)
    ...
    const emitScore = (i, cand) => {
      ...
      // Bass boost (only addition to v10)
  ...
  inKey(pc, keyRoot, minor) {
    ...
    // Check if it's diatonic
    const diatonicPcs = scale.map(interval => this.toPc(keyRoot + interval));
    if (diatonicPcs.includes(this.toPc(pc))) return true;
    
    // ✨ SMART TOLERANCE: Allow common functional borrowings
    // These are NOT errors - they're standard harmonic practice!
    
    if (minor) {
      // Minor keys: Allow V major (harmonic minor) and VII major
      if (rel === 7) return true;  // V major instead of v minor (E in Am)
      if (rel === 11) return true; // VII major (G# in Am)
    } else {
      // Major keys: Allow common borrowings
      if (rel === 2) return true;  // II major = V/V (D in C major → secondary dominant)
      if (rel === 10) return true; // bVII (Bb in C major → modal borrowing)
      if (rel === 8) return true;  // bVI (Ab in C major → modal borrowing)
      // Note: III major check removed - too ambiguous
    }
  ...
  buildCircleOfFifths(key) {
    ...
    const chords = this.getDiatonicChords(keyName, key.minor ? 'minor' : 'major');
