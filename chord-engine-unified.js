...
class ChordEngineEnhanced {
  ...
  decorateQualitiesEnhanced(timeline, feats, key, harmonyMode, extensionMultiplier = 1.0) {
    ...
    timeline.forEach((chord, i) => {
      ...
      const has5 = chroma[(root + 7) % 12];  // ✅ FIXED: Perfect 5th = 7 semitones!
      ...
      const has11 = chroma[(root + 5) % 12];  // ✅ 11th = Perfect 4th (5 semitones)
      const has13 = chroma[(root + 9) % 12];  // 13th = Major 6th (9 semitones)
