/**
 * 🎹 ChordEngine UNIFIED v14.2
 * 
 * 🔧 v14.1 FIXES:
 * 1. Tonic threshold: 95% → 75%
 * 2. First chord bonus: 5 → 80 points (listen to third!)
 * 3. Universal veto for chromatic contradictions
 * 
 * 🆕 v14.2 NEW FIXES:
 * 4. Skip intro noise for KEY DETECTION ONLY (not chord detection!)
 * 5. Stronger minor key preference: +100 first, +50 last
 * 6. Noise filtering in chroma (zero out flat spectrum)
 * 
 * ⚠️ CRITICAL: Chord timeline is NEVER modified - only key detection!
 */
...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // Extract features from FULL audio (includes intro noise)
    const feats = this.extractFeatures(audioData);
    
    // 🆕 v14.2: Filter noise in chroma (but keep timeline intact!)
    this.filterNoiseInChroma(feats);
    
    if (opts.progressCallback) {
      opts.progressCallback({ stage: 'detecting_key', progress: 0.3 });
    }
    
    // 🆕 v14.2: Use cleaned features for key detection ONLY
    const featsForKey = this.skipIntroNoiseForKeyDetection(feats);
    let key = this.detectKeyEnhanced(featsForKey, audioData.duration);
    ...
    // ⚠️ CRITICAL: Use ORIGINAL features (with noise) for chord detection!
    // This preserves timeline sync!
    ...
    // 🔧 v14.1: Lower threshold from 95% to 75%
  ...
  // 🆕 v14.2: Skip intro noise for KEY DETECTION ONLY
  skipIntroNoiseForKeyDetection(feats) {
    const { frameE, chroma, bassPc, hop, sr } = feats;
    
    // Find first "real music" frame (high energy + consistent)
    const threshold = this.percentile(frameE, 65);  // Top 35% energy
    
    let firstMusicFrame = 0;
    let consecutiveStrong = 0;
    
    for (let i = 0; i < frameE.length; i++) {
      if (frameE[i] > threshold) {
        consecutiveStrong++;
        if (consecutiveStrong >= 5) {  // 5 consecutive strong frames (~0.5 sec)
          firstMusicFrame = Math.max(0, i - 4);
          break;
        }
      } else {
        consecutiveStrong = 0;
    ...
    // Only skip if we found strong music AND it's not too late
    // (don't skip more than 10 seconds)
    const maxSkipFrames = Math.floor(10.0 / (hop / sr));
    if (firstMusicFrame > maxSkipFrames) {
      firstMusicFrame = 0;  // Don't skip - might be a quiet song
    }
    
    // Return cleaned features FOR KEY DETECTION ONLY
    return {
      chroma: chroma.slice(firstMusicFrame),
      bassPc: bassPc.slice(firstMusicFrame),
      frameE: frameE.slice(firstMusicFrame),
      hop: hop,
      sr: sr,
      skippedFrames: firstMusicFrame  // Track how much we skipped
    };
  }

  // 🆕 v14.2: Filter noise in chroma (zero out flat spectrum)
  filterNoiseInChroma(feats) {
    const { chroma } = feats;
    
    for (let i = 0; i < chroma.length; i++) {
      const c = chroma[i];
      
      // Check if chroma is "flat" (all values similar = noise/silence)
      const mean = c.reduce((a, b) => a + b, 0) / 12;
      
      // If mean is very low → silence
      if (mean < 0.02) {
        for (let p = 0; p < 12; p++) {
          c[p] = 0;
        }
      ...
      // If stddev is very low → flat spectrum → noise!
      // But don't be too aggressive - only filter extreme cases
      if (stddev < 0.04) {
        // Attenuate (don't zero completely - preserve sync)
        for (let p = 0; p < 12; p++) {
          c[p] *= 0.3;  // Reduce to 30% instead of zeroing
        }
      }
    }
  }
  ...
  // 🔧 v14.1 FIX 2 & 3 + 🆕 v14.2 FIX 5: Enhanced validation
  validateKeyFromChords(timeline, currentKey, feats) {
    ...
    for (let keyRoot = 0; keyRoot < 12; keyRoot++) {
      for (let keyMinor of [false, true]) {
        ...
        // 🔧 v14.1 FIX 3: Universal veto for chromatic contradictions
    ...
    // 🔧 v14.1 FIX 2 + 🆕 v14.2 FIX 5: MASSIVE first/last chord bonus
    ...
    if (firstChord >= 0) {
      for (const cand of candidates) {
        if (firstChord === cand.root) {
          cand.score += 50.0;  // Base tonic bonus
          
          // 🆕 v14.2 FIX 5: STRONGER minor preference
          if (firstIsMinor && cand.minor) {
            cand.score += 50.0;  // Was 30, now 50!
          }
    ...
    // 🆕 v14.2 FIX 5: Last chord bonus (especially for minor)
    const lastChord = this.parseRoot(timeline[timeline.length - 1].label);
    const lastChordLabel = timeline[timeline.length - 1].label;
    const lastIsMinor = /m(?!aj)/.test(lastChordLabel);
    
    if (lastChord >= 0) {
      for (const cand of candidates) {
        if (lastChord === cand.root) {
          cand.score += 10.0;  // Base last chord bonus
          
          // 🆕 v14.2: If both first AND last are minor tonic → definitely minor!
          if (firstChord === lastChord && firstIsMinor && lastIsMinor && cand.minor) {
            cand.score += 50.0;  // MASSIVE bonus!
          }
        }
      }
    }
