...
class ChordEngineEnhanced {
  constructor() {
    ...
    this.THRESHOLD_EXTENSION = 0.18;  // Extensions
  ...
  async detect(audioBuffer, options = {}) {
    ...
    let timeline = this.buildChordsFromBassEnhanced(feats, estimatedBPM);
    ...
    timeline = this.decorateQualitiesEnhanced(timeline, feats, key, opts.harmonyMode);
  ...
  buildChordsFromBassEnhanced(feats, bpm) {
    const timeline = [];
    ...
    for (let fi = 0; fi < feats.chroma.length; fi++) {
      ...
      if (chromaSum < 0.1 || bassAmp < 0.05) continue;
  ...
  decorateQualitiesEnhanced(timeline, feats, key, harmonyMode) {
    timeline.forEach((chord, i) => {
      ...
      if (diff < this.THRESHOLD_MIN_DIFF) {
        ...
        if (has5 > this.THRESHOLD_STRONG && has4 < 0.1) {
          quality = 'sus4';
        } else if (has9 > this.THRESHOLD_STRONG && has4 < 0.1) {
          quality = 'sus2';  // 🆕
        } else {
        ...
        if (has6 > this.THRESHOLD_EXTENSION) {
          quality = 'dim';  // 🆕
        }
      } else {
        // Major
        ...
        // Check for augmented
        if (chroma[(root + 8) % 12] > this.THRESHOLD_EXTENSION) {
          quality = 'aug';  // 🆕
        }
      }
      ...
      if (!quality.includes('sus')) {
        if (has7flat > this.THRESHOLD_EXTENSION || has7 > this.THRESHOLD_EXTENSION) {
          if (has7 > has7flat + this.THRESHOLD_MIN_DIFF) {
            quality += 'maj7';
          } else if (has7flat > has7 + this.THRESHOLD_MIN_DIFF) {
            if (quality === 'dim') {
              quality = 'dim7';  // 🆕
            } else if (quality === 'm' && has6 > this.THRESHOLD_EXTENSION) {
      ...
      if (!quality.includes('7') && has6th > this.THRESHOLD_EXTENSION) {
        quality += '6';  // 🆕
      }
      
      // 4. Extensions (only in jazz mode or if strong)
      if (harmonyMode === 'jazz' || harmonyMode === 'auto') {
        if (has9 > this.THRESHOLD_EXTENSION && !quality.includes('sus2')) {
          if (quality.includes('7')) {
            quality += has9 > this.THRESHOLD_STRONG ? '9' : '(add9)';
          }
        }
        
        if (has11 > this.THRESHOLD_EXTENSION && !quality.includes('sus4')) {
          if (quality.includes('7') || quality.includes('9')) {
            quality += '11';  // 🆕 Actually add it!
          }
        }
        
        if (has13 > this.THRESHOLD_EXTENSION) {
          if (quality.includes('7') || quality.includes('9')) {
            quality += '13';  // 🆕
          }
        }
