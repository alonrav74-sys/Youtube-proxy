/**
 * 🎹 ChordEngine UNIFIED v11.3 MUSICAL INTELLIGENCE
 * 
 * 🔥 REVOLUTIONARY KEY DETECTION:
 * Think like a musician, not a statistician!
 * 
 * 1. Find all unique chords in the song
 * 2. Check which keys contain ALL these chords (diatonic validation)
 * 3. If only ONE key matches → that's the key!
 * 4. If multiple keys match → check cadences (V→I, IV→V→I)
 * 5. Still ambiguous? → check first/last chord
 * 6. NEVER suggest a key that doesn't contain all the chords!
 * 
 * This fixes the catastrophic bug where "Hallelujah" (C-Am-F-G) 
 * was detected as Cm minor even though Cm contains NONE of these chords!
 * 
 * TARGET ACCURACY (Israeli/Rock/Pop):
 * - Key/Tonic: 98%+ (was ~88%, now MUSICAL!)
 * - Simple songs like Hallelujah: 100%!
 * 
...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // Initial key guess using chroma (can be wrong!)
    const keyGuess = this.detectKeyChroma(feats, audioData.duration);
    
    // HMM chord tracking with initial key guess
    let timeline = this.chordTrackingHMM(feats, keyGuess, opts.bassMultiplier);
    timeline = this.finalizeTimeline(timeline, keyGuess, audioData.bpm, feats);
    timeline = this.enforceEarlyDiatonic(timeline, keyGuess, feats, audioData.bpm);
    
    // 🎯 MUSICAL KEY DETECTION - analyze actual chords!
    const key = this.detectKeyMusically(timeline, feats);
    
    // Re-run HMM with correct key if different
    if (key.root !== keyGuess.root || key.minor !== keyGuess.minor) {
      timeline = this.chordTrackingHMM(feats, key, opts.bassMultiplier);
  ...
  // ========================================
  // INITIAL KEY GUESS (Chroma-based, can be wrong)
  // ========================================
  
  detectKeyChroma(feats, duration) {
    const { chroma } = feats;
    ...
    for (let i = 0; i < chroma.length; i++) {
      ...
      let weight = position < 0.08 ? 3.0 : (position > 0.92 ? 2.0 : 1.0);
  ...
  // ========================================
  // 🎯 MUSICAL KEY DETECTION - Think like a musician!
  // ========================================
  
  detectKeyMusically(timeline, feats) {
    if (!timeline || timeline.length < 3) {
      return this.detectKeyChroma(feats, 0);
    }
    
    // Step 1: Extract unique chord roots (ignore decorations)
    ...
    timeline.forEach(chord => {
      ...
      if (root >= 0) {
        ...
        chordRoots.add({ root, isMinor });
      }
    });
    
    if (chordRoots.size === 0) {
      return this.detectKeyChroma(feats, 0);
    }
    
    // Step 2: Find ALL keys that contain these chords diatonically
    const candidates = [];
    ...
    for (let keyRoot = 0; keyRoot < 12; keyRoot++) {
      for (let keyMinor of [false, true]) {
        ...
        const diatonicChords = scale.map((degree, i) => ({
          ...
          isMinor: qualities[i] === 'm'
        }));
        
        // Check if ALL song chords are in this key
        let allMatch = true;
        for (const songChord of chordRoots) {
          const found = diatonicChords.some(dc => 
            dc.root === songChord.root && dc.isMinor === songChord.isMinor
          );
          if (!found) {
            allMatch = false;
            break;
        ...
        if (allMatch) {
          candidates.push({ root: keyRoot, minor: keyMinor, score: 0 });
        }
      }
    }
    
    // Step 3: If only ONE candidate → that's the key!
    if (candidates.length === 0) {
      return this.detectKeyChroma(feats, 0);
    }
    
    if (candidates.length === 1) {
      return { root: candidates[0].root, minor: candidates[0].minor, confidence: 1.0 };
    }
    
    // Step 4: Multiple candidates → use musical logic
    
    // Check cadences (V→I, IV→V→I)
    for (let i = 0; i < timeline.length - 1; i++) {
      ...
      if (currRoot >= 0 && nextRoot >= 0) {
        ...
        // V→I cadence (perfect 5th up = 7 semitones, or 4th down = 5 semitones)
        if (interval === 5 || interval === 7) {
          for (const cand of candidates) {
            if (nextRoot === cand.root) {
              cand.score += 10.0; // Strong evidence!
            }
          }
        }
      }
      ...
      if (chord1 >= 0 && chord2 >= 0 && chord3 >= 0) {
        for (const cand of candidates) {
          ...
          if (chord1 === IV && chord2 === V && chord3 === I) {
            cand.score += 15.0; // Very strong evidence!
          }
        }
      }
    }
    ...
    if (lastChord >= 0) {
      for (const cand of candidates) {
        if (lastChord === cand.root) {
          cand.score += 8.0; // Ending on tonic is strong!
        }
      }
    }
    ...
    return { root: best.root, minor: best.minor, confidence: Math.min(1.0, 0.8 + best.score / 50) };
  }

  // ========================================
  // MUSICAL TONIC DETECTION
  // ========================================
  ...
  // ========================================
  // HMM with CORRECT Qualities per Scale Degree
  // ========================================
