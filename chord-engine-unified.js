...
class ChordEngineEnhanced {
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier) {
    ...
    for (let i = 0; i < diatonicPcs.length; i++) {
      ...
      diatonicSet.add(this.nameSharp(r) + qual);
    }
    
    // Build ALL 24 candidates (12 major + 12 minor)
    const candidates = [];
    ...
    for (let r = 0; r < 12; r++) {
      ...
      candidates.push({
        root: r,
        label: this.nameSharp(r),
        type: 'major',
        isDiatonic: diatonicSet.has(this.nameSharp(r))
      });
      
      // Minor
      candidates.push({
        root: r,
        label: this.nameSharp(r) + 'm',
        type: 'minor',
        isDiatonic: diatonicSet.has(this.nameSharp(r) + 'm')
      });
    }
    
    // Add sus variants for diatonic roots only
    for (let i = 0; i < diatonicPcs.length; i++) {
      const r = diatonicPcs[i];
      candidates.push({ root: r, label: this.nameSharp(r) + 'sus4', type: 'sus4', isDiatonic: false });
      candidates.push({ root: r, label: this.nameSharp(r) + 'sus2', type: 'sus2', isDiatonic: false });
    }
