 ...
 *
 *  Changes vs v14.2:
 * - Stronger intro-noise filtering (skip 15%, energy threshold at 80%)
 * - First-chord weighting when bass is weak
 * - Earlier HMM activation threshold (0.7)
 * - Borrowed-chord tolerance in key validation (bVI, bVII, bIII, iv in major; IV major in minor)
 * - Basic modulation detection across 3 sections
 * - Extended mode detection (Mixolydian / Lydian / Dorian) when evident
 */
...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    const opts = {
      ...
      tonicRerunThreshold: options.tonicRerunThreshold
    };
    ...
    // CHANGED: earlier HMM use
    const useFullHMM = key.confidence > 0.70; // v14.2 was 0.80
    ...
    // better key validation (borrowed-chord tolerant)
    ...
    // FIX 1: Lower threshold from 95% to 75% (kept from v14.2)
    ...
    // NEW: simple modulation count across thirds
    const modulations = this.detectModulations(feats);
    ...
    return {
      ...
      stats: stats,
      mode: this.detectMode(key, feats) // CHANGED: pass feats for extended modes
    };
  }

  // CHANGED: extended mode detection (keeps backward-compat in return string)
  detectMode(key, feats) {
    if (key.minor) return 'Natural Minor (Aeolian)';

    // Heuristic mode cues from aggregated chroma
    if (!feats || !feats.chroma) return 'Major (Ionian)';

    const agg = new Array(12).fill(0);
    for (const c of feats.chroma) for (let i = 0; i < 12; i++) agg[i] += c[i];

    const root = key.root;
    const degree = d => this.toPc(root + d);

    const tonic = agg[degree(0)] || 0;
    const M2   = agg[degree(2)] || 0;  // Dorian emphasis cue
    const P4   = agg[degree(5)] || 0;  // Lydian/Mixo balance with #4 vs b7
    const M6   = agg[degree(9)] || 0;  // Major 6 (Dorian cue if minor — but here major)
    const b7   = agg[degree(10)] || 0; // Mixolydian cue
    const #4   = agg[degree(6)] || 0;  // Lydian cue (tritone above tonic)

    // Lydian: strong #4 vs 4, and relatively weak b7
    if (#4 > P4 * 1.35 && b7 < tonic * 0.6) return 'Lydian';

    // Mixolydian: strong b7 relative to tonic, normal 4
    if (b7 > tonic * 0.6 && #4 < P4 * 1.2) return 'Mixolydian';

    // Dorian (approx in major context): strong 2 & 6 coloration
    if (M2 > P4 * 1.3 && M6 > P4 * 1.1) return 'Dorian-like (major context)';
  ...
  // CHANGED: stronger intro-skip + energy threshold for bass tonic
  detectTonicFromBass(feats) {
    const { bassPc, frameE } = feats;
    const bassHist = new Array(12).fill(0);

    const threshold = this.percentile(frameE, 80); // CHANGED: 80% (was 70)
    const skipFrames = Math.floor(bassPc.length * 0.15); // CHANGED: 15% (was 10%)
  ...
  // CHANGED: also blends first-chord estimation when bass is weak
  detectKeyEnhanced(feats, duration) {
    ...
    if (bassTonic.confidence > 0.30) {
      ...
      // CHANGED: skip 15% and use 80% energy frames
      const skipFrames = Math.floor(chroma.length * 0.15);
      ...
      const minorThird = this.toPc(root + 3);
      const majorThird = this.toPc(root + 4);
      ...
      // NEW: first-chord weighting when bass is weak
      if (bassTonic.confidence < 0.50) {
        const firstPc = this.estimateFirstChordPc(feats);
        if (firstPc >= 0) {
          out.root = firstPc; // prefer first chord as tonic anchor if bass weak
          out.confidence = Math.min(1.0, (out.confidence + 0.20));
        }
      }
    ...
    // (same logic as v14.2 but refactored)
    const ks = this.ksKeyDetectionFromFeats(feats);
    // NEW: blend with first chord if available
    if (ks && ks.confidence < 0.6) {
      const firstPc = this.estimateFirstChordPc(feats);
      if (firstPc >= 0) ks.root = firstPc;
    }
    return ks;
  }

  // NEW: simple KS detection reusable on feats
  ksKeyDetectionFromFeats(feats) {
  ...
  // NEW: estimate first-chord root pc from strong frames in early window (after intro skip)
  estimateFirstChordPc(feats) {
    const { chroma, frameE, sr, hop } = feats;
    if (!chroma || !chroma.length) return -1;

    const skipFrames = Math.floor(chroma.length * 0.15); // align with intro skip
    ...
    for (let p = 0; p < 12; p++) {
      if (agg[p] > bestVal) { bestVal = agg[p]; best = p; }
    }
    return best;
  }
  ...
  // CHANGED: borrowed-chord tolerant validation
  validateKeyFromChords(timeline, currentKey, feats) {
    ...
    for (let keyRoot = 0; keyRoot < 12; keyRoot++) {
      for (let keyMinor of [false, true]) {
        ...
        // Prepare set of "allowed borrowed" roots relative to this candidate
        const isBorrowedAllowed = (ch) => {
          const rel = this.toPc(ch.root - keyRoot);
          if (!keyMinor) {
            // major: bIII (3), bVI (8), bVII (10), iv-minor (5 minor)
            if (rel === 3 || rel === 8 || rel === 10) return true;
            if (rel === 5 && ch.isMinor) return true;
          } else {
            // minor: IV major (5 major) commonly borrowed/harmonic
            if (rel === 5 && !ch.isMinor) return true;
            // also allow V major (7) and leading tone (11) contextually
            if (rel === 7 && !ch.isMinor) return true;
            if (rel === 11) return true;
          }
          return false;
        };

        // CHANGED: Universal veto, but skip for borrowed-allowed chords
        let hasContradiction = false;
        for (const songChord of chordRoots) {
          if (/sus/.test(songChord.label)) continue;
          if (isBorrowedAllowed(songChord)) continue; // don't veto borrowed
        ...
        for (const songChord of chordRoots) {
          ...
          if (/sus/.test(songChord.label) || isBorrowedAllowed(songChord)) {
            matchCount++; // neutral/allowed
            continue;
          }
    ...
    // Massive first chord bonus + listen to third! (kept)
    ...
    if (firstChord >= 0) {
      for (const cand of candidates) {
        if (firstChord === cand.root) {
          cand.score += 50.0;
          if (firstIsMinor && cand.minor) cand.score += 30.0;
          if (!firstIsMinor && !cand.minor) cand.score += 30.0;
        }
      }
    }

    // Last chord bonus (kept)
    const lastChord = this.parseRoot(timeline[timeline.length - 1].label);
    if (lastChord >= 0) {
      for (const cand of candidates) {
        if (lastChord === cand.root) cand.score += 10.0;
      }
    }
  ...
  // NEW: simple modulation detector by thirds of the track
  detectModulations(feats) {
    const { chroma, frameE } = feats;
    if (!chroma || chroma.length < 60) return 0;

    const block = Math.floor(chroma.length / 3);
    const parts = [
      { chroma: chroma.slice(0, block), frameE: frameE.slice(0, block) },
      { chroma: chroma.slice(block, 2 * block), frameE: frameE.slice(block, 2 * block) },
      { chroma: chroma.slice(2 * block), frameE: frameE.slice(2 * block) }
    ];

    const keys = parts.map(p => this.ksKeyDetectionFromFeats(p));
    let mods = 0;
    for (let i = 1; i < keys.length; i++) {
      if (!keys[i] || !keys[i - 1]) continue;
      const d = Math.min((keys[i].root - keys[i - 1].root + 12) % 12, (keys[i - 1].root - keys[i].root + 12) % 12);
