...
class ChordEngine {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log(`🎼 Initial key estimate: ${this.nameSharp(key.root)}${key.minor ? 'm' : ''}`);
    ...
    // Strong cadence evidence?
    if (cadenceTonic >= 0 && maxCadence >= 2.0 && cadenceTonic !== krumhanslTonic) {
      const distCadence = Math.min((cadenceTonic - krumhanslTonic + 12) % 12, (krumhanslTonic - cadenceTonic + 12) % 12);
      
      // Cadence + bass agreement?
      if (cadenceTonic === maxBassPc || cadenceTonic === finalBassPc) {
        console.log(`✅ Tonic corrected: ${this.nameSharp(krumhanslTonic)}${key.minor?'m':''} → ${this.nameSharp(cadenceTonic)}${key.minor?'m':''} (cadence + bass agreement)`);
        key.root = cadenceTonic;
      }
      // Strong cadence alone?
      else if (maxCadence >= 3.5 && distCadence >= 2) {
        // Check if it's relative minor/major
        const relToCadence = this.toPc(cadenceTonic - krumhanslTonic);
        if (relToCadence === 9 && !key.minor) {
          // Relative minor
          console.log(`✅ Tonic corrected: ${this.nameSharp(krumhanslTonic)} → ${this.nameSharp(cadenceTonic)}m (cadence analysis + relative minor)`);
          key.root = cadenceTonic;
          key.minor = true;
        } else if (relToCadence === 3 && key.minor) {
          // Relative major
          console.log(`✅ Tonic corrected: ${this.nameSharp(krumhanslTonic)}m → ${this.nameSharp(cadenceTonic)} (cadence analysis + relative major)`);
          key.root = cadenceTonic;
          key.minor = false;
        } else if (maxCadence >= 5.0) {
          // Very strong cadence evidence
          console.log(`✅ Tonic corrected: ${this.nameSharp(krumhanslTonic)}${key.minor?'m':''} → ${this.nameSharp(cadenceTonic)}${key.minor?'m':''} (strong cadence evidence)`);
          key.root = cadenceTonic;
        }
      }
    }
    // Bass evidence without cadence
    else if (maxBassPc >= 0 && bassConfidence > 0.25) {
      const scale = key.minor ? this.MINOR_SCALE : this.MAJOR_SCALE;
      const relToKS = this.toPc(maxBassPc - krumhanslTonic);
      
      // Is maxBassPc the relative minor/major?
      const isRelativeMinor = !key.minor && relToKS === 9;
      const isRelativeMajor = key.minor && relToKS === 3;
      
      if (isRelativeMinor && bassConfidence > 0.30) {
        console.log(`✅ Tonic corrected: ${this.nameSharp(krumhanslTonic)} → ${this.nameSharp(maxBassPc)}m (bass histogram + relative minor)`);
        key.root = maxBassPc;
        key.minor = true;
      } else if (isRelativeMajor && bassConfidence > 0.30) {
        console.log(`✅ Tonic corrected: ${this.nameSharp(krumhanslTonic)}m → ${this.nameSharp(maxBassPc)} (bass histogram + relative major)`);
        key.root = maxBassPc;
        key.minor = false;
      }
      // Final note agreement?
      else if (finalBassPc >= 0 && finalBassPc === maxBassPc && maxBassPc !== krumhanslTonic) {
        const distToFinal = Math.min((maxBassPc - krumhanslTonic + 12) % 12, (krumhanslTonic - maxBassPc + 12) % 12);
        if (distToFinal >= 2 && bassConfidence > 0.28) {
          console.log(`✅ Tonic corrected: ${this.nameSharp(krumhanslTonic)}${key.minor?'m':''} → ${this.nameSharp(maxBassPc)}${key.minor?'m':''} (bass + final note agreement)`);
          key.root = maxBassPc;
        }
      }
    }
