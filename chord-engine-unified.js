...
class ChordEngine {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    const energyThreshold = this.percentileLocal(frameE, 40); // 40th percentile
    
    for (let i = 0; i < Math.min(feats.bassPc.length, 100); i++) { // First 100 frames
      if (frameE[i] >= energyThreshold && bassPc[i] >= 0) {
        firstChordRoot = bassPc[i];
    ...
    if (firstChordRoot < 0) {
      ...
      for (let i = 0; i < firstSectionEnd; i++) {
        const bp = feats.bassPc[i];
        if (bp >= 0 && frameE[i] >= energyThreshold) {
          firstHist[bp]++;
        }
      }
      let maxFirst = 0;
