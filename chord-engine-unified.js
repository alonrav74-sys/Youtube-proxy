/**
 * 🎹 ChordEngine UNIFIED v14.3 - Extreme Noise Filtering
 * 
 * 🔧 CRITICAL FIXES:
 * 1. Skip first 15% in bass analysis (was 10%)
 * 2. Use top 20% energy frames only (was top 30%)
 * 3. Listen to the third after finding bass tonic
 * 4. Major/minor determined by m3 vs M3 strength (20% bias to major for arpeggios)
 * 
 * 🎯 KEY INSIGHT: Key = Bass Tonic + Third
 * - Bass: Most common STRONG bass note (skip intro, top 20% energy)
 * - Third: m3 (minor) or M3 (major)
 * - Direct spectral analysis from STRONG frames only!
 */
...
class ChordEngineEnhanced {
  ...
  detectTonicFromBass(feats) {
    ...
    // Skip first 15% to avoid intro noise
    const skipFrames = Math.floor(bassPc.length * 0.15);  // Was 0.10, now 0.15
    console.log(`🎸 Bass analysis: skipping first ${skipFrames} frames (15%), threshold: ${threshold.toFixed(4)}`);
  ...
  detectKeyEnhanced(feats, duration) {
    ...
    if (bassTonic.confidence > 0.30) {
      ...
      // Skip first 15% (same as bass analysis)
      const skipFrames = Math.floor(chroma.length * 0.15);  // Was 0.10, now 0.15
