...
class ChordEngine {
  constructor() {
    ...
    console.log('✅ ChordEngine UNIFIED (v5.0.0 - Musical Theory) loaded');
  }
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // 🎓 MUSICAL TONIC DETECTOR - Theory-Based!
    // First, get preliminary timeline for cadence analysis
    console.log('\n🎓 === MUSICAL TONIC DETECTION (Theory-Based) ===\n');
    
    const tempKey = this.estimateKey(feats.chroma, feats.bassPc);
    const preliminaryTimeline = this.chordTrackingWithHMM(feats, tempKey);
    
    // Now use Musical Tonic Detector
    const detector = new MusicalTonicDetector();
    const key = detector.detectTonic(preliminaryTimeline, feats.bassPc, feats.frameE, feats.sr, feats.hop);
    
    console.log(`\n🎼 FINAL KEY: ${this.nameSharp(key.root)}${key.minor ? 'm' : ''}\n`);
