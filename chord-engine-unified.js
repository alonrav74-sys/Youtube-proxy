...
class ChordEngineEnhanced {
  ...
  detectTonicFromBass(feats) {
    ...
    // 🆕 Skip first 10% to avoid intro noise affecting bass analysis
    const skipFrames = Math.floor(bassPc.length * 0.10);
    console.log(`🎸 Bass analysis: skipping first ${skipFrames} frames (10%)`);
  ...
  detectKeyEnhanced(feats, duration) {
    ...
    if (bassTonic.confidence > 0.30) {
      ...
      // Aggregate chroma from musical section only
      ...
      const isMinor = m3Strength > M3Strength;
