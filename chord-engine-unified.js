...
class ChordEngineEnhanced {
    ...
    for (const section of sections) {
      ...
      if (diatonicRatio < 0.60) {
        ...
        if (bestNewKey) {
          modCount++;
          lastKey = bestNewKey;
          if (this.DEBUG) console.log(`🎵 Modulation detected: ${this.NOTES_SHARP[bestNewKey.root]}${bestNewKey.minor ? 'm' : ''} (${(bestRatio * 100).toFixed(0)}% diatonic)`);
        }
      }
    }
  ...
  detectTonicFromBass(feats) {
    ...
    if (this.DEBUG) console.log(`🎸 Bass analysis: skipping first ${skipFrames} frames (dynamic, max 8s), threshold: ${threshold.toFixed(4)}`);
    ...
    if (this.DEBUG) console.log(`🎸 Bass tonic: ${this.NOTES_SHARP[tonicPc]} (${(confidence * 100).toFixed(1)}% confidence)`);
  ...
  detectKeyEnhanced(feats, duration) {
    ...
    if (this.DEBUG) console.log(`🎯 Bass tonic: ${this.NOTES_SHARP[bassTonic.root]} (${(bassTonic.confidence * 100).toFixed(1)}%)`);
    ...
    if (bassTonic.confidence > 0.30) {
      ...
      if (this.DEBUG) console.log(`🎵 Third analysis (strong frames only, top 20%):`);
      if (this.DEBUG) console.log(`   Minor 3rd (${this.NOTES_SHARP[minorThird]}): ${(m3Strength * 100).toFixed(1)}%`);
      if (this.DEBUG) console.log(`   Major 3rd (${this.NOTES_SHARP[majorThird]}): ${(M3Strength * 100).toFixed(1)}%`);
      ...
      if (this.DEBUG) console.log(`🏆 Key: ${this.NOTES_SHARP[root]}${isMinor ? 'm' : ''} (${isMinor ? 'minor' : 'major'} third)`);
      ...
      if (bassTonic.confidence < 0.50) {
        ...
        if (firstPc >= 0) {
          if (this.inKey(firstPc, root, isMinor)) {
            ...
            if (this.DEBUG) console.log(`✅ First chord ${this.NOTES_SHARP[firstPc]} coherent with ${this.NOTES_SHARP[root]}${isMinor?'m':''} - boosting confidence`);
            result.confidence = Math.min(1.0, result.confidence + 0.15);
          } else {
            // First chord not coherent - small boost only
            if (this.DEBUG) console.log(`⚠️ First chord ${this.NOTES_SHARP[firstPc]} not fully coherent with ${this.NOTES_SHARP[root]}${isMinor?'m':''}`);
    ...
    if (this.DEBUG) console.log(`⚠️ Bass not confident enough, using Krumhansl-Schmuckler`);
    ...
    if (ksResult.confidence < 0.60) {
      ...
      if (firstPc >= 0) {
        if (this.inKey(firstPc, ksResult.root, ksResult.minor)) {
          if (this.DEBUG) console.log(`✅ KS + First chord ${this.NOTES_SHARP[firstPc]} coherent - boosting`);
          ksResult.confidence = Math.min(1.0, ksResult.confidence + 0.15);
        } else {
          if (this.DEBUG) console.log(`⚠️ KS + First chord ${this.NOTES_SHARP[firstPc]} not coherent`);
  ...
  estimateFirstChordPc(feats) {
    ...
    if (this.DEBUG) console.log(`🎸 First chord estimation: ${this.NOTES_SHARP[best]} (${(bestVal / used * 100).toFixed(1)}%)`);
