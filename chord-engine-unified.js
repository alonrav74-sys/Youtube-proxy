/**
 * 🎹 ChordEngine UNIFIED v14.2 - Bass-Based Key Detection
 * 
 * 🔧 CRITICAL FIXES:
 * 1. Skip first 10% in bass analysis (avoid intro noise)
 * 2. Listen to the third after finding bass tonic
 * 3. Major/minor determined by m3 vs M3 strength
 * 
 * 🎯 KEY INSIGHT: Key = Bass Tonic + Third
 * - Bass: Most common bass note (skip intro)
 * - Third: m3 (minor) or M3 (major)
...
class ChordEngineEnhanced {
  ...
  detectTonicFromBass(feats) {
    ...
    // Only count strong bass notes (top 30% energy)
    const threshold = this.percentile(frameE, 70);  // Was 40, now 70 = stronger notes only
    
    // Skip first 10% to avoid intro noise
    const skipFrames = Math.floor(bassPc.length * 0.10);
    console.log(`🎸 Bass analysis: skipping first ${skipFrames} frames (10%), threshold: ${threshold.toFixed(4)}`);
  ...
  detectKeyEnhanced(feats, duration) {
    ...
    if (bassTonic.confidence > 0.30) {
      ...
      // Skip first 10% (same as bass analysis)
      const skipFrames = Math.floor(chroma.length * 0.10);
      
      // 🔧 FIX: Only use strong frames for third analysis (ignore weak background)
      const threshold = this.percentile(feats.frameE, 70);
      ...
      console.log(`🎵 Third analysis (strong frames only):`);
