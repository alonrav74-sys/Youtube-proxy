...
class ChordEngine {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // 🎯 NEW: First & Last Chord Analysis
    // Most songs start AND end on the tonic!
    let firstChordRoot = -1, lastChordRoot = -1;
    
    // Find first stable chord (first 5% of song)
    const firstSectionEnd = Math.floor(feats.bassPc.length * 0.05);
    const firstHist = new Array(12).fill(0);
    for (let i = 0; i < firstSectionEnd; i++) {
      const bp = feats.bassPc[i];
      if (bp >= 0) firstHist[bp]++;
    }
    let maxFirst = 0;
    for (let p = 0; p < 12; p++) {
    ...
    if (cadenceTonic >= 0 && maxCadence > 0) {
      const cadencePoints = Math.floor(maxCadence * 30); // Up to 120 points
