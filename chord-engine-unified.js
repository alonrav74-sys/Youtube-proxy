...
class ChordEngine {
  constructor() {
    ...
    this.CHORD_TEMPLATES = {
      ...
      // 🔥 6th chords (jazz/vintage)
      maj6: { intervals: [0, 4, 7, 9], weights: [1.0, 0.9, 0.8, 0.75], label: '6' },
      m6: { intervals: [0, 3, 7, 9], weights: [1.0, 0.9, 0.8, 0.75], label: 'm6' },
      ...
      dom7sharp5: { intervals: [0, 4, 8, 10], weights: [1.0, 0.9, 0.85, 0.8], label: '7#5' },
      dom7flat5: { intervals: [0, 4, 6, 10], weights: [1.0, 0.9, 0.85, 0.8], label: '7b5' },
      dom7sharp9: { intervals: [0, 4, 7, 10, 15], weights: [1.0, 0.9, 0.8, 0.75, 0.6], label: '7#9' },
      dom7flat9: { intervals: [0, 4, 7, 10, 13], weights: [1.0, 0.9, 0.8, 0.75, 0.6], label: '7b9' },
    ...
    console.log('✅ ChordEngine UNIFIED (v3.2.0 - Tonic Master) loaded');
  }
  ...
  /**
   * Check if a chord quality (major/minor) is diatonic for a scale degree
   */
  isDiatonicQuality(degree, key, isMinor) {
    if (key.minor) {
      // Natural minor: i, ii°, III, iv, v, VI, VII
      const minorDegrees = [0, 3, 5]; // i, iv, v (minor)
      const majorDegrees = [2, 7, 10]; // III, VI, VII (major)
      return isMinor ? minorDegrees.includes(degree) : majorDegrees.includes(degree);
    } else {
      // Major: I, ii, iii, IV, V, vi, vii°
      const majorDegrees = [0, 5, 7]; // I, IV, V (major)
      const minorDegrees = [2, 4, 9]; // ii, iii, vi (minor)
      return isMinor ? minorDegrees.includes(degree) : majorDegrees.includes(degree);
    }
  }
  ...
  chordTrackingWithHMM(feats, key) {
    ...
    // For each root, add BOTH major and minor as candidates
    // BUT give preference to diatonic quality via emission score
    for (const r of allRoots) {
      const degreeInKey = this.toPc(r - key.root);
      
      // Add both qualities - HMM will choose based on chroma
      candidates.push({ root: r, label: this.nameSharp(r), isDiatonic: this.isDiatonicQuality(degreeInKey, key, false) });
      candidates.push({ root: r, label: this.nameSharp(r) + 'm', isDiatonic: this.isDiatonicQuality(degreeInKey, key, true) });
    }
    ...
    const emitScore = (i, cand) => {
      ...
      // 🎵 DIATONIC BONUS - prefer diatonic quality
      if (cand.isDiatonic) {
        score += 0.10; // Bonus for diatonic quality
      }
  ...
  async detect(audioBuffer, options = {}) {
    ...
    const key = this.estimateKey(feats.chroma, feats.bassPc);
    console.log(`🎼 Initial key estimate: ${this.nameSharp(key.root)}${key.minor ? 'm' : ''} (confidence: ${key.confidence.toFixed(2)})`);
    
    // 🎯 TONIC REINFORCEMENT: Bass histogram + final note + cadence analysis
    const bassHist = new Array(12).fill(0);
    const chromaDuration = new Array(12).fill(0);
    
    for (let i = 0; i < feats.bassPc.length; i++) {
      const bp = feats.bassPc[i];
      if (bp >= 0) bassHist[bp]++;
      
      // Weight by energy
      const en = feats.frameE[i] || 0;
      const c = feats.chroma[i];
      if (c) {
        for (let p = 0; p < 12; p++) {
          chromaDuration[p] += (c[p] || 0) * en;
        }
      }
    }
    
    // Normalize
    const totalBass = bassHist.reduce((a, b) => a + b, 0) || 1;
    const totalChroma = chromaDuration.reduce((a, b) => a + b, 0) || 1;
    for (let p = 0; p < 12; p++) {
      bassHist[p] /= totalBass;
      chromaDuration[p] /= totalChroma;
    }
    
    // Find most common bass note
    let maxBass = 0, maxBassPc = -1;
    for (let p = 0; p < 12; p++) {
      if (bassHist[p] > maxBass) {
        maxBass = bassHist[p];
        maxBassPc = p;
      }
    }
    
    // Find last stable bass note (last 10% of song)
    let finalBassPc = -1;
    const startFinal = Math.floor(feats.bassPc.length * 0.9);
    const finalHist = new Array(12).fill(0);
    for (let i = startFinal; i < feats.bassPc.length; i++) {
      const bp = feats.bassPc[i];
      if (bp >= 0) finalHist[bp]++;
    }
    let maxFinal = 0;
    for (let p = 0; p < 12; p++) {
      if (finalHist[p] > maxFinal) {
        maxFinal = finalHist[p];
        finalBassPc = p;
      }
    }
    
    // 🎵 CADENCE ANALYSIS: Look for V→I patterns
    const cadenceCandidates = new Array(12).fill(0);
    
    for (let i = 1; i < feats.bassPc.length; i++) {
      const prevBass = feats.bassPc[i - 1];
      const currBass = feats.bassPc[i];
      
      if (prevBass >= 0 && currBass >= 0 && prevBass !== currBass) {
        // Check if this could be V→I (perfect 5th down or perfect 4th up)
        const interval = this.toPc(currBass - prevBass);
        
        // Perfect 5th down = 7 semitones down = 5 semitones up
        // Perfect 4th up = 5 semitones up
        if (interval === 5 || interval === 7) {
          // prevBass is V (dominant), currBass is I (tonic)
          const tonic = currBass;
          
          // Check if there's energy drop (cadence resolution)
          const prevEnergy = feats.frameE[i - 1] || 0;
          const currEnergy = feats.frameE[i] || 0;
          const energyRise = currEnergy > prevEnergy * 0.8; // Allow some drop but not too much
          
          // Weight by position (cadences at end are more important)
          const positionWeight = (i / feats.bassPc.length);
          const weight = energyRise ? (1.0 + positionWeight * 2.0) : 0.5;
          
          cadenceCandidates[tonic] += weight;
        }
      }
    }
    
    // Find strongest cadence target
    let maxCadence = 0, cadenceTonic = -1;
    for (let p = 0; p < 12; p++) {
      if (cadenceCandidates[p] > maxCadence) {
        maxCadence = cadenceCandidates[p];
        cadenceTonic = p;
      }
    }
    
    console.log(`🎵 Bass analysis: Most common=${maxBassPc>=0?this.nameSharp(maxBassPc):'?'} (${(maxBass*100).toFixed(1)}%), Final=${finalBassPc>=0?this.nameSharp(finalBassPc):'?'}`);
    console.log(`🎵 Cadence analysis: Strongest V→I target=${cadenceTonic>=0?this.nameSharp(cadenceTonic):'?'} (${maxCadence.toFixed(2)} cadences)`);
    
    // 🎯 IMPROVED: First & Last Chord Analysis
    // Find the ACTUAL first chord (first strong bass), not histogram of first 5%!
    let firstChordRoot = -1, lastChordRoot = -1;
    
    // Find first STRONG bass note (skip weak intro)
    const energyThreshold = this.percentileLocal(feats.frameE, 40); // 40th percentile
    
    for (let i = 0; i < Math.min(feats.bassPc.length, 100); i++) { // First 100 frames
      if (feats.frameE[i] >= energyThreshold && feats.bassPc[i] >= 0) {
        firstChordRoot = feats.bassPc[i];
        console.log(`🎯 First strong bass found at frame ${i}: ${this.nameSharp(firstChordRoot)}`);
        break;
      }
    }
    
    // If no strong bass in first 100 frames, use histogram of first 10%
    if (firstChordRoot < 0) {
      const firstSectionEnd = Math.floor(feats.bassPc.length * 0.10);
      const firstHist = new Array(12).fill(0);
      for (let i = 0; i < firstSectionEnd; i++) {
        const bp = feats.bassPc[i];
        if (bp >= 0 && feats.frameE[i] >= energyThreshold) {
          firstHist[bp]++;
        }
      }
      let maxFirst = 0;
      for (let p = 0; p < 12; p++) {
        if (firstHist[p] > maxFirst) {
          maxFirst = firstHist[p];
          firstChordRoot = p;
        }
      }
      console.log(`🎯 First chord (histogram method): ${firstChordRoot>=0?this.nameSharp(firstChordRoot):'?'}`);
    }
    
    // Last chord already calculated as finalBassPc
    lastChordRoot = finalBassPc;
    
    console.log(`🎯 First chord: ${firstChordRoot>=0?this.nameSharp(firstChordRoot):'?'}, Last chord: ${lastChordRoot>=0?this.nameSharp(lastChordRoot):'?'}`);
    
    // DECISION LOGIC: Combine all evidence
    const krumhanslTonic = key.root;
    const bassConfidence = maxBass;
    
    // 🎯 ADVANCED SCORING SYSTEM: Each piece of evidence gets weighted points
    // This is MORE ACCURATE than if-else chains!
    
    // 🎯 MUSICAL APPROACH: Final bass note = TONIC!
    // This is the most reliable indicator in 99% of songs!
    
    const tonicScores = new Array(12).fill(0);
    
    // 🎵 PRIMARY: Final bass note (songs END on tonic - DEFINITIVE!)
    if (finalBassPc >= 0) {
      tonicScores[finalBassPc] += 300; // 🔥 ULTRA MASSIVE! This IS the tonic!
      console.log(`📊 🎯 Final bass note: ${this.nameSharp(finalBassPc)} (+300 ABSOLUTE TONIC!)`);
    }
    
    // 🎵 SECONDARY: First chord (songs often START on tonic)
    if (firstChordRoot >= 0) {
      tonicScores[firstChordRoot] += 100; // Strong but not definitive
      console.log(`📊 First chord: ${this.nameSharp(firstChordRoot)} (+100)`);
    }
    
    // 🎵 TERTIARY: Cadence analysis (V→I confirms tonic)
    if (cadenceTonic >= 0 && maxCadence > 0) {
      const cadencePoints = Math.floor(maxCadence * 15); // 🔥 Reduced from 30 to 15!
      tonicScores[cadenceTonic] += cadencePoints;
      console.log(`📊 Cadence V→I: ${this.nameSharp(cadenceTonic)} (+${cadencePoints} from ${maxCadence.toFixed(2)} cadences)`);
    }
    
    // 🎵 SUPPORT: Bass histogram (frequent bass notes)
    if (maxBassPc >= 0) {
      const bassPoints = Math.floor(bassConfidence * 80); // Reduced from 130!
      tonicScores[maxBassPc] += bassPoints;
      console.log(`📊 Bass histogram: ${this.nameSharp(maxBassPc)} (+${bassPoints} from ${(bassConfidence*100).toFixed(1)}%)`);
    }
    
    // 🎵 BACKUP: Krumhansl-Schmuckler (often wrong, minimal weight)
    tonicScores[krumhanslTonic] += 10; // 🔥 Reduced from 15 to 10!
    console.log(`📊 KS suggests: ${this.nameSharp(krumhanslTonic)} (+10 backup only)`);
    
    // 🎵 SUPER CONFIRMATION: First AND Last agree (almost certain!)
    if (firstChordRoot >= 0 && lastChordRoot >= 0 && firstChordRoot === lastChordRoot) {
      tonicScores[firstChordRoot] += 150; // 🔥 Even more bonus!
      console.log(`📊 First & Last AGREE: ${this.nameSharp(firstChordRoot)} (+150 CONFIRMATION!)`);
    }
    
    // Find winner by score
    let bestTonic = krumhanslTonic;
    let bestScore = tonicScores[krumhanslTonic];
    let secondBest = { pc: -1, score: 0 };
    
    for (let p = 0; p < 12; p++) {
      if (tonicScores[p] > bestScore) {
        secondBest = { pc: bestTonic, score: bestScore };
        bestScore = tonicScores[p];
        bestTonic = p;
      } else if (tonicScores[p] > secondBest.score && p !== bestTonic) {
        secondBest = { pc: p, score: tonicScores[p] };
      }
    }
    
    // Show top candidates
    const sortedCandidates = tonicScores
      .map((score, pc) => ({ pc, score, name: this.nameSharp(pc) }))
      .filter(c => c.score > 0)
      .sort((a, b) => b.score - a.score)
      .slice(0, 3);
    
    console.log(`🎯 Tonic scores:`, sortedCandidates.map(c => `${c.name}(${c.score})`).join(', '));
    
    // Apply winner if different from KS
    if (bestTonic !== krumhanslTonic) {
      const margin = bestScore - tonicScores[krumhanslTonic];
      console.log(`✅ Tonic corrected: ${this.nameSharp(krumhanslTonic)} → ${this.nameSharp(bestTonic)} (score: ${bestScore} vs ${tonicScores[krumhanslTonic]}, margin: +${margin})`);
      key.root = bestTonic;
    } else {
      console.log(`✅ Tonic confirmed: ${this.nameSharp(bestTonic)} (score: ${bestScore}${secondBest.pc >= 0 ? `, runner-up: ${this.nameSharp(secondBest.pc)}(${secondBest.score})` : ''})`);
    }
    
    // 🎵 MAJOR vs MINOR decision - ALWAYS check the 3rd!
    // This is the DEFINING characteristic of major/minor!
    
    const minorThird = this.toPc(bestTonic + 3);
    const majorThird = this.toPc(bestTonic + 4);
    
    // Aggregate chroma across entire song
    const avgChroma = new Array(12).fill(0);
    for (const c of feats.chroma) {
      for (let p = 0; p < 12; p++) avgChroma[p] += (c[p] || 0);
    }
    const total = avgChroma.reduce((a, b) => a + b, 0) || 1;
    for (let p = 0; p < 12; p++) avgChroma[p] /= total;
    
    const minorStrength = avgChroma[minorThird];
    const majorStrength = avgChroma[majorThird];
    
    console.log(`🎵 3rd analysis: minor 3rd=${minorStrength.toFixed(3)}, major 3rd=${majorStrength.toFixed(3)}`);
    
    // 🎯 Decision based on which 3rd is stronger
    const threshold = 1.05; // Very small threshold - let the data decide!
    
    if (minorStrength > majorStrength * threshold) {
      // Minor 3rd is stronger → MINOR key
      if (!key.minor) {
        console.log(`✅ Mode detected: ${this.nameSharp(bestTonic)}m (minor 3rd dominant: ${minorStrength.toFixed(3)} vs ${majorStrength.toFixed(3)})`);
        key.minor = true;
      } else {
        console.log(`✅ Mode confirmed: ${this.nameSharp(bestTonic)}m (minor 3rd dominant)`);
      }
    } else if (majorStrength > minorStrength * threshold) {
      // Major 3rd is stronger → MAJOR key
      if (key.minor) {
        console.log(`✅ Mode detected: ${this.nameSharp(bestTonic)} (major 3rd dominant: ${majorStrength.toFixed(3)} vs ${minorStrength.toFixed(3)})`);
        key.minor = false;
      } else {
        console.log(`✅ Mode confirmed: ${this.nameSharp(bestTonic)} (major 3rd dominant)`);
      }
    } else {
      // Very close - use KS as tiebreaker
      console.log(`⚠️ Mode unclear (3rds very close), using KS: ${this.nameSharp(bestTonic)}${key.minor?'m':''}`);
    }
    
    console.log(`🎼 Final key: ${this.nameSharp(key.root)}${key.minor ? 'm' : ''}`);
    ...
    // 🎵 JAZZ BOOST: Detect and reinforce ii-V-I progressions
    timeline = this.detectJazzProgressions(timeline, key);
  ...
  // ============================================
  // 🎵 JAZZ PROGRESSION DETECTION
  // ============================================
  
  /**
   * Detect and reinforce ii-V-I progressions (most common in jazz)
   * Also detects: ii-V, V-I, iii-vi-ii-V-I (turnaround)
   */
  detectJazzProgressions(timeline, key) {
    ...
    for (let i = 0; i < timeline.length - 1; i++) {
      ...
      const next2Root = next2 ? this.parseRoot(next2.label) : -1;
      
      if (currRoot < 0 || nextRoot < 0) continue;
      
      // Calculate intervals relative to key
      const currDegree = this.toPc(currRoot - key.root);
      const nextDegree = this.toPc(nextRoot - key.root);
      const next2Degree = next2Root >= 0 ? this.toPc(next2Root - key.root) : -1;
      
      // Pattern 1: ii-V-I (THE jazz progression)
      if (next2) {
        const isIIminor = (currDegree === 2 && curr.label.includes('m'));
        const isVdom = (nextDegree === 7 && next.label.includes('7') && !next.label.includes('maj7'));
        const isI = (next2Degree === 0);
        
        if (isIIminor && isVdom && isI) {
          console.log(`  🎺 ii-V-I detected: ${curr.label} → ${next.label} → ${next2.label}`);
          curr.confidence = Math.min(100, (curr.confidence || 70) + 15);
          next.confidence = Math.min(100, (next.confidence || 70) + 15);
          next2.confidence = Math.min(100, (next2.confidence || 70) + 15);
          curr.progressionType = 'ii-V-I';
          next.progressionType = 'ii-V-I';
          next2.progressionType = 'ii-V-I';
        }
      }
      
      // Pattern 2: V-I (authentic cadence)
      const isVdom = (currDegree === 7 && curr.label.includes('7'));
      const isI = (nextDegree === 0);
      
      if (isVdom && isI) {
        console.log(`  🎺 V-I detected: ${curr.label} → ${next.label}`);
        curr.confidence = Math.min(100, (curr.confidence || 70) + 10);
        next.confidence = Math.min(100, (next.confidence || 70) + 10);
        curr.progressionType = 'V-I';
        next.progressionType = 'V-I';
      }
      
      // Pattern 3: ii-V (half cadence)
      const isIIminor = (currDegree === 2 && curr.label.includes('m'));
      const isV = (nextDegree === 7);
      
      if (isIIminor && isV) {
        console.log(`  🎺 ii-V detected: ${curr.label} → ${next.label}`);
        curr.confidence = Math.min(100, (curr.confidence || 70) + 10);
        next.confidence = Math.min(100, (next.confidence || 70) + 10);
        curr.progressionType = 'ii-V';
        next.progressionType = 'ii-V';
      }
      
      // Pattern 4: IV-V-I (plagal resolution)
      if (next2) {
        const isIV = (currDegree === 5);
        const isVinPlagal = (nextDegree === 7);
        const isIinPlagal = (next2Degree === 0);
        
        if (isIV && isVinPlagal && isIinPlagal) {
          console.log(`  🎺 IV-V-I detected: ${curr.label} → ${next.label} → ${next2.label}`);
          curr.confidence = Math.min(100, (curr.confidence || 70) + 8);
          next.confidence = Math.min(100, (next.confidence || 70) + 8);
          next2.confidence = Math.min(100, (next2.confidence || 70) + 8);
        }
      }
    }
  ...
  estimateKey(chroma, bassPc = null) {
    const KS_MAJOR = [6.35, 2.23, 3.48, 2.33, 4.38, 4.09, 2.52, 5.19, 2.39, 3.66, 2.29, 2.88];
    const KS_MINOR = [6.33, 2.68, 3.52, 5.38, 2.60, 3.53, 2.54, 4.75, 3.98, 2.69, 3.34, 3.17];
    
    // Aggregate chroma with BASS WEIGHTING
    const agg = new Array(12).fill(0);
    const bassWeight = new Array(12).fill(0);
    
    // Count bass occurrences
    if (bassPc) {
      for (let i = 0; i < bassPc.length; i++) {
        const bp = bassPc[i];
        if (bp >= 0) bassWeight[bp]++;
      }
      const totalBass = bassWeight.reduce((a, b) => a + b, 0) || 1;
      for (let p = 0; p < 12; p++) bassWeight[p] /= totalBass;
    }
    ...
    // 🎯 BOOST: Add bass weight (bass is 3x more important for tonic!)
    if (bassPc) {
      for (let p = 0; p < 12; p++) {
        agg[p] = agg[p] * 0.6 + bassWeight[p] * 0.4; // 60% chroma, 40% bass
      }
    }
  ...
  decorateQualitiesBassFirst(tl, feats, key, mode, decMul = 1.0) {
    ...
    for (const ev of tl) {
      ...
      // 🔥 NEW: Validate altered dominants (7#5, 7b5, 7#9, 7b9)
      // These should ONLY appear in jazz contexts or as secondary dominants
      if (label.includes('7#5') || label.includes('7b5') || label.includes('7#9') || label.includes('7b9')) {
        // Check if this is in a V-I context (secondary dominant)
        const nextEv = tl[tl.indexOf(ev) + 1];
        if (nextEv) {
          const nextRoot = this.parseRoot(nextEv.label);
          if (nextRoot >= 0) {
            const interval = this.toPc(nextRoot - root);
            // Perfect 5th down = dominant resolution
            if (interval === 5 || interval === 7) {
              console.log(`   ✓ Altered dominant confirmed: ${label} → ${nextEv.label} (V→I resolution)`);
            } else {
              // No resolution - might be false positive, downgrade to regular dom7
              const baseLabel = rootName + '7';
              console.log(`   ⚠️ Altered dominant without resolution: ${label} → ${baseLabel}`);
              label = baseLabel;
            }
          }
        }
  ...
  addInversionsIfNeeded(tl, feats, bassSens = 1.25) {
    ...
    return tl.map(ev => {
      ...
      // 🎯 VERY CONSERVATIVE: Require bass to be MUCH stronger than root
      // Bass must be 2.5× stronger to consider inversion
      if (bassNotVotes < rootVotes * 2.5) {
        console.log(`   ⚠️ Inversion rejected: bass not dominant enough (${bassNotVotes} vs ${rootVotes})`);
        return ev;
      }
      
      // 🎯 CRITICAL MUSICAL LOGIC:
      // If 3rd is in bass, this is likely the RELATIVE chord, not an inversion!
      // Example: Am (A-C-E) with C bass → This is C major (C-E-G+A)!
      // Example: C (C-E-G) with A bass → This is Am (A-C-E)!
      ...
      // 🔄 If minor 3rd is in bass of minor chord → Change to relative MAJOR!
      if (bassInterval === 3 && isMinor && !has7) {
        const newRoot = dominantBass;
        const newLabel = this.nameSharp(newRoot);
        console.log(`   🔄 Chord corrected: ${ev.label} → ${newLabel} (m3 in bass = relative major)`);
        return { ...ev, label: newLabel };
      }
      
      // 🔄 If major 6th is in bass of major chord → Change to relative MINOR!
      if (bassInterval === 9 && !isMinor && !has7) {
        const newRoot = dominantBass;
        const newLabel = this.nameSharp(newRoot) + 'm';
        console.log(`   🔄 Chord corrected: ${ev.label} → ${newLabel} (M6 in bass = relative minor)`);
        return { ...ev, label: newLabel };
      }
      
      // 🎸 Only allow 5th or 7th in bass as TRUE inversions
      let allowedBassIntervals = [7]; // Perfect 5th
      
      if (has7 || hasMaj7) {
        allowedBassIntervals.push(hasMaj7 ? 11 : 10); // 7th
      }
      
      if (allowedBassIntervals.includes(bassInterval)) {
        const bassNote = this.nameSharp(dominantBass);
        console.log(`   🎸 Inversion: ${ev.label} → ${ev.label}/${bassNote} (bass: ${this.nameSharp(dominantBass)})`);
  ...
  removeRedundantChords(tl, bpm) {
    const spb = 60 / Math.max(60, bpm || 120);
    const minDuration = spb * 2.0; // 🔥 Minimum 2 FULL beats between same chords!
    ...
    for (const ev of tl) {
      // Always keep if label is different
      if (ev.label !== lastLabel) {
        out.push(ev);
        lastLabel = ev.label;
        lastTime = ev.t;
        continue;
      ...
      // Same label - only keep if enough time has passed (2+ beats)
      const timeSinceLast = ev.t - lastTime;
    ...
    console.log(`🔍 Removed ${tl.length - out.length} redundant chords (${tl.length} → ${out.length})`);
    return out;
  }
...
if (typeof window !== 'undefined') {
  ...
  console.log('✅ ChordEngine UNIFIED (v3.2.0 - Tonic Master) - Single file with filtering!');
}
