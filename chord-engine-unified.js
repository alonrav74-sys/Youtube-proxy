...
class ChordEngineEnhanced {
  ...
  detectMusicStart(bassPc, chroma) {
    ...
    for (let start = 0; start < bassPc.length - windowSize; start += 10) {
      ...
      if (periodicity > 0.3) {
        console.log(`🎵 Music starts at frame ${start} (periodicity: ${periodicity.toFixed(2)})`);
        return start;
      }
    }
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // 🆕 v15.0: Get initial chords WITHOUT key (to find music start)
    let preliminaryKey = this.detectKeyEnhanced(feats, audioData.duration);
    let preliminaryTimeline = this.chordTrackingHMMHybrid(feats, preliminaryKey, opts.bassMultiplier, false);
    
    // 🆕 v15.0: NOW use structural analysis on chords
    let key = this.detectKeyStructural(preliminaryTimeline, feats);
    key.confidence = preliminaryKey.confidence; // inherit confidence
