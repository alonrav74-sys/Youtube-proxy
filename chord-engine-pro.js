...
class ChordEnginePro extends ChordEngine {
  constructor() {
    ...
    this.models = {
      essentia: null,
      initialized: false,
      failed: false  // 🆕 Track if loading failed
    };
  ...
  /**
   * 🎛️ Initialize Essentia.js (lazy loading with timeout & fallback)
   */
  async initAIModels() {
    if (this.models.initialized) return;
    if (this.models.failed) {
      console.warn('⚠️ Essentia previously failed, using Fast mode');
    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Essentia timeout after 10s')), 10000)
        )
      ]);
      ...
      console.warn('⚠️ Essentia.js not available, falling back to Fast mode:', err.message);
      this.currentMode = this.MODE.FAST;
      this.models.failed = true; // ← לא ננסה שוב
      this.models.initialized = true; // ← מסמן כמאותחל
    }
  }
  ...
  async loadEssentia() {
    ...
    console.log('📦 Loading Essentia from local files...');
    
    await this.loadScript('./essentia-wasm.web.js');
    await this.loadScript('./essentia.js-core.js');
