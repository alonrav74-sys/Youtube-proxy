 ...
 * @version 2.0.0 - Confidence Booster
 * @author Alon
 */
...
class ChordEnginePro extends ChordEngine {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    this.stats = {
      ...
      temporalFixes: 0,
      keyConstrainedFixes: 0,  // 🆕 New stat
      secondaryDominants: 0     // 🆕 New stat
    };
    ...
    const feats = this.extractFeatures({ x, sr }, bpm);
  ...
  /**
   * 🎼 Extract features (your original algorithm)
   */
  extractFeatures(audio, bpm) {
    const hopSec = 60 / (bpm * 4);
    const hop = Math.round(hopSec * audio.sr);
    
    return super.extractChroma(audio.x, audio.sr, hop);
  }
...
console.log('✅ ChordEngine Pro (Confidence Booster) loaded!');
