...
class ChordEnginePro extends ChordEngine {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // 5️⃣ Key detection - NOW using first and last chords!
  ...
  /**
   * 🎵 Build Circle of Fifths (diatonic scale)
   */
  ...
  decorateQualitiesBassFirst(timeline, feats, key, harmonyMode, bassConfidence) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      const prevChord = i > 0 ? timeline[i - 1] : null;
      const nextChord = i < timeline.length - 1 ? timeline[i + 1] : null;
      
      // Classify as structural or ornamental
      chord.ornamentType = this.classifyChordType(chord, prevChord, nextChord, key);
      
      decorated.push(chord);
    }
  ...
  /**
   * 🎯 Classify chord as structural or ornamental
   */
  classifyChordType(chord, prevChord, nextChord, key) {
    const label = chord.label || '';
    
    // Structural chords (main harmonic pillars):
    // I, IV, V and their variants
    const structuralPattern = /^[A-G][b#]?(m|maj|M|\+|°|dim)?$/;
    
    // Check if it's a diatonic chord in the key
    const isDiatonic = this.isInKey(label, key);
    
    // Check duration - longer chords are more likely structural
    const duration = chord.endFrame - chord.fi;
    const isLong = duration > 3; // more than 3 frames
    
    // Ornamental patterns:
    // - Very short duration (passing chords)
    // - 7th, 9th, 11th, 13th extensions
    // - Diminished, augmented (unless long)
    // - Sus chords (unless long)
    const isExtended = /[79]|11|13|sus|add/.test(label);
    const isDimAug = /dim|°|\+|aug/.test(label);
    const isShort = duration <= 2;
    
    // Decision logic:
    if (isShort && !isDiatonic) return 'ornamental';
    if (isExtended && isShort) return 'ornamental';
    if (isDimAug && !isLong) return 'ornamental';
    
    // Default to structural for main triads and long chords
    if (isDiatonic && (structuralPattern.test(label) || isLong)) {
      return 'structural';
    }
    
    // If uncertain, check context
    if (prevChord && nextChord) {
      // If surrounded by same chord, it's likely ornamental passing
      if (prevChord.label === nextChord.label && prevChord.label !== label) {
        return 'ornamental';
      }
    }
  ...
  findCadences(timeline) {
    ...
    for (let i = 0; i < timeline.length - 1; i++) {
      ...
      const isPerfectCadence = interval === 7; // V → I
      const isPlagalCadence = interval === 5;  // IV → I
