/**
 * ChordEngineEnhanced v14.36 - FIXED2
 * ✅ Fixed backtrack issue causing undefined labels
 */
...
class ChordEngineEnhanced {
  ...
  detectKeyEnhanced(feats) {
    ...
    if (bassTonic.confidence > 0.25) {
      ...
      // ✅ NEW: If rest analysis strongly disagrees, reconsider
      if (restRoot >= 0 && restRoot !== root) {
        // Check if restRoot is the V of detected root (common mistake)
        const fifthOfRest = toPc(restRoot + 7);
        if (fifthOfRest === root) {
          // We probably detected V instead of I - switch to restRoot
          return { root: restRoot, minor: !!isMinor, confidence: confidence * 0.9 };
        }
      }
