...
const SyncEngine = {
  ...
  refreshSheetTabView(state, words, text, language, capo, sanitizeLabel, applyCapoToLabel, sheetEl) {
    ...
    // Build the sheet
    const html = this.buildChordSheet(chords, words, isRTL, gateOffset);
   ...
   */
  buildChordSheet(chords, words, isRTL, gateOffset) {
    if (!words || words.length === 0) {
      return '<div style="color:#cbd5e1;padding:20px">⏳ ממתין למילים...</div>';
    }
    
    // Group words into lines
    const lines = this.groupIntoLines(words, gateOffset, isRTL);
   ...
   */
  groupIntoLines(words, gateOffset, isRTL) {
    const lines = [];
    let currentLine = [];
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      currentLine.push({
        text: text,
        time: word.start + gateOffset,
        end: (word.end || word.start + 0.3) + gateOffset
      });
  ...
  buildLine(lineWords, allChords, isRTL) {
    ...
    const words = isRTL ? [...lineWords].reverse() : lineWords;
    
    // Find chords that belong to this line
    const lineStart = words[0].time;
    const lineEnd = words[words.length - 1].end;
    
    const lineChordsRaw = allChords.filter(ch => 
      ch.time >= lineStart - 0.5 && ch.time <= lineEnd + 0.5
    );
    
    // Reverse chords for RTL
    const lineChords = isRTL ? [...lineChordsRaw].reverse() : lineChordsRaw;
    ...
    for (const chord of lineChords) {
      ...
      for (let i = 0; i < words.length; i++) {
        ...
        // Chord happens during or near this word
        if (chord.time >= word.time - 0.2 && chord.time <= word.end + 0.2) {
          ...
          if (wordProgress > 0.3) {
            position += Math.floor(word.text.length * Math.min(wordProgress, 0.8));
          }
    ...
    // Build HTML
    return `<div class="chord-line">${escapeHtml(chordLine)}</div>\n<div class="lyric-line">${escapeHtml(lyricText)}</div>\n`;
  }
};
