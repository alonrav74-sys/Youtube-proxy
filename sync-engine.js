...
const SyncEngine = {
  ...
  refreshSheetTabView(state, whisperWords, whisperText, detectedLanguage, capoValue, sanitizeLabel, applyCapoToLabel, targetElement) {
    ...
    // 🔍 DEBUG: Log first few items to check timing
    console.log('🔍 Sync Debug - First 5 items:');
    allItems.slice(0, 5).forEach(item => {
      if (item.type === 'chord') {
        console.log(`  ${item.time.toFixed(2)}s: CHORD ${item.label} (original: ${item.originalTime.toFixed(2)}s)`);
      } else {
        console.log(`  ${item.time.toFixed(2)}s: WORD "${item.text}"`);
      }
    });
      ...
      if (isRTL) {
        ...
        for (let w = 0; w < words.length; w++) {
          ...
          for (const ch of chords) {
            // Check if chord is near this word (within ±0.5s window)
          ...
          // 🔍 DEBUG: Log matching for first line only
          if (lines.indexOf(line) === 0 && w < 3) {
            console.log(`🔍 RTL Match: word="${word.text}" at ${word.time.toFixed(2)}s → chord=${closestChord?.label || 'none'} (dist=${minDist.toFixed(2)}s)`);
          }
