...
const SyncEngine = {
  ...
  refreshSheetTabView(state, whisperWords, whisperText, detectedLanguage, capoValue, sanitizeLabel, applyCapoToLabel, targetElement) {
    ...
    for (const line of lines) {
      ...
      if (isRTL) {
        // RTL: Build table with cells from right to left
        ...
        for (let i = words.length - 1; i >= 0; i--) {
          ...
          const chord = chords.find(ch => {
            ...
            return ch.time >= word.time && ch.time <= wordEnd;
          });
          
          const chordLabel = chord ? this.escapeHtml(chord.label) : '';
          html += `<td style="color:#38bdf8;font-weight:700;font-size:16px;padding:2px 8px;text-align:center;font-family:monospace">${chordLabel}</td>`;
        }
        html += `</tr>`;
        ...
        for (let i = words.length - 1; i >= 0; i--) {
          const word = words[i];
          html += `<td style="color:#ffffff;font-size:18px;padding:2px 8px;text-align:center">${this.escapeHtml(word.text)}</td>`;
        }
        html += `</tr>`;
