// chord-engine.js - Standalone Chord Detection Engine
// מנוע זיהוי אקורדים מקומי - עובד בדפדפן!
...
class ChordEngine {
  ...
  // 1. Estimate key using Krumhansl-Schmuckler
  estimateKey(chroma) {
  ...
  // 4. Add extensions (7ths, 9ths, etc)
  ...
  analyze(features, bpm, config = {}) {
    ...
    // 2. Detect mode
    const mode = this.detectMode(chroma, key);
    console.log(`Mode: ${mode}`);
    
    // 3. Build chords
    let timeline = this.buildChordsFromBass(bassPc, chroma, frameE, key, bpm, hop, sr);
    ...
    if(timeline.length === 0) {
      ...
      timeline.unshift({
        ...
        endFrame: Math.min(10, chroma.length),
    ...
    // 4-9. Process timeline
    timeline = this.decorateQualities(timeline, extMode, decSens);
    timeline = this.addInversions(timeline, bassPc, bassSens);
