/**
 * ChordEngine - Integrated Musical Intelligence Version
 * 🎸 Enhanced with robust tonic detection and harmonic validation
 * 
 * @version 4.0.0
 * @author Alon
 */
...
class ChordEngine {
  ...
  getDiatonicChords(tonic, mode) {
    ...
    if (mode === 'major') {
      ...
      for (let i = 0; i < scale.length; i++) {
        ...
        chords.push(this.NOTES_SHARP[root] + qualities[i]);
      }
    } else {
  ...
  estimateKey(chroma) {
    ...
    // Prefer K-S if high confidence, otherwise validate with peaks
    if (ksKey.confidence > 0.7) {
      return ksKey;
    } else if (ksKey.root === peaksKey.root) {
      return { ...ksKey, confidence: (ksKey.confidence + peaksKey.confidence) / 2 };
    } else {
      // Choose the one with higher confidence
      return ksKey.confidence > peaksKey.confidence ? ksKey : peaksKey;
    }
  }
  ...
  estimateKeyKS(chroma) {
    ...
    // Normalize confidence (K-S scores typically 3-5)
  ...
  buildChordsFromBass(feats, key, bpm){
    ...
    while(i < bassPc.length){
      ...
      // Check if bass is harmonically allowed
      ...
      // Use scale knowledge if unclear
  ...
  decideMajorMinorFromChroma(root, avgChroma) {
    ...
    return minor3rd >= majorScore * 0.85;
  }
  ...
  /**
   * 🔥 Classify chords - only long chords are structural
   */
...
if (typeof window !== 'undefined') {
  ...
  console.log('✅ ChordEngine v4.0 (Integrated Musical Intelligence) loaded');
}
