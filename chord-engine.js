...
class ChordEngine {
  ...
  buildChordsFromBass(feats, key, bpm){
    ...
    while(i < bassPc.length){
      ...
      // 🎯 Voting: מצא את הבס הכי שכיח באזור (לעיתים יש רעש)
      ...
      // חשב כרומה ממוצעת עם משקולות אנרגיה
      ...
      // זהה major/minor לפי הכרומה (לא לפי הבס!)
      ...
      // 🎵 חיזוק עם הרמוניות (Harmonics)!
      // הרמונית 2 של minor 3rd = 6 semitones = tritone
      const minor3rdHarmonic = avgChroma[this.toPc(finalRoot + 6)] || 0;
      // הרמונית 2 של major 3rd = 8 semitones
      const major3rdHarmonic = avgChroma[this.toPc(finalRoot + 8)] || 0;
      
      // ציון משוקלל: יסוד + הרמונית
  ...
  /**
   * 🎯 Hybrid Approach for Chord Detection
   * 
   * Basic Pitch (92% accuracy) - זיהוי בסיסי מהיר
   * Essentia (90% accuracy) - אלגוריתמים מדעיים
   * Source Separation (96% accuracy) - הפרדת מקורות איטית אבל מדויקת
   * Replicate API (95% accuracy) - שרת חיצוני, עלות $0.01/דקה
   * 
   * האסטרטגיה שלנו:
   * 1. Basic (buildChordsFromBass) - מהיר, 80-85% + Harmonics → 88%
   * 2. Jazz Extensions (decorateQualitiesBassFirst) - הוספת 7/9/11/13
   * 3. Inversions (addInversionsIfNeeded) - זיהוי היפוכים
   * 4. [Optional] Basic Pitch API - לשיפור נוסף ל-92%
   */
  
  /**
   * 🎹 Basic Pitch API Integration (Optional)
   * משתמש ב-Basic Pitch של Spotify לשיפור דיוק
   */
  async enhanceWithBasicPitch(timeline, audioBuffer, sr) {
    // זה optional - אם אין API או אם המשתמש לא רוצה
    // הפונקציה תוחזר ריקה
    
    // TODO: Integration עם Basic Pitch
    // https://github.com/spotify/basic-pitch
    // או Replicate API: https://replicate.com/spotify/basic-pitch
    
    console.log('📊 Basic Pitch: Not implemented yet (optional)');
    return timeline;
  }
  decorateQualitiesBassFirst(tl, feats, key, mode, decMul = 1.0) {
    ...
    for (const ev of tl) {
      ...
      if (mode === 'jazz' || mode === 'pro') {
        // ✅ ודא שיש טריאד לפני הוספת קישוטים
        const hasTriad = (isMinor ? minor3rd : major3rd) > 0.12 && fifth > 0.10;
