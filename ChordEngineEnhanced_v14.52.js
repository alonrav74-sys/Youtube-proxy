...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log(`   TONIC (before mode): ${this.NOTES_SHARP[tonic.root]}${tonic.minor ? 'm' : ''} (confidence: ${tonic.confidence}%)\n`);
    
    // ✅ GPT RECOMMENDATION: Finalize mode (major/minor) using ALL evidence
    key = this.finalizeModeFromTimeline(key, tonic, timeline, feats);
    
    // ✅ GPT SUGGESTION 1: Check if ROOT needs to change (mode already decided above)
    if (tonic.root !== key.root) {
      const proposedKey = { 
        root: tonic.root, 
        minor: key.minor, // Use mode from finalizeModeFromTimeline!
        confidence: key.confidence 
      };
      ...
      if (this.canChangeKey(proposedKey, tonic.confidence)) {
        ...
        // Re-finalize mode after re-run
        key = this.finalizeModeFromTimeline(key, { root: key.root, minor: key.minor }, timeline, feats);
        timings.rerunTonic = now() - tRe2;
      }
    }
    
    // ✅ GPT SUGGESTION 2: Protect minor tonic (sanity check only, doesn't change mode)
    if (key.minor) {
      const protectedKey = this.protectMinorTonic(timeline, key, feats);
      if (!protectedKey.minor && key.minor) {
        console.log(`   ⚠️ protectMinorTonic wanted to change mode, but finalizeModeFromTimeline is authoritative`);
      }
    }
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    // ✅ CRITICAL FIX: 3 categories for chords
    // 1. Diatonic (in-key with correct quality) → strongest bonus
    // 2. Common borrowed (secondary dominants, modal interchange) → medium bonus
    // 3. Rare/wrong → no bonus or penalty
    
    const scale = key.minor ? this.MINOR_SCALE : this.MAJOR_SCALE;
    const qualities = key.minor ? ['m','dim','','m','m','',''] : ['','m','m','','','m','dim'];
    const diatonicPcs = scale.map(s => toPc(key.root + s));
    
    // Helper: Is this a common borrowed chord?
    const isCommonBorrowed = (root, type, keyRoot, keyIsMinor) => {
      const rel = toPc(root - keyRoot);
      
      if (keyIsMinor) {
        // In minor: III, VI, VII (from natural minor) are common
        if ((rel === 3 || rel === 8 || rel === 10) && type === 'major') return true;
        // V (from harmonic/melodic minor) is common
        if (rel === 7 && type === 'major') return true;
      } else {
        // In major: iv, bVII, bVI are common borrowed from parallel minor
        if (rel === 5 && type === 'minor') return true;  // iv
        if (rel === 10 && type === 'major') return true; // bVII
        if (rel === 8 && type === 'minor') return true;  // bVI
        
        // Secondary dominants are VERY common!
        // V/ii (E in C), V/iii (F# in C), V/IV (C in C), V/V (D in C), V/vi (E in C)
        // These are major chords a fifth above diatonic chords
        
        // E in C = V/vi (very common!)
        if (rel === 4 && type === 'major') return true;  // E in C
        // D in C = V/V (very common!)
        if (rel === 2 && type === 'major') return true;  // D in C
        // A in C = V/ii (common in progressions like Am-A-Dm)
        if (rel === 9 && type === 'major') return true;  // A in C
      }
    ...
    for (const cand of candidates) {
      const rootIsDiatonic = diatonicPcs.includes(cand.root);
      
      if (rootIsDiatonic) {
        // Root is in scale, check quality
        const rel = toPc(cand.root - key.root);
        const idx = scale.indexOf(rel);
        
        if (idx >= 0) {
          const expectedQuality = qualities[idx];
          const actualIsMinor = cand.type === 'minor';
          const expectedIsMinor = (expectedQuality === 'm' || expectedQuality === 'dim');
          
          if (actualIsMinor === expectedIsMinor) {
            // Perfect diatonic match!
            cand.diatonic = true;
            cand.borrowed = false;
          } else if (isCommonBorrowed(cand.root, cand.type, key.root, key.minor)) {
            // Common borrowed (e.g., E in C major)
            cand.diatonic = false;
            cand.borrowed = 'common';
          } else {
            // Rare/wrong quality
            cand.diatonic = false;
            cand.borrowed = false;
        ...
        // Root not in scale - check if common borrowed
        if (isCommonBorrowed(cand.root, cand.type, key.root, key.minor)) {
          cand.diatonic = false;
          cand.borrowed = 'common';
        } else {
          cand.diatonic = false;
    ...
    const emitScore = (i, cand) => {
      ...
      // ✅ SMART BONUSES: Let chroma guide, but prefer logical chords
      // 1. Diatonic (Am in C): +0.18 - strong preference
      // 2. Common borrowed (E in C, V/vi): +0.12 - medium preference
      // 3. Rare/wrong: +0.00
      
      // This way: if chroma is clear (E has G#), it can beat Em even with diatonic bonus
      // But if chroma is ambiguous, prefer diatonic
      
      if (cand.diatonic) {
        score += 0.18; // Diatonic: strong preference but not overpowering
      } else if (cand.borrowed === 'common') {
        score += 0.12; // Common borrowed: E in C gets decent bonus
      }
    ...
    const transitionCost = (a, b) => {
      ...
      let cost = 0.4 + 0.08 * dist; // ✅ DECLARE cost FIRST!

      // ✅ CRITICAL FIX: Block truly forbidden/illogical chords
      // But allow common borrowed chords like bVII, iv in major, etc.
      const isForbidden = (chordRoot) => {
        const rel = toPc(chordRoot - key.root);
        
        if (key.minor) {
          // In minor keys, these are extremely rare:
          if (rel === 1) return true;   // b2 (e.g., C in Bm)
          if (rel === 6) return true;   // b5/tritone (e.g., F in Bm)
          if (rel === 11) return true;  // M7 (e.g., A# in Bm) - very rare
        } else {
          // In major keys, these are extremely rare:
          if (rel === 1) return true;   // #1/b2 (e.g., C# or Db in C)
          if (rel === 6) return true;   // #4/b5 tritone (e.g., F# in C)
          if (rel === 8) return true;   // #5/b6 (e.g., G# or Ab in C)
          // Note: rel===10 is bVII (Bb in C) - very common in pop/rock, DON'T block!
        }
        
        return false;
      };
      
      // Heavy penalty for forbidden chords (but don't completely block)
      if (isForbidden(b.root)) {
        cost += 2.0; // Very high penalty
      }
  ...
  // ✅ Helper: Get expected chord quality in a key
  getExpectedQuality(chordRoot, key) {
    const toPc = n => ((n % 12) + 12) % 12;
    const scale = key.minor ? this.MINOR_SCALE : this.MAJOR_SCALE;
    const qualities = key.minor ? ['m','dim','','m','m','',''] : ['','m','m','','','m','dim'];
    const rel = toPc(chordRoot - key.root);
    const idx = scale.indexOf(rel);
    return idx >= 0 ? qualities[idx] : null;
  }
  ...
  reinforceChordsByBassAnd135(timeline, feats, key) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // 5. ✅ NEW: SCALE PREFERENCE - huge bonus if chord matches expected quality!
      
      let shouldBeMinor = currentIsMinor; // Default to current
      let confidence = 0;
      
      // ✅ CRITICAL FIX: What does the SCALE expect?
      // In C major: A should be Am (vi)
      // In Bm: D should be D (III)
      const expectedQuality = this.getExpectedQuality(root, key);
      const scaleExpectsMinor = (expectedQuality === 'm' || expectedQuality === 'dim');
      ...
      // ✅ SCALE PREFERENCE - this is VERY important for simple diatonic songs!
      if (scaleExpectsMinor) {
        shouldBeMinor = true;
        confidence += 35; // Heavy bias toward scale
        console.log(`   📊 Scale expects ${this.NOTES_SHARP[root]}m → +35 for minor`);
      } else if (expectedQuality === '') {
        // Scale expects major
        shouldBeMinor = false;
        confidence += 35;
        console.log(`   📊 Scale expects ${this.NOTES_SHARP[root]} → +35 for major`);
      }
      ...
      if (root === 9 && key.root === 0) {
        // This is A in C major - should be Am!
        console.log(`   🔍 DEBUG A vs Am in C major:`);
        console.log(`      m3 (C): ${chromam3.toFixed(3)}, M3 (C#): ${chromaM3.toFixed(3)}`);
        console.log(`      m3Strong: ${m3Strong}, M3Strong: ${M3Strong}`);
        console.log(`      m3StrongerThanM3: ${m3StrongerThanM3}, M3StrongerThanm3: ${M3StrongerThanm3}`);
        console.log(`      expectedQuality: ${expectedQuality}, scaleExpectsMinor: ${scaleExpectsMinor}`);
        console.log(`      confidence so far: ${confidence}, shouldBeMinor: ${shouldBeMinor}`);
      }
      ...
      if (confidence >= 50 && shouldBeMinor !== currentIsMinor) {
        const newLabel = this.NOTES_SHARP[root] + (shouldBeMinor ? 'm' : '');
        
        console.log(`   🔧 ${chord.label} → ${newLabel} (confidence=${confidence}, bass=${bass === root ? this.NOTES_SHARP[bass] : 'X'}, m3=${chromam3.toFixed(2)}, M3=${chromaM3.toFixed(2)}, 5=${chroma5.toFixed(2)})`);
  ...
  // ✅ GPT RECOMMENDATION: Single source of truth for mode (major/minor)
  // This runs ONCE after all timeline is built, using ALL available evidence
  finalizeModeFromTimeline(key, tonic, timeline, feats) {
    const toPc = n => ((n % 12) + 12) % 12;
    
    console.log('\n🎼 FINAL MODE DECISION (major/minor):');
    
    // 1. Count chords by root and quality, weighted by duration
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      rootDur[r] += dur;
      if (/m(?!aj)/.test(ch.label)) {
        minorDur[r] += dur;
      } else if (!/dim|sus|m7b5/.test(ch.label)) {
        majorDur[r] += dur;
      }
    }
    ...
    const minorRatio = minorDur[root] / totDur;
    const majorRatio = majorDur[root] / totDur;
    
    console.log(`   Root: ${this.NOTES_SHARP[root]}`);
    console.log(`   Duration: ${minorDur[root].toFixed(1)}s minor, ${majorDur[root].toFixed(1)}s major`);
    console.log(`   Ratio: ${(minorRatio * 100).toFixed(0)}% minor, ${(majorRatio * 100).toFixed(0)}% major`);
    
    // 2. Heavy weight to first and last chord on tonic root
    let firstIsMinor = false, lastIsMinor = false;
    const chordsOnRoot = timeline.filter(ch => ch && this.parseRoot(ch.label) === root);
    
    if (chordsOnRoot.length) {
      const first = chordsOnRoot[0];
      const last = chordsOnRoot[chordsOnRoot.length - 1];
      firstIsMinor = /m(?!aj)/.test(first.label);
      lastIsMinor = /m(?!aj)/.test(last.label);
      console.log(`   First on root: ${first.label} → ${firstIsMinor ? 'minor' : 'major'}`);
      console.log(`   Last on root: ${last.label} → ${lastIsMinor ? 'minor' : 'major'}`);
    }
    
    let scoreMinor = minorRatio * 100;
    let scoreMajor = majorRatio * 100;
    
    if (firstIsMinor) { scoreMinor += 25; console.log(`   +25 minor (first)`); }
    if (lastIsMinor) { scoreMinor += 25; console.log(`   +25 minor (last)`); }
    if (!firstIsMinor && chordsOnRoot.length) { scoreMajor += 15; console.log(`   +15 major (first)`); }
    if (!lastIsMinor && chordsOnRoot.length) { scoreMajor += 15; console.log(`   +15 major (last)`); }
    
    // 3. Consider bass tonic hint
    const bassTonic = this.detectTonicFromBass(feats || {});
    if (bassTonic && bassTonic.minorHint !== undefined) {
      const bassBonus = bassTonic.bassMinorConfidence * 40;
      if (bassTonic.minorHint) {
        scoreMinor += bassBonus;
        console.log(`   +${bassBonus.toFixed(0)} minor (bass hint)`);
      } else {
        scoreMajor += bassBonus;
        console.log(`   +${bassBonus.toFixed(0)} major (bass hint)`);
      }
    }
    
    // 4. Consider existing key hint (light weight)
    if (key && key.minor !== undefined) {
      const keyBonus = (key.confidence || 0.5) * 20;
      if (key.minor) {
        scoreMinor += keyBonus;
        console.log(`   +${keyBonus.toFixed(0)} minor (key hint)`);
      } else {
        scoreMajor += keyBonus;
        console.log(`   +${keyBonus.toFixed(0)} major (key hint)`);
      }
    }
    
    const isMinor = scoreMinor > scoreMajor;
    const confidence = Math.max(
      key?.confidence || 0.5,
      Math.min(1.0, Math.abs(scoreMinor - scoreMajor) / 200)
    );
    
    console.log(`   SCORES: Minor=${scoreMinor.toFixed(0)}, Major=${scoreMajor.toFixed(0)}`);
    console.log(`   ✅ DECISION: ${this.NOTES_SHARP[root]}${isMinor ? 'm' : ''} (confidence: ${(confidence * 100).toFixed(0)}%)\n`);
