...
class EnhancedKeyDetection {
  ...
  /**
   * Check if this is relative minor/major confusion (any pair)
   */
  static isRelativeMinorConfusion(key) {
    // All relative minor/major pairs (3 semitones apart)
    ...
    const root = key.root;
    // Check if this root is part of any relative pair
    return relativePairs.some(pair => pair.includes(root));
  }
  ...
  static analyzeBoundaryChords(timeline, key) {
    ...
    // First chord (STRONG indicator - usually the key!)
    // Check first 5 chords to handle inversions/passing chords
    let firstChordScore = 0;
    for (let i = 0; i < Math.min(5, timeline.length); i++) {
      const chord = timeline[i];
      if (!chord || !chord.label) continue;
      
      const root = this.parseRoot(chord.label);
      const isMinor = chord.label.includes('m') && !chord.label.includes('maj');
      
      // Get relative major/minor roots
      const relativeMinorRoot = this.toPc(key.root - 3);
      const relativeMajorRoot = key.root;
      
      // If chord matches major tonic
      if (root === relativeMajorRoot && !isMinor) {
        firstChordScore = 30;
        majorScore += 30;
        console.log(`      Chord ${i+1} is ${this.noteNames[root]} (major tonic - strong evidence)`);
        break;
      }
      
      // If chord matches minor tonic
      if (root === relativeMinorRoot && isMinor) {
        firstChordScore = 30;
        minorScore += 30;
        console.log(`      Chord ${i+1} is ${this.noteNames[root]}m (minor tonic - strong evidence)`);
        break;
      }
    }
    
    // Last chord (STRONGER evidence - songs usually end on tonic!)
    ...
    if (lastChord && lastChord.label) {
      ...
      // If last chord matches major tonic
      if (root === relativeMajorRoot && !isMinor) {
        majorScore += 35; // Even stronger weight
        console.log(`      Last chord is ${this.noteNames[root]} (major tonic - very strong evidence)`);
      }
      
      // If last chord matches minor tonic
      if (root === relativeMinorRoot && isMinor) {
        minorScore += 35;
        console.log(`      Last chord is ${this.noteNames[root]}m (minor tonic - very strong evidence)`);
      }
    }
