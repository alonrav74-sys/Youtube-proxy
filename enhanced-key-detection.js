...
class EnhancedKeyDetection {
  ...
  static analyzeBoundaryChords(timeline, key) {
    ...
    if (firstChord && firstChord.label) {
      ...
      const relativeMinorRoot = this.toPc(key.root - 3); // 3 semitones down
      const relativeMajorRoot = key.root;
      
      // If first chord matches major tonic
      if (root === relativeMajorRoot && !isMinor) {
        majorScore += 30;
        console.log(`      First chord is ${this.noteNames[root]} (major tonic - strong evidence)`);
      }
      
      // If first chord matches minor tonic
      if (root === relativeMinorRoot && isMinor) {
        minorScore += 30;
        console.log(`      First chord is ${this.noteNames[root]}m (minor tonic - strong evidence)`);
      }
    }
