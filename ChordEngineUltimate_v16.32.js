/**
 * ChordEngineUltimate v16.32 - Based on v14.36 + Label Fix
 * 
 * This is v14.36 (which works well) with ONE fix:
 * - Added label validation to prevent undefined labels
 * 
 * NO other changes from v14.36!
 */
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // ✅ v16.32 FIX: Filter out any events without valid labels
    ...
    const stats = {
      ...
      borrowed: timeline.filter(e => e.modalContext && e.modalContext.includes('borrowed')).length,
      chromatic: timeline.filter(e => !e.inScale && (!e.modalContext || !e.modalContext.includes('borrowed'))).length,
      modulations,
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    for (let i = 1; i < M; i++) {
      if (states[i] !== cur) {
        // ✅ v16.32 FIX: Check that candidate exists and has label
    ...
    if (lastCand && lastCand.label) {
      timeline.push({ t: start * secPerHop, label: lastCand.label, fi: start });
    }
  ...
  decorateQualitiesUltimate(timeline, feats, key, mode, extensionMul, extensionSensitivity = 1.0) {
    if (mode === 'basic') return timeline.map(e => e ? { ...e } : e).filter(e => e && e.label);
  ...
  analyzeModalContext(timeline, key) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      out.push({ ...ev, modalContext, inScale: this.inKey(r, key.root, key.minor) });
    }
  ...
  quickModulationCheck(timeline, primaryKey) {
    ...
    for (const section of sections) {
      ...
      const validChords = section.filter(c => c && c.label).length;
      const diatonicRatio = validChords > 0 ? diatonicCount / validChords : 0;
      ...
      for (let newRoot = 0; newRoot < 12; newRoot++) {
        for (const newMinor of [false, true]) {
          ...
          const ratio = validChords > 0 ? cnt / validChords : 0;
