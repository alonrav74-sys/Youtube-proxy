...
class ChordEngineEnhanced {
  ...
  detectKeyEnhanced(feats) {
    ...
    if (bassTonic.confidence > 0.25) {
      ...
      if (openingW > 0) {
        for (let p = 0; p < 12; p++) opening[p] /= openingW;
      }
      if (closingW > 0) {
        for (let p = 0; p < 12; p++) closing[p] /= closingW;
      }
      ...
      if (thirdRatio >= 1.03) {
        minorScore += 5.0 * Math.min(3.0, thirdRatio - 1.0);
      } else if (thirdRatio <= 0.97) {
        majorScore += 5.0 * Math.min(3.0, 1.0 / thirdRatio - 1.0);
      }

      const sixthRatio = (m6 + 0.0001) / (M6 + 0.0001);
      if (sixthRatio >= 1.08) {
        minorScore += 3.0 * Math.min(2.5, sixthRatio - 1.0);
      } else if (sixthRatio <= 0.93) {
        majorScore += 3.0 * Math.min(2.5, 1.0 / sixthRatio - 1.0);
      }

      const seventhRatio = (m7 + 0.0001) / (M7 + 0.0001);
      if (seventhRatio >= 1.08) {
        minorScore += 2.0 * Math.min(2.0, seventhRatio - 1.0);
      } else if (seventhRatio <= 0.93) {
        majorScore += 2.0 * Math.min(2.0, 1.0 / seventhRatio - 1.0);
      }

      const openingThirdRatio = (opening_m3 + 0.0001) / (opening_M3 + 0.0001);
      if (openingThirdRatio > 1.05) {
        minorScore += 4.0;
      } else if (openingThirdRatio < 0.95) {
        majorScore += 4.0;
      }

      const closingThirdRatio = (closing_m3 + 0.0001) / (closing_M3 + 0.0001);
      if (closingThirdRatio > 1.05) {
        minorScore += 4.0;
      } else if (closingThirdRatio < 0.95) {
        majorScore += 4.0;
      }

      if (bassTonic.minorHint !== undefined) {
        if (bassTonic.minorHint) {
          minorScore += bassTonic.bassMinorConfidence * 3.0;
        } else {
          majorScore += bassTonic.bassMinorConfidence * 3.0;
        }
      }
  ...
  finalizeTimeline(timeline, key, bpm, feats) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      if (dur < minDur && filtered.length > 0 && (isWeak || !isDiatonic)) {
        continue;
      }
