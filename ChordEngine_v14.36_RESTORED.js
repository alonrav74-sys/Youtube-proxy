...
class ChordEngineEnhanced {
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    const emitScore = (i, cand) => {
      ...
      // 🔧 Lowered threshold from 0.35 to 0.20 to detect more chords
      if (score < 0.20) return -Infinity;

      // ═══════════════════════════════════════════════════════════════
      // 🎸 TRIAD VERIFICATION (1,3,5) - Check basic chord tones first
      // ═══════════════════════════════════════════════════════════════
      const intervals = cand.type === 'minor' ? [0,3,7] : [0,4,7];
      const root_tone = c[toPc(cand.root)] || 0;
      const third_tone = c[toPc(cand.root + intervals[1])] || 0;
      const fifth_tone = c[toPc(cand.root + intervals[2])] || 0;
      
      // All three should be present for a valid chord - LOWERED thresholds
      if (root_tone < 0.04 || third_tone < 0.03 || fifth_tone < 0.03) {
        score *= 0.8;  // Gentle penalty for weak triad (was 0.7)
      }

      // ═══════════════════════════════════════════════════════════════
      // 🎯 DIATONIC SCORING - EXACT v14.36 weights
      // ═══════════════════════════════════════════════════════════════
      if (!cand.borrowed) score += 0.20;
      else score -= 0.25;

      // ═══════════════════════════════════════════════════════════════
      // 🎸 BASS VALIDATION - Strengthened + semitone resolution
      // ═══════════════════════════════════════════════════════════════
      const detectedBass = bassPc[i];
      if (detectedBass >= 0) {
        if (detectedBass === cand.root) {
          // Perfect bass match
          score += 0.25 * bassMultiplier;  // Was 0.15 → 67% stronger
        } 
        else {
          // Check if we're within a semitone (bass detection uncertainty)
          const semitone_dist = Math.min(
            Math.abs(toPc(detectedBass - cand.root)),
            Math.abs(toPc(cand.root - detectedBass))
          );
          
          if (semitone_dist === 1) {
            // Very close - check which note is actually stronger in audio
            const bassEnergy = c[detectedBass] || 0;
            const candEnergy = c[cand.root] || 0;
            
            if (candEnergy >= bassEnergy * 0.7) {
              score += 0.12 * bassMultiplier;  // Partial credit
            }
          }
        }
      }
