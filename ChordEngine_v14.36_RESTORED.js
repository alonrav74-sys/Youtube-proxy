...
class ChordEngineEnhanced {
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    const emitScore = (i, cand) => {
      ...
      // ═══════════════════════════════════════════════════════════════
      // 🎸 TRIAD CHECK (1,3,5) - ALWAYS verify basic chord tones
      // ═══════════════════════════════════════════════════════════════
      const intervals = cand.type === 'minor' ? [0,3,7] : [0,4,7];
      const root = c[toPc(cand.root)] || 0;
      const third = c[toPc(cand.root + intervals[1])] || 0;
      const fifth = c[toPc(cand.root + intervals[2])] || 0;
      
      // Verify all three tones are present
      const triadStrength = root * 0.4 + third * 0.35 + fifth * 0.25;
      if (triadStrength < 0.10) {
        return -Infinity;  // Not a real chord if triad is too weak
      }

      // ═══════════════════════════════════════════════════════════════
      // 🎯 DIATONIC SCORING - Weakened to allow borrowed chords
      // ═══════════════════════════════════════════════════════════════
      if (!cand.borrowed) score += 0.15;  // Was 0.20 - reduced
      else score -= 0.15;  // Was -0.25 - less penalty

      // ═══════════════════════════════════════════════════════════════
      // 🎸 BASS VALIDATION - Much stronger + semitone accuracy
      // ═══════════════════════════════════════════════════════════════
      const detectedBass = bassPc[i];
      if (detectedBass >= 0) {
        // Check exact match
        if (detectedBass === cand.root) {
          score += 0.30 * bassMultiplier;  // Strong bass boost
        } 
        // Check semitone neighbors (bass detection can be off by 1)
        else if (toPc(detectedBass - cand.root) === 1 || toPc(detectedBass - cand.root) === 11) {
          score += 0.15 * bassMultiplier;  // Partial credit for close match
        }
      }
