...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    if (typeof MajorMinorRefiner !== 'undefined' && opts.useMajorMinorRefiner !== false) {
      try {
        ...
        const refinedTimeline = await refiner.refineChordTimeline(
          ...
          {
            debug: false,
            minConfidenceToOverride: 0.60  // Balanced - default from MajorMinorRefiner
          }
        );
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // Provides enhanced bass detection and inversion recognition
    ...
    if (typeof BassEngine !== 'undefined' && opts.useBassEngine !== false) {
      try {
        ...
        // BassEngine refines bass notes and adds better inversions
        const refinedTimeline = await bassEngine.refineBassInTimeline(
          ...
          key,
          {
            ...
            minInversionConfidence: 0.65  // Only add strong inversions
          }
        );
        ...
        for (let i = 0; i < timeline.length; i++) {
          if (refinedTimeline[i]?.label && refinedTimeline[i].label !== timeline[i].label) {
            if (refinedTimeline[i].label.includes('/')) inversionCount++;
        ...
        if (inversionCount > 0) {
          console.log(`🎸 BassEngine: ${inversionCount} inversions added`);
          if (opts.progressCallback) {
            opts.progressCallback({
              stage: 'refined_bass',
              progress: 0.88,
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    const emitScore = (i, cand) => {
      ...
      // 🎸 ENHANCED: Gentle bass boost + smart semitone tolerance
      const detectedBass = bassPc[i];
      if (detectedBass >= 0) {
        if (detectedBass === cand.root) {
          // Perfect match - gentle boost
          score += 0.20 * bassMultiplier;  // Was 0.15 → +33%
        } else {
          // Semitone tolerance - only if bass note is strong in chroma
          const bassChroma = c[detectedBass] || 0;
          const candChroma = c[cand.root] || 0;
          
          // Check if within semitone AND bass is actually strong
          const dist = Math.min((detectedBass - cand.root + 12) % 12, (cand.root - detectedBass + 12) % 12);
          if (dist === 1 && bassChroma > 0.12 && candChroma >= bassChroma * 0.75) {
            score += 0.10 * bassMultiplier;  // Partial credit for close match
          }
        }
      }
  ...
  finalizeTimeline(timeline, key, bpm, feats) {
    ...
    const minDur = Math.max(0.5, 0.45 * spb);  // ✅ v14.36 original
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      const isWeak = energy < energyMedian * 0.80;  // ✅ v14.36 original

      const r = this.parseRoot(a.label);
      const isDiatonic = r >= 0 && this.inKey(r, key.root, key.minor);

      // ✅ v14.36 original: Gentle filtering
  ...
  enforceEarlyDiatonic(timeline, key, feats, bpm) {
    ...
    // ✅ v14.36 original: Reasonable intro cleaning
