    ...
    if (typeof MajorMinorRefiner !== 'undefined' && opts.useMajorMinorRefiner !== false) {
      try {
        ...
        const refinedTimeline = await refiner.refineChordTimeline(
          ...
          {
            debug: false,
            minConfidenceToOverride: 0.55  // Lower threshold - trust strong 3rd detection!
          }
        );
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    const emitScore = (i, cand) => {
      ...
      if (cand.perfectDiatonic) {
        // Perfect match (root + mode) → boost
        score += 0.25;
        
        // 🎯 EXTRA BOOST: I chord with correct mode
        if (isTonicChord) {
        ...
        // 🎵 I chord with wrong mode → slight penalty
        if (isTonicChord) {
          score -= 0.05;
        }
      } else {
      ...
      // ═══════════════════════════════════════════════════════════════
      // 🎸 BASS VALIDATION - STRENGTHENED SIGNIFICANTLY!
      // ═══════════════════════════════════════════════════════════════
      const bassMatches = (bassPc[i] >= 0 && cand.root === bassPc[i]);
      
      if (bassMatches) {
        // 🎸 BASS IS CRITICAL - Much stronger weight!
        score += 0.35 * bassMultiplier;  // Was 0.15 - MORE THAN DOUBLED!
        
        // 🎯 SYNERGY BONUS: perfectDiatonic + bass = HUGE confidence!
        if (cand.perfectDiatonic) {
          score += 0.25;  // Was 0.10 - Massive synergy reward!
        }
      }
