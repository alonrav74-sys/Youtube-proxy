...
class ChordEngineEnhanced {
    ...
    if (typeof MajorMinorRefiner !== 'undefined' && opts.useMajorMinorRefiner !== false) {
      try {
        ...
        const refinedTimeline = await refiner.refineChordTimeline(
          ...
          {
            debug: false,
            minConfidenceToOverride: 0.75  // CONSERVATIVE: Only if VERY confident!
          }
        );
        ...
        for (let i = 0; i < timeline.length; i++) {
          if (refinedTimeline[i]?.qualityRefined && 
              refinedTimeline[i].label !== timeline[i].label) {
            corrections++;
            // 🎯 APPLY THE CORRECTION!
            timeline[i].label = refinedTimeline[i].label;
            timeline[i].refinedBy = 'MajorMinorRefiner';
            timeline[i].refinerConfidence = refinedTimeline[i].qualityConfidence;
          }
        }
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    const emitScore = (i, cand) => {
      ...
      // 🔧 FIX: Lowered threshold from 0.35 to 0.20 to detect more chords
      if (score < 0.20) return -Infinity;

      // ═══════════════════════════════════════════════════════════════
      // 🎯 DIATONIC SCORING - Enhanced with perfectDiatonic
      // ═══════════════════════════════════════════════════════════════
      const isTonicChord = (cand.root === key.root);
      
      if (cand.perfectDiatonic) {
        // Perfect match (root + mode) → stronger boost
        score += 0.25;
        
        // 🎯 EXTRA BOOST: I chord with correct mode (Em in E minor, E in E major)
        // Give strong advantage to the "home" chord
        if (isTonicChord) {
          score += 0.20;  // Total: +0.45 for perfect I!
        }
      } else if (!cand.borrowed) {
        // Diatonic root but wrong mode
        score += 0.10;
        
        // 🎵 GENTLE NUDGE: I chord with wrong mode
        // Don't penalize heavily - audio signal is king!
        // But give slight disadvantage (Em vs E in E major, etc)
        if (isTonicChord) {
          score -= 0.05;  // Total: +0.05 (small nudge, not penalty)
        }
      } else {
      ...
      // ═══════════════════════════════════════════════════════════════
      // 🎸 BASS VALIDATION - Enhanced
      // ═══════════════════════════════════════════════════════════════
      const bassMatches = (bassPc[i] >= 0 && cand.root === bassPc[i]);
      
      if (bassMatches) {
        score += 0.15 * bassMultiplier;
        
        // 🎯 SYNERGY BONUS: perfectDiatonic + bass = extra confidence!
        if (cand.perfectDiatonic) {
          score += 0.10;  // Both signals agree → strong evidence
        }
      }
