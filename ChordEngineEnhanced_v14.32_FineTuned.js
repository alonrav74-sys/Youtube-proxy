/**
 * ChordEngineEnhanced_v14.33_MinorFix - Em/E Detection Fixed
 * תיקון זיהוי Minor/Major משופר עם בדיקה רב-ממדית
 */
...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    if (
      ...
      key = {
        root: tonic.root,
        minor: key.minor,
        confidence: Math.max(key.confidence, tonic.confidence / 100)
      };
  ...
  detectKeyEnhanced(feats) {
    ...
    // ✅ תיקון: threshold נמוך יותר + בדיקה משופרת
    if (bassTonic.confidence > 0.25) {
      ...
      // ✅ בדיקה רב-ממדית משופרת
      const m3 = agg[this.toPc(root + 3)] || 0;   // minor third
      const M3 = agg[this.toPc(root + 4)] || 0;   // major third
      const m6 = agg[this.toPc(root + 8)] || 0;   // minor sixth
      const M6 = agg[this.toPc(root + 9)] || 0;   // major sixth
      const m7 = agg[this.toPc(root + 10)] || 0;  // minor seventh
      const M7 = agg[this.toPc(root + 11)] || 0;  // major seventh

      // ✅ חישוב ציון מורכב
      let minorScore = 0;
      let majorScore = 0;

      // השלישייה - הכי חשוב (משקל כפול)
      if (m3 > M3 * 1.20) minorScore += 2.0;
      if (M3 > m3 * 1.20) majorScore += 2.0;

      // השישית
      if (m6 > M6 * 1.15) minorScore += 1.0;
      if (M6 > m6 * 1.15) majorScore += 1.0;

      // השביעית
      if (m7 > M7 * 1.15) minorScore += 1.0;
      if (M7 > m7 * 1.15) majorScore += 1.0;

      // ✅ החלטה: מינור רק אם יש יתרון ברור
      const isMinor = minorScore > majorScore + 0.5;

      // ✅ ביטחון מבוסס על הפרדה
      const spread = Math.abs(m3 - M3) + Math.abs(m6 - M6) + Math.abs(m7 - M7);
      let confidence = 0.3 + bassTonic.confidence * 0.3 + spread * 1.0;
    ...
    // 2. Fallback: Krumhansl-Schmuckler
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    const transitionCost = (a, b) => {
      ...
      const I  = key.root;
      const V  = this.toPc(key.root + localScale[4]);
      const IV = this.toPc(key.root + localScale[3]);
      const II = this.toPc(key.root + localScale[1]);
  ...
  validateKeyFromChords(timeline, currentKey, feats) {
    ...
    // ✅ כלל זהב: לעולם לא משנים minor/major
    if (best.minor !== currentKey.minor) {
      return currentKey;
    }

    if (!current || best.score > currentScore + 40) {
      const newConf = Math.min(0.99,
        Math.max(currentKey.confidence || 0.5, 0.6 + best.score / 200)
      );
      return {
        root: best.root,
        minor: best.minor,
  ...
  detectTonicMusically(timeline, key, duration) {
    ...
    return {
      root: bestRoot,
      label: this.NOTES_SHARP[this.toPc(bestRoot)] + (key.minor ? 'm' : ''),
      confidence
    };
  }
  ...
  analyzeHarmonicProgressions(feats, keyRoot, keyMinor) {
    ...
    const scale = keyMinor ? this.MINOR_SCALE : this.MAJOR_SCALE;
    const I  = keyRoot;
    const II = this.toPc(keyRoot + scale[1]);
    const III= this.toPc(keyRoot + scale[2]);
    const IV = this.toPc(keyRoot + scale[3]);
    const V  = this.toPc(keyRoot + scale[4]);
    const VI = this.toPc(keyRoot + scale[5]);
    const VII= this.toPc(keyRoot + scale[6]);
  ...
  recognizeProgressionPattern(recentChords, key) {
    ...
    const degrees = recentChords.map(chord => {
      ...
      for (let i = 0; i < scale.length; i++) {
        if (this.toPc(scale[i]) === rel) return i + 1;
      }
      return null;
    }).filter(d => d !== null);
  ...
  predictNextChord(recentChords, key) {
    ...
    if (progression && progression.next !== null) {
      ...
      const targetRoot = this.toPc(key.root + scale[deg - 1]);
      predictions.push({
        root: targetRoot,
    ...
    const fifthUp = this.toPc(lastRoot + 7);
    const fifthDown = this.toPc(lastRoot - 7);
  ...
  enrichTimelineWithTheory(timeline, feats, key) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      if (recent.length >= 2) {
        ...
        if (prog) {
          analyzed.recognizedProgression = prog.name;
        }
      }
