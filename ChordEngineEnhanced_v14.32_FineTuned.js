/**
 * ChordEngineEnhanced - Clean Production Version
 * בלי console, בלי דיבוג.
 * שומר על זיהוי סולם קיים:
 *  1. קודם באס
 *  2. אח"כ דרגת ה־3 (M3/m3)
 *  3. בלי העדפה מלאכותית למז'ור
 * משופר כך שלאחר קביעת סולם:
 *  - אקורדים דיאטוניים בסולם מקבלים עדיפות חזקה
 *  - חריגות הרמוניות "חוקיות" מקבלות עדיפות בינונית
 *  - שאר האקורדים מקבלים עדיפות נמוכה / ענישה קלה
 */
...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // Key (bass → third, בלי להטות למז'ור)
    const tKey = now();
    ...
    if (validatedKey.root !== key.root || validatedKey.minor !== key.minor) {
      ...
      timeline = this.finalizeTimeline(timeline, key, audioData.bpm, feats);
      timings.rerunKeyValidation = now() - tReKey;
    }
    ...
    timeline = this.validateAndRefine(timeline, key, feats);
    ...
    // Tonic check (לא שוברים סולם, רק אם הביטחון גבוה)
    ...
    if (
      ...
      const newKey = { root: tonic.root, minor: key.minor, confidence: key.confidence || tonic.confidence / 100 };
      const tReTon = now();
      const tl2 = this.chordTrackingHMMHybrid(feats, newKey, opts.bassMultiplier, true);
      const tl2f = this.finalizeTimeline(tl2, newKey, audioData.bpm, feats);
      const validated = this.validateKeyFromChords(tl2f, newKey, feats);

      // מגינים על סולם: מחליפים רק אם חדש עדיף משמעותית
      if (validated.confidence && validated.confidence > (key.confidence || 0.7) + 0.05) {
        key = validated;
        timeline = this.enforceEarlyDiatonic(tl2f, key, feats, audioData.bpm);
        timeline = this.decorateQualitiesUltimate(timeline, feats, key, opts.harmonyMode, opts.extensionMultiplier, opts.extensionSensitivity);
        timeline = this.adjustMinorMajors(timeline, feats, key);
        timeline = this.addInversionsUltimate(timeline, feats, key, opts.bassMultiplier);
        timeline = this.validateAndRefine(timeline, key, feats);
  ...
  detectKeyEnhanced(feats) {
    ...
    if (!chroma || !chroma.length) {
      return { root: 0, minor: false, confidence: 0.0 };
    }

    const bassTonic = this.detectTonicFromBass(feats);

    // 1. אם הבאס מספיק חזק → בודקים שליש
    if (bassTonic.confidence > 0.30) {
      ...
      if (totalW > 0) {
        for (let p = 0; p < 12; p++) agg[p] /= totalW;
      }
      ...
      const confidence = Math.min(
        1.0,
        0.4 + bassTonic.confidence * 0.4 + spread * 0.8
      );
    ...
    // 2. אחרת: Krumhansl–Schmuckler בלי העדפה למז'ור
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    // דיאטוני מלא (major/minor triads על כל דרגה)
    for (const r of diatonicPcs) {
      const nn = this.getNoteName(r, key);
      candidates.push({ root: r, label: nn, type: 'major', borrowed: false });
      candidates.push({ root: r, label: nn + 'm', type: 'minor', borrowed: false });
    }

    // Borrowed / modal / harmonic exceptions כמועמדים (לא שוברים key detection)
    if (!key.minor) {
    ...
    // תבניות לציון התאמה ספקטרלית
    const chordTemplates = new Map();
    for (const cand of candidates) {
      const intervals =
        cand.type === 'minor' ? [0,3,7] :
        cand.type === 'dim'   ? [0,3,6] :
        [0,4,7];
    ...
    const emitScore = (i, cand) => {
      ...
      // 1️⃣ עדיפות חזקה לדיאטוני בסולם
      const isDia = this.isDiatonicTriadCandidate(cand, key);
      const isExc = this.isHarmonicExceptionCandidate(cand, key);

      if (isDia) {
        score += 0.22;
      } else if (isExc) {
        score += 0.08;
      } else if (cand.borrowed && useFullMode) {
        score -= 0.15;
      }
    ...
    const transitionCost = (a, b) => {
      ...
      const aDia = this.isDiatonicTriadCandidate(a, key);
      const bDia = this.isDiatonicTriadCandidate(b, key);
      const aExc = this.isHarmonicExceptionCandidate(a, key);
      const bExc = this.isHarmonicExceptionCandidate(b, key);

      // Borrowed/Chromatic penalization + bonus לזרימה דיאטונית
      if (aDia && bDia) {
        cost -= 0.12; // מעבר חלק בתוך הסולם
      } else if ((a.borrowed && b.borrowed)) {
        cost += 0.30;
      } else if (a.borrowed || b.borrowed) {
        cost += 0.18;
      } else if ((aExc && bDia) || (aDia && bExc)) {
        cost += 0.02; // חריג → דיאטוני / דיאטוני → חריג
      }
  ...
  enforceEarlyDiatonic(timeline, key, feats, bpm) {
    ...
    const spb = 60 / Math.max(60, Math.min(200, bpm || 120));
    const earlyWindow = Math.max(10.0, 4 * spb); // בהתאם לתיקון שביקשת
    const scale = key.minor ? this.MINOR_SCALE : this.MAJOR_SCALE;
    const diatonicPcs = scale.map(s => this.toPc(key.root + s));
  ...
  inKey(pc, keyRoot, minor) {
    ...
    if (minor) {
      ...
      if (rel === 7 || rel === 11) return true; // הרמוני/מלודי
    } else {
      const rel = this.toPc(pc - keyRoot);
      if (rel === 2 || rel === 10 || rel === 8) return true; // II, bVII, bVI כרפרנס
    }

    return false;
  }

  // דיאטוני טהור (ללא הרחבות) עבור HMM
  isDiatonicTriadCandidate(cand, key) {
    const { root, type } = cand;
    const scale = key.minor ? this.MINOR_SCALE : this.MAJOR_SCALE;
    const qualities = key.minor
      ? ['m','dim','','m','m','','']
      : ['','m','m','','','m','dim'];
  ...
  // חריגות הרמוניות "חוקיות" (III+, IVm, bVII, V במינור וכו')
  isHarmonicExceptionCandidate(cand, key) {
    const { root, type } = cand;
    const rel = this.toPc(root - key.root);
    ...
    if (!key.minor) {
      ...
      if (rel === 8  && type === 'major') return true; // bVI
      if (rel === 3  && type === 'major') return true; // bIII
      if (rel === 5  && type === 'minor') return true; // iv
      if (rel === 2  && type === 'major') return true; // II (lydian feel)
      if (rel === 1  && type === 'major') return true; // bII (neapolitan)
    } else {
      // סולם מינור
      if (rel === 7 && type === 'major') return true; // V (harmonic minor)
      if (rel === 5 && type === 'major') return true; // IV major
      if (rel === 9 && type === 'major') return true; // VI major
      if (rel === 11 && type === 'major') return true; // VII major
      if (rel === 10 && type === 'major') return true; // bVII
      if (rel === 8 && type === 'major') return true; // bVI
      if (rel === 2 && type === 'major') return true; // II (dorian)
    }
