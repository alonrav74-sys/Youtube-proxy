/**
 * ChordEngineEnhanced - Clean Production Version (Fixed Key Logic)
 * בלי console, בלי דיבוג, בלי הטיה למז'ור.
 */
...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    timings.hmmTracking = now() - tHmmStart;
    ...
    if (validatedKey.root !== key.root || validatedKey.minor !== key.minor) {
      ...
      timeline = this.finalizeTimeline(timeline, key, audioData.bpm, feats);
      timings.rerunKeyValidation = now() - tRerun;
    }
    ...
    timings.postProcessing = now() - tPostStart;

    // Tonic (מבוסס timeline, לא מחזיר אותנו להטיה)
    const tonic = this.detectTonicMusically(timeline, key, audioData.duration);

    // ריצת תיקון טוניק (רק אם מאוד משכנע)
    if (
      tonic.root !== key.root &&
      ...
      key = {
        root: tonic.root,
        minor: key.minor,
        confidence: key.confidence || tonic.confidence / 100
      };
      ...
      timeline = this.classifyOrnaments(timeline, audioData.bpm, feats);
      timeline = this.analyzeModalContext(timeline, key);
      timings.rerunTonic = now() - tRerun2;
    }
   ...
   * זיהוי סולם:
   * 1. קודם באס (root)
   * 2. אחר כך שליש (m3/M3) עם משקל גבוה
   * 3. בלי ברירת מחדל למז'ור כשהפער קטן
   * 4. fallback: K-S על כל 24 הסולמות
   */
  detectKeyEnhanced(feats) {
    ...
    const ksScore = (agg, root, minor) => {
      const prof = minor ? KS_MINOR : KS_MAJOR;
      let s = 0;
      for (let i = 0; i < 12; i++) {
        s += agg[this.toPc(root + i)] * prof[i];
      }
      return s;
    };
    ...
    if (haveBass) {
      ...
      // אוספים רק פריימים חזקים סביב השליש
      ...
      if (m3 > M3 * margin) {
        minor = true;
      } else if (M3 > m3 * margin) {
        minor = false;
      } else {
        // אם לא ברור מהשליש – בודקים K-S רק לשני המצבים של אותו שורש
        const sMaj = ksScore(agg, root, false);
        const sMin = ksScore(agg, root, true);
        minor = sMin > sMaj;
      }

      const thirdDiff = Math.abs(m3 - M3);
      const thirdConfidence = Math.max(0, Math.min(1, thirdDiff * 4)); // ככל שהפער גדול יותר, בטוח יותר

      const bassWeight = 0.6;
      const thirdWeight = 0.4;
      const confidence = Math.max(
        0.4,
        Math.min(
          1.0,
          bassWeight * bassTonic.confidence + thirdWeight * thirdConfidence
        )
      );
    ...
    // 2) Fallback: K-S מלא על כל 24 סולמות (בלי העדפת מז'ור)
    if (!chroma || !chroma.length) {
      return { root: 0, minor: false, confidence: 0.3 };
    }

    const aggAll = new Array(12).fill(0);
    ...
    let sumAll = 0;
    for (let p = 0; p < 12; p++) sumAll += aggAll[p];
    if (sumAll > 0) {
      for (let p = 0; p < 12; p++) aggAll[p] /= sumAll;
    }

    let best = { score: -Infinity, root: 0, minor: false };
    let second = { score: -Infinity, root: 0, minor: false };

    for (let r = 0; r < 12; r++) {
      const sMaj = ksScore(aggAll, r, false);
      if (sMaj > best.score) {
        second = best;
        best = { score: sMaj, root: r, minor: false };
      } else if (sMaj > second.score) {
        second = { score: sMaj, root: r, minor: false };
      }

      const sMin = ksScore(aggAll, r, true);
      if (sMin > best.score) {
        second = best;
        best = { score: sMin, root: r, minor: true };
      } else if (sMin > second.score) {
        second = { score: sMin, root: r, minor: true };
      }
    }

    const diff = best.score - second.score;
    const confidence = Math.max(0.3, Math.min(1.0, best.score / 10 * 0.7 + diff * 0.03));
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    // Diatonic triads (major + minor on כל דרגה – נותן גמישות)
    ...
    const lowE =
      (percentiles && percentiles.p30) ||
      this.percentile(frameE, 30);
    ...
    const emitScore = (i, cand) => {
      ...
      // ✂️ תיקון #1: עדיפות לדיאטוניים, ענישה ל-borrowed
      if (!cand.borrowed) {
        score += 0.20;
      } else {
        score -= 0.25;
    ...
    const transitionCost = (a, b) => {
      ...
      // ✂️ תיקון #2: התאמות משקלים + עלות borrowed
  ...
  enforceEarlyDiatonic(timeline, key, feats, bpm) {
    ...
    const spb = 60 / Math.max(60, Math.min(200, bpm || 120));
    // ✂️ תיקון #3: חלון ארוך יותר וקשיח יותר
