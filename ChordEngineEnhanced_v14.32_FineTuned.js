/**
 * ChordEngineEnhanced_v14.35_MinorDetectionFixed
 * תיקון מושלם לזיהוי Em vs E
 * חישוב יחסי במקום threshold קשיח
 */
...
class ChordEngineEnhanced {
  ...
  detectKeyEnhanced(feats) {
    ...
    if (bassTonic.confidence > 0.25) {
      ...
      const thirdRatio = (m3 + 0.001) / (M3 + 0.001);
      if (thirdRatio > 1.15) {
        minorScore += 3.0 * Math.min(2.0, thirdRatio - 1.0);
      } else if (thirdRatio < 0.87) {
        majorScore += 3.0 * Math.min(2.0, 1.0 / thirdRatio - 1.0);
      }

      const sixthRatio = (m6 + 0.001) / (M6 + 0.001);
      if (sixthRatio > 1.10) {
        minorScore += 1.5 * Math.min(2.0, sixthRatio - 1.0);
      } else if (sixthRatio < 0.91) {
        majorScore += 1.5 * Math.min(2.0, 1.0 / sixthRatio - 1.0);
      }

      const seventhRatio = (m7 + 0.001) / (M7 + 0.001);
      if (seventhRatio > 1.10) {
        minorScore += 1.5 * Math.min(2.0, seventhRatio - 1.0);
      } else if (seventhRatio < 0.91) {
        majorScore += 1.5 * Math.min(2.0, 1.0 / seventhRatio - 1.0);
      }
