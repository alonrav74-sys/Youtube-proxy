...
class ChordEngineEnhanced {
  ...
  detectKeyEnhanced(feats) {
    ...
    // 1) אם הבאס מספיק חזק – קובעים טוניקה לפי באס ואז בודקים שליש קטן/גדול
    ...
    // 2) אם הבאס לא חד-משמעי – נופלים חזרה ל-KS, בלי הטייה למז'ור.
    ...
    // משקל עדין לאקורד ראשון, בלי לכפות מז'ור.
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    // כל האקורדים הדיאטוניים (גם מז'ור וגם מינור) - ניטרלי.
    ...
    // Borrowed / modal options - מסומנים borrowed:true, מטופלים בעונש/בונוס ב-HMM.
    if (!key.minor) {
    ...
    const emitScore = (i, cand) => {
      ...
      // ✂️ תיקון #1: עדכון יחס borrowed / non-borrowed (ללא הטיה למז'ור, רק דיאטוני מול לוואי)
      if (!cand.borrowed) {
        score += 0.20;
      } else {
        score -= 0.25;
    ...
    const transitionCost = (a, b) => {
      ...
      // ✂️ תיקון #2: עדכון משקל מרחקים ועונשי borrowed
  ...
  validateKeyFromChords(timeline, currentKey) {
  ...
  enforceEarlyDiatonic(timeline, key, feats, bpm) {
    ...
    // ✂️ תיקון #3: חלון גדול ומחמיר יותר בתחילת השיר
