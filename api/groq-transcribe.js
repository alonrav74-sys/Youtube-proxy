...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('📦 RapidAPI Response structure:', JSON.stringify(rapidData, null, 2).substring(0, 1000));
    console.log('📦 Response keys:', Object.keys(rapidData).join(', '));
    
    // Find audio URL - try all possible locations
    let audioUrl;
    
    // Try different structures
    if (rapidData.audios && Array.isArray(rapidData.audios) && rapidData.audios.length > 0) {
      audioUrl = rapidData.audios[0].url || rapidData.audios[0].link;
      console.log('✓ Found in audios array');
    } else if (rapidData.audio_url) {
      audioUrl = rapidData.audio_url;
      console.log('✓ Found in audio_url');
    ...
    } else if (rapidData.formats && Array.isArray(rapidData.formats)) {
      const audioFormat = rapidData.formats.find(f => 
        f.audio_only || 
        f.type === 'audio' || 
        f.mimeType?.includes('audio') ||
        f.container === 'mp4' && !f.hasVideo
      );
      audioUrl = audioFormat?.url || audioFormat?.link || rapidData.formats[0]?.url;
      console.log('✓ Found in formats');
    } else if (rapidData.download_url) {
      audioUrl = rapidData.download_url;
      console.log('✓ Found in download_url');
    }
    
    if (!audioUrl) {
      console.error('❌ Could not find audio URL in response!');
