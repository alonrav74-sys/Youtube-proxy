export default async function handler(req, res) {
  ...
  const piped = [
    ...
    'https://yt.artemislena.eu' // ✅ fallback חדש
  ];

  for (const base of piped) {
    try {
      const r = await fetchWithTimeout(`${base}/streams/${id}`, 10000);
      if (!r.ok) continue;
  ...
  for (const base of invidious) {
    try {
      ...
      const onlyAudio = a1.filter(f => (f?.type || '').includes('audio'));
      const audioStreams = onlyAudio.map(f => ({
        url: f.url,
        mimeType: f.type,
        bitrate: Number(f.bitrate || f.averageBitrate),
        quality: f.audioQuality || 'audio',
      }));
      if (audioStreams.length) return res.status(200).json({ audioStreams });
    } catch (_) {}
  }
