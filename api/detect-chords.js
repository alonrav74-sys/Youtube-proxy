...
// Chord detection API for Vercel - receives audio data, returns chords
...
module.exports = async (req, res) => {
  ...
  try {
    const { 
      features,  // chroma, bassPc, frameE from client
      bpm,
      duration,
      config = {}
    } = req.body;
    ...
    function buildChords(bassPc, chroma, frameE, key, bpm, hop, sr) {
      ...
      while (i < bassPc.length) {
        if (bassPc[i] < 0 || frameE[i] < percentile(frameE, 15)) {
          i++;
          continue;
        }
    ...
    // 4. Add extensions (7ths, 9ths)
    function addExtensions(timeline, extMode = 'jazz', decSens = 1.0) {
    ...
    // 5. Quantize
    function quantize(timeline, bpm, quantValue = 4) {
      const spb = 60 / Math.max(60, bpm || 120);
      ...
      return timeline.map((ev, i) => {
        ...
        const duration = nextEv ? (nextEv.time - ev.time) : spb;
        
        return { ...ev, time: quantized, duration };
      });
    }

    // 6. Classify ornaments
    function classifyOrnaments(timeline, bpm) {
      const spb = 60 / Math.max(60, bpm || 120);
    ...
    let timeline = buildChords(bassPc, chroma, frameE, key, bpm, hop, sr);
    ...
    timeline = addExtensions(timeline, config.extMode, config.decSens);
    timeline = classifyOrnaments(timeline, bpm);
    timeline = quantize(timeline, bpm, config.quantValue);
