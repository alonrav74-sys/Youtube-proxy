...
class ChordEnginePro extends ChordEngine {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // 🆕 Don't assume C major - let bass detection be truly agnostic
    ...
    // 🆕 Quick pre-scan: detect key from chroma BEFORE using chords
    console.log('🎹 Pre-scan: Detecting key from chroma...');
    const chromaKeyEstimate = this.detectKeyByTemplate(feats.chroma);
    console.log(`   Chroma suggests: ${chromaKeyEstimate.tonic} ${chromaKeyEstimate.mode} (conf: ${chromaKeyEstimate.confidence.toFixed(2)})`);
    
    // 🆕 4.5️⃣ TONIC DETECTION - Most reliable method!
    console.log('🎯 Running tonic detection...');
    const tonicResult = this.detectTonic(timeline, feats.chroma);
    console.log(`   Tonic: ${this.noteNames[tonicResult.root]} ${tonicResult.minor ? 'minor' : 'major'} (conf: ${tonicResult.confidence.toFixed(2)})`);
    ...
    // 🆕 DECISION TREE: Choose best key estimate
    let finalKey;
    
    if (tonicResult.confidence > 0.7) {
      // High confidence from tonic detection - USE IT!
      finalKey = {
        root: tonicResult.root,
        minor: tonicResult.minor,
        confidence: tonicResult.confidence
      };
      console.log(`   ✅ Using TONIC detection (high confidence)`);
    } else if (keyEstimate.confidence > 0.7) {
      // High confidence from multi-algorithm
      finalKey = {
        root: this.parseRoot(keyEstimate.tonic),
        minor: keyEstimate.mode === 'minor',
        confidence: keyEstimate.confidence
      };
      console.log(`   ✅ Using MULTI-ALGORITHM detection`);
    } else if (chromaKeyEstimate.confidence > 0.7) {
      // Fall back to chroma
      finalKey = {
        root: this.parseRoot(chromaKeyEstimate.tonic),
        minor: chromaKeyEstimate.mode === 'minor',
        confidence: chromaKeyEstimate.confidence
      };
      console.log(`   ✅ Using CHROMA detection (fallback)`);
    } else {
      // Use tonic even with lower confidence
      finalKey = {
        root: tonicResult.root,
        minor: tonicResult.minor,
        confidence: tonicResult.confidence
      };
      console.log(`   ⚠️ Low confidence everywhere - using TONIC as best guess`);
    }
    
    const key = finalKey;
    console.log(`🎹 FINAL Key: ${this.noteNames[key.root]} ${key.minor ? 'minor' : 'major'} (conf: ${key.confidence.toFixed(2)})`);
  ...
  estimateKey(chromaArray, timeline = null) {
    ...
    if (timeline && timeline.length >= 3) {
      ...
      // 🆕 DEBUG: Show initial chords
      console.log(`   🔍 DEBUG: First 5 chords: ${timeline.slice(0, 5).map(c => c.label).join(', ')}`);
    }
    ...
    console.log(`   🔍 DEBUG: All candidates:`, candidates.map(c => `${c.tonic}${c.mode[0]} (${c.method}: ${c.confidence.toFixed(2)})`).join(', '));
  ...
  /**
   * 🎯 TONIC DETECTION - Find the true tonic of the song
   * Uses multiple heuristics to identify the home chord
   */
  detectTonic(timeline, chromaArray) {
    if (!timeline || timeline.length < 3) {
      return { root: 0, minor: false, confidence: 0 };
    }
    
    console.log('🎯 Detecting tonic (home chord)...');
    
    const candidates = new Map(); // root -> { major: score, minor: score }
    
    // ═══════════════════════════════════════════════════════════
    // Heuristic 1: First Chord (40% weight)
    // ═══════════════════════════════════════════════════════════
    const firstChord = timeline[0];
    const firstRoot = this.parseRoot(firstChord.label);
    const firstMinor = firstChord.label && firstChord.label.match(/m(?!aj)/);
    
    if (firstRoot >= 0) {
      if (!candidates.has(firstRoot)) {
        candidates.set(firstRoot, { major: 0, minor: 0 });
      }
      const entry = candidates.get(firstRoot);
      if (firstMinor) {
        entry.minor += 40;
      } else {
        entry.major += 40;
      }
      console.log(`   1️⃣ First chord: ${firstChord.label} → ${this.noteNames[firstRoot]} (+40)`);
    }
    
    // ═══════════════════════════════════════════════════════════
    // Heuristic 2: Last Chord (35% weight)
    // ═══════════════════════════════════════════════════════════
    const lastChord = timeline[timeline.length - 1];
    const lastRoot = this.parseRoot(lastChord.label);
    const lastMinor = lastChord.label && lastChord.label.match(/m(?!aj)/);
    
    if (lastRoot >= 0) {
      if (!candidates.has(lastRoot)) {
        candidates.set(lastRoot, { major: 0, minor: 0 });
      }
      const entry = candidates.get(lastRoot);
      if (lastMinor) {
        entry.minor += 35;
      } else {
        entry.major += 35;
      }
      console.log(`   2️⃣ Last chord: ${lastChord.label} → ${this.noteNames[lastRoot]} (+35)`);
    }
    
    // ═══════════════════════════════════════════════════════════
    // Heuristic 3: Most Common Chord (15% weight)
    // ═══════════════════════════════════════════════════════════
    ...
    if (mostCommon) {
      const [rootStr, mode] = mostCommon.split('_');
      const root = parseInt(rootStr);
      const isMinor = mode === 'm';
      
      if (!candidates.has(root)) {
        candidates.set(root, { major: 0, minor: 0 });
      }
      const entry = candidates.get(root);
      if (isMinor) {
        entry.minor += 15;
      } else {
        entry.major += 15;
      }
      console.log(`   3️⃣ Most common: ${this.noteNames[root]}${isMinor ? 'm' : ''} (${maxFreq}x) → +15`);
    }
    
    // ═══════════════════════════════════════════════════════════
    // Heuristic 4: Cadence Resolution (10% weight)
    // Songs tend to end on V→I cadences
    // ═══════════════════════════════════════════════════════════
    if (timeline.length >= 2) {
      const secondLast = timeline[timeline.length - 2];
      const secondLastRoot = this.parseRoot(secondLast.label);
      
      // Check if last two chords form V→I
      if (secondLastRoot >= 0 && lastRoot >= 0) {
        const interval = this.toPc(lastRoot - secondLastRoot);
        
        // Perfect cadence: V→I (up 4th or down 5th)
        if (interval === 5 || interval === 7) {
          ...
          console.log(`   4️⃣ V→I cadence detected: ${secondLast.label}→${lastChord.label} (+10)`);
        }
      }
    }
    ...
    for (const [root, scores] of candidates.entries()) {
      if (scores.major > bestScore) {
        bestScore = scores.major;
        bestRoot = root;
        bestMinor = false;
      }
      if (scores.minor > bestScore) {
        bestScore = scores.minor;
        bestRoot = root;
        bestMinor = true;
      }
    }
    ...
    console.log(`   ✅ TONIC: ${this.noteNames[bestRoot]} ${bestMinor ? 'minor' : 'major'} (score: ${bestScore}, conf: ${confidence.toFixed(2)})`);
