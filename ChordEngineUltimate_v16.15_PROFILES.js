  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    if (bassNote >= 0) {
      ...
      for (const c of candidates) {
        ...
        // v16.15 FIX: בוסט לטוניקה באינברסיה כשיש את כל תווי הטוניקה!
        // אם G בבאס אבל יש C, E, G - זה C/G ולא G
        if (
          c.inversionBass === bassNote &&
          c.root === key.root
        ) {
          // בדיקה: האם כל תווי הטריאדה של הטוניקה קיימים?
          ...
          const tonicRootEnergy = avg[tonicRoot];
          const tonicThirdEnergy = avg[tonicThird];
          const tonicFifthEnergy = avg[tonicFifth];
          
          const tonicTriadPresent = tonicRootEnergy > 0.05 && tonicThirdEnergy > 0.05 && tonicFifthEnergy > 0.05;
          const tonicTriadScore = tonicRootEnergy + tonicThirdEnergy * 0.8 + tonicFifthEnergy * 0.6;
          
          if (tonicTriadPresent && tonicTriadScore > 0.15) {
            // יש את כל התווים של הטוניקה - זו בוודאות טוניקה בהיפוך!
            c.score += 40; // בוסט חזק ל-C/G על פני G
            console.log(`🎵 Boosting tonic inversion ${this.NOTES_SHARP[c.root]}/${this.NOTES_SHARP[bassNote]} (triad score: ${tonicTriadScore.toFixed(2)})`);
          } else if (strongMajorTriad || strongMinorTriad) {
            // טריאדה חזקה על הבאס עצמו - קנס קל לטוניקה באינברסיה
            c.score -= 10;
          }
        }
      }
