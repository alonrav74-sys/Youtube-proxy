/**
 * ChordEngine v16.42 - Conservative Filtering
 * 
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log('🎵 ChordEngine v16.42 (Conservative - Fewer but Solid Chords)');
    ...
    // v16.42: TONIC VALIDATION after timeline (section 6)
    ...
    // v16.42: Enhanced slash chord detection (section 6)
    ...
    // v16.42: AUTOMATIC STYLE PROFILE DETECTION
    ...
    // v16.42: Pattern memory V2 (section 6 - fixes anomalies in repeated patterns)
    ...
    return {
      ...
      mode: modeResult,
      styleProfile,  // v16.42: easy_pop / colorful_pop / jazz_like
  ...
  processAudio(audioBuffer) {
    ...
    // v16.42: Clean signal before processing
  ...
  // v16.42: Clean signal - pre-emphasis + noise gate + smoothing
  cleanSignal(x, sr) {
  ...
  extractFeatures(audio) {
    ...
    for (let start = 0; start + win <= x.length; start += hop) {
      ...
      // v16.42: Improved chroma with harmonic weighting
      ...
      for (let bin = 1; bin < mags.length; bin++) {
        ...
        // v16.42: Weight by harmonic clarity
    ...
    // v16.42: Compute spectral flux for chord change detection
    ...
    // v16.42: Bass histogram for debug
    ...
    return {
      ...
      flux, fluxP80, bassHist, // v16.42: Added bassHist
  ...
  // v16.42: Spectral flux - detects changes in chroma
  ...
  // v16.42: Find candidate frames for chord changes
  ...
  detectBassNote(mags, sr, N) {
    // v16.42: Restored from v14.36 - autocorrelation method that works!
    const fmin = 40;
    const fmax = 250;
    const win = N;
  ...
  // v16.42: Improved - also detect clear pitch without bass (guitar/piano intros)
  findMusicStart(features) {
    ...
    for (let i = 0; i < energy.length; i++) {
      ...
      // v16.42: Check for clear musical pitch in chroma (for guitar/piano intros)
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.42: SONG PROFILE ANALYSIS (Dynamic thresholds) - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.42: TONIC VALIDATION FROM CHORDS - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.42: BETTER SLASH CHORD DETECTION - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.42: IMPROVED PATTERN MEMORY (3+4 chord patterns) - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  detectTonicHybrid(features, startFrame) {
    // 🔬 v16.42: Capture all detection steps
    ...
    // 🔬 v16.42: Save all sources + bass histogram
    ...
    // v16.42: Bass histogram - shows all bass notes detected
    ...
    // 🔧 v16.42: הבס - משקל גבוה כשה-confidence גבוה מאוד!
    ...
    // 🔧 v16.42: אקורד ראשון - פחות משקל אם הבס לא מסכים!
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 v16.42: STABLE BASS CHORD BUILDING with Beat-Aware Detection
  // ═══════════════════════════════════════════════════════════
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    // v16.42: MUCH STRICTER thresholds to avoid noise
    ...
    // v16.42: Get chord change candidate frames (beat-aligned + flux + bass changes)
    const changeFrames = this.getChordChangeCandidates(features, bpm, startFrame);
    console.log(`🎵 v16.42: ${changeFrames.length} chord change candidates (beat-aligned)`);
  ...
  getChordsWithBassNote(bassNote, key) {
    ...
    for (const bc of borrowed) {
      // v16.42: III major gets higher priority (very common in pop!)
  ...
  getBorrowedChords(key) {
    ...
    if (!key.minor) {
      // v16.42: III major (E in C) - VERY common in pop! (Hallelujah, etc.)
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    if (candidates.length === 0) {
      // v16.42: Chromatic fallback - only if VERY clear
      for (const isMinor of [false, true]) {
        const score = this.scoreChordCandidate(avg, bassNote, isMinor, bassNote, false);
        if (score > 60) { // v16.42: Higher threshold (was 40)
          ...
          if (isReasonable || score > 80) { // Only if reasonable OR super strong
            candidates.push({
              ...
              inversionBass: null,
              chordType: 'chromatic',
              score: score - 30, // v16.42: Bigger penalty (was -20)
              priority: 20        // v16.42: Lower priority (was 30)
            });
          }
        }
      }
    ...
    if (best.score < 45 && durSec < 0.40) return null; // v16.42: Stricter threshold
    
    // v16.42: Enharmonic substitution for unlikely chromatics
    ...
    // v16.42: AUTO-CORRECT quality based on actual third (use finalRoot!)
    const m3 = avg[this.toPc(finalRoot + 3)];
    const M3 = avg[this.toPc(finalRoot + 4)];
    
    let finalIsMinor = best.isMinor;
    
    // v16.42: For III borrowed chord, FORCE major (E in C major is ALWAYS E major, not Em)
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale) {
    ...
    // v16.42: AUTO-DETECT which third is actually present!
    ...
    else if (bassNote === actualThird) score += 10; // v16.42: Slightly lower for first inversion
    else if (bassNote === fifth) score += 8;        // v16.42: Slightly lower for second inversion
  ...
  // v16.42: Check if chromatic chord makes harmonic sense
  isReasonableChromaticChord(root, key, prevChord) {
    ...
    if (!key.minor) {
      ...
      // v16.42: REMOVED bVI (Ab/G#) - too rare and often confused with E
  ...
  validateWithCircleOfFifths(timeline, key, features) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.42: STRICT chromatic filtering
  ...
  addExtensions(timeline, features, key, opts) {
    ...
    return timeline.map(ev => {
      ...
      // v16.42: Check all chord tones properly
      ...
      // 3. Check for 7th chords - v16.42: More precise detection
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
        ...
        if (!isMinor && M7 > 0.10 && M7 > b7 * 1.5 && M3 > 0.08) {
          // v16.42: Also check that M7 is significant relative to root
          if (M7 > r * 0.15) {
        ...
        // Dominant 7th (C7, G7) - only for major chords with clear b7
          ...
          // v16.42: b7 must be prominent enough
          if (b7 > r * 0.12) {
            label += '7';
          }
        }
      ...
      // 4. Check for sus chords - v16.42: More precise
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
      ...
      // 5. Add 6 if present (C6, Am6) - v16.42: Stricter to avoid melodic 6ths
  ...
  // v16.42: Enhanced filtering from v14.36 with KEY parameter
  finalizeTimeline(timeline, bpm, features, key) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.42: Use KEY to determine if diatonic (not relying on ev.inScale)
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.42: ENFORCE EARLY DIATONIC (from v14.36)
  ...
  // v16.42: Check if pitch class is in key (from v14.36)
  ...
  // v16.42: Detect if song is "easy rock" (mostly diatonic)
  ...
  // v16.42: Force non-diatonic chords to nearest diatonic
  ...
  // v16.42: Remove isolated chromatic chords between identical neighbors
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.42: HMM ENGINE (parallel to Bass engine)
  // ═══════════════════════════════════════════════════════════
    ...
    for (let i = startFrame + 1; i < M; i++) {
      if (path[i] !== cur) {
        ...
        timeline.push({
          ...
          bassNote: bass[start] >= 0 ? bass[start] : cand.root, // v16.42: Add bass
          inScale: true,
          confidence: 75,
          chordType: 'hmm',
          engine: 'hmm'
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.42: MERGE ENGINES - Compare and choose best
  // ═══════════════════════════════════════════════════════════
