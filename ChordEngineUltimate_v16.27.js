/**
 * ChordEngine v16.27 - Smart HPSS
 * 
 * HPSS (Harmonic-Percussive Separation) improvements:
 * - Auto-detect when to use HPSS (only for percussive songs)
 * - Use percussive component for better beat/onset detection
 * - Skip HPSS for acoustic/simple songs
 * 
 * Based on v16.27 + smart HPSS logic
 */
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log('🎵 ChordEngine v16.27 (Smart HPSS)');
    ...
    // v16.27: Detect if song has strong percussion (for smart HPSS)
    ...
    // v16.27: Run HMM engine in parallel and compare
    ...
    // v16.27: Auto-detect "easy rock" songs and clean up
    const stats = this.buildStats(timeline, key);
    const isEasy = this.isEasyRockSong(stats, key);
    
    if (isEasy) {
      console.log(`🎸 v16.27: Easy rock song detected - enforcing diatonic`);
      timeline = this.enforceEasyDiatonic(timeline, key);
      
      // v16.27: Apply pattern memory for easy rock songs
      timeline = this.applyPatternMemory(timeline, key);
    }
    
    // v16.27: Always run outlier smoothing (removes isolated chromatic chords)
  ...
  processAudio(audioBuffer) {
    ...
    // v16.27: Clean signal before processing
  ...
  // v16.27: Clean signal - pre-emphasis + noise gate + smoothing
  cleanSignal(x, sr) {
  ...
  extractFeatures(audio) {
    ...
    for (let start = 0; start + win <= x.length; start += hop) {
      ...
      // v16.27: Improved chroma with harmonic weighting
      ...
      for (let bin = 1; bin < mags.length; bin++) {
        ...
        // v16.27: Weight by harmonic clarity
    ...
    // v16.27: Compute spectral flux with energy for better onset detection
  ...
  // v16.27: Spectral flux - detects changes in chroma
  ...
  computeSpectralFlux(chroma, energy = null) {
    ...
    for (let i = 1; i < chroma.length; i++) {
      ...
      // v16.27: Boost flux if there's also an energy spike (percussive onset)
  ...
  // v16.27: Find candidate frames for chord changes
  ...
  detectBassNote(mags, sr, N) {
    // v16.27: Smart HPSS - only apply if needed
  ...
  // v16.27: Decide if HPSS should be applied
  shouldApplyHPSS() {
  ...
  // v16.27: Detect if song has strong percussion (drums/kicks)
  ...
  // v16.27: Apply HPSS (Harmonic-Percussive Separation) to magnitude spectrum
  ...
  // v16.27: Fast median calculation
  medianValue(arr) {
  ...
  // v16.27: getFirstStrongChord and detectTriadFromChroma removed - not used in new tonic detection
  ...
  // v16.27: detectModeHybrid removed - using detectModeFromThird directly
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 v16.27: STABLE BASS CHORD BUILDING - Bass is King
  // ═══════════════════════════════════════════════════════════
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    const MIN_DURATION_NO_BASS_CHANGE = 1.2; // v16.27: If bass doesn't change, need longer duration
    const CHROMA_CHANGE_THRESHOLD = 0.35;
    const SIGNIFICANT_THIRD_FIFTH_CHANGE = 0.25; // v16.27: Threshold for 3rd/5th change
    ...
    console.log(`🎵 v16.27: ${changeFrames.length} chord change candidates`);
    ...
    for (const i of changeFrames) {
      ...
      if (currentChroma) {
        ...
        // v16.27: Check specifically for 3rd and 5th changes
        thirdFifthChange = this.calculateThirdFifthChange(currentChroma, chroma[i], currentBass);
      }
      
      // v16.27: Decision logic - Bass is King
      ...
      if (bassChanged) {
        ...
        if (durationSec >= MIN_DURATION_NO_BASS_CHANGE && chromaChange >= CHROMA_CHANGE_THRESHOLD) {
          ...
          shouldCommit = true;
          console.log(`  🎹 v16.27: Harmonic change detected (3rd/5th) without bass change at ${(i * secPerFrame).toFixed(2)}s`);
        }
  ...
  // v16.27: Check for significant change in 3rd and 5th intervals
  ...
  getChordsWithBassNote(bassNote, key) {
    ...
    // v16.27: Check if bass note is diatonic scale degree
    const bassIsDiatonic = diatonic.pcs.includes(bassNote);
    
    // v16.27: HIGHEST PRIORITY - Bass is root of diatonic chord
    ...
    for (const dc of diatonic.chords) {
      if (dc.root === bassNote) {
        candidates.push({
          ...
          priority: bassIsDiatonic ? 120 : 100 // v16.27: Extra boost if diatonic bass
        });
      }
    }
    
    // v16.27: LOWER PRIORITY for inversions when bass is diatonic
    ...
    for (const dc of diatonic.chords) {
      ...
      if (third === bassNote) {
        candidates.push({
          ...
          priority: bassIsDiatonic ? 70 : 85 // v16.27: Lower if bass is diatonic (prefer root position)
        });
      }
    }
  ...
  getBorrowedChords(key) {
    ...
    if (!key.minor) {
      ...
      // v16.27: REMOVED bVI (Ab/G#) - causes confusion with E
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    for (const tc of theoryCandidates) {
      // v16.27: Pass inversionBass to scoreChordCandidate
        ...
        if (score > 60) { // v16.27: Higher threshold (was 40)
          ...
          if (isReasonable || score > 80) { // Only if reasonable OR super strong
            candidates.push({
              ...
              inversionBass: null,
              chordType: 'chromatic',
              score: score - 30, // v16.27: Bigger penalty (was -20)
              priority: 20        // v16.27: Lower priority (was 30)
            });
          }
        }
      }
    ...
    if (best.score < 45 && durSec < 0.40) return null; // v16.27: Stricter threshold
    
    // v16.27: Enharmonic substitution for unlikely chromatics
    ...
    // v16.27: AUTO-CORRECT quality based on actual third (use finalRoot!)
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale, inversionBass = null) {
    ...
    // v16.27: AUTO-DETECT which third is actually present!
    ...
    else if (bassNote === actualThird) score += 10; // v16.27: Slightly lower for first inversion
    else if (bassNote === fifth) score += 8;        // v16.27: Slightly lower for second inversion
    
    // v16.27: Extra validation for inversions - the bass note must be STRONG in the audio
  ...
  // v16.27: Check if chromatic chord makes harmonic sense
  isReasonableChromaticChord(root, key, prevChord) {
    ...
    if (!key.minor) {
      ...
      // v16.27: REMOVED bVI (Ab/G#) - too rare and often confused with E
  ...
  validateWithCircleOfFifths(timeline, key, features) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.27: STRICT chromatic filtering
  ...
  addExtensions(timeline, features, key, opts) {
    ...
    return timeline.map(ev => {
      ...
      // v16.27: Check all chord tones properly
      ...
      // 3. Check for 7th chords - v16.27: More precise detection
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
        ...
        if (!isMinor && M7 > 0.10 && M7 > b7 * 1.5 && M3 > 0.08) {
          // v16.27: Also check that M7 is significant relative to root
          if (M7 > r * 0.15) {
        ...
        // Dominant 7th (C7, G7) - only for major chords with clear b7
          ...
          // v16.27: b7 must be prominent enough
          if (b7 > r * 0.12) {
            label += '7';
          }
        }
      ...
      // 4. Check for sus chords - v16.27: More precise
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
      ...
      // 5. Add 6 if present (C6, Am6) - v16.27: Stricter to avoid melodic 6ths
  ...
  finalizeTimeline(timeline, bpm, features) {
    ...
    const spb = 60 / Math.max(60, Math.min(200, bpm));
    const minDuration = 0.5 * spb; // v16.27: Longer minimum (was 0.4)
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      const isStrong = ev.confidence >= 90; // v16.27: Higher threshold (was 85)
  ...
  // v16.27: Detect if song is "easy rock" (mostly diatonic)
  ...
  // v16.27: Force non-diatonic chords to nearest diatonic
  ...
  // v16.27: Remove isolated chromatic chords between identical neighbors
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.27: PATTERN MEMORY - Learn and apply recurring progressions
  // ═══════════════════════════════════════════════════════════
  ...
  // v16.27: mixStereo removed - stereo mixing is in processAudio
  // v16.27: resampleLinear removed - use resample directly
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.27: HMM ENGINE (parallel to Bass engine)
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.27: MERGE ENGINES - Compare and choose best
  // ═══════════════════════════════════════════════════════════
