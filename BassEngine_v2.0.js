...
class BassEngine {
  ...
  async analyzeBass(audioBuffer, options = {}) {
    const opts = {
      ...
      energyPercentile: options.energyPercentile || 60, // Dynamic threshold (was 75, now 60 - more forgiving)
    ...
    for (let i = 0; i < numFrames; i++) {
      ...
      // ✅ RELAXED: was 0.15, now 0.20 (allow more bass through)
      if (zcr > 0.20) {
    ...
    for (let i = 0; i < numFrames; i++) {
      ...
      // ✅ RELAXED: Filter by energy AND quality
      // Was 0.3, now 0.20 (accept more bass)
      if (energies[i] < energyThreshold || candidates[i].quality < 0.20) {
        candidates[i] = null;
        filteredCount++;
      }
    }
