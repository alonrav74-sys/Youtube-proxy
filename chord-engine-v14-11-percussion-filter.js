...
class ChordEngineEnhanced {
  ...
  finalizeTimeline(timeline, key, bpm, feats) {
    ...
    // 🆕 v14.10: Remove chords from percussion-only intro
    const filtered = [];
    
    // Calculate average chroma strength (harmonic content)
    const avgChromaStrength = [];
    for (let i = 0; i < merged.length; i++) {
      const ev = merged[i];
      const frameIdx = ev.fi || 0;
      
      if (frameIdx < feats.chroma.length) {
        // Sum of all chroma values = harmonic content
        const chromaSum = feats.chroma[frameIdx].reduce((a, b) => a + b, 0);
        avgChromaStrength.push({ ev, chromaSum, frameIdx });
      } else {
        avgChromaStrength.push({ ev, chromaSum: 0, frameIdx });
      }
    }
    
    // Find median chroma strength
    const chromaSums = avgChromaStrength.map(x => x.chromaSum).filter(x => x > 0);
    chromaSums.sort((a, b) => a - b);
    const medianChroma = chromaSums[Math.floor(chromaSums.length / 2)] || 0.1;
    
    console.log(`🎵 Median chroma strength: ${medianChroma.toFixed(3)}`);
    
    for (const item of avgChromaStrength) {
      const { ev, chromaSum, frameIdx } = item;
      
      // 🥁 Filter percussion-only sections:
      // If chroma is very weak (< 30% of median) → probably just drums
      if (chromaSum < medianChroma * 0.3) {
        const energy = feats.frameE[frameIdx] || 0;
        console.log(`🥁 Skipping percussion-only at ${ev.t.toFixed(1)}s (chroma: ${chromaSum.toFixed(3)}, energy: ${energy.toFixed(3)})`);
        continue;
      }
    ...
    console.log(`✅ Filtered ${merged.length - filtered.length} percussion-only chords`);
