/**
 * 🎵 ChordEngineEnhanced v15.0 - COMPLETE & CLEAN
 * 
 * All features from v14.36, rewritten with clean code
 * 
 * Features:
 * ✅ Ultimate Minor Detection (m3/M3, m6/M6, m7/M7 analysis)
 * ✅ Bass-based tonic detection with confidence scoring
 * ✅ HMM chord tracking with beam search
 * ✅ Circle of Fifths for transitions and key detection
 * ✅ Borrowed chords (modal mixture: bVII, bVI, bIII, iv)
 * ✅ Ornament classification (passing, neighbor, pedal, structural)
 * ✅ Modal context (secondary dominants, neapolitan, modal borrowing)
 * ✅ Harmonic progression analysis (ii-V-I, IV-V-I patterns)
 * ✅ Key validation from detected chords
 * ✅ Extensions (7th, maj7, 9th, 11th, 13th, sus2, sus4, dim, aug, 6th)
 * ✅ Inversions with bass note detection
 * ✅ Prediction system for next chord
...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    try {
      ...
      timings.audioProcessing = this.now() - t1;
      this.progress(opts, 'extracting', 0.1);

      // 2. Extract features
      const t2 = this.now();
      const features = this.extractFeatures(audioData);
      timings.featureExtraction = this.now() - t2;
      this.progress(opts, 'detecting_key', 0.3);

      // 3. Detect key with enhanced algorithm
      const t3 = this.now();
      let key = this.detectKeyEnhanced(features);
      timings.keyDetection = this.now() - t3;
      this.progress(opts, 'key_detected', 0.4, { 
        key: this.formatKey(key), 
        confidence: key.confidence 
      });

      // 4. Track chords with HMM
      const useFullHMM = key.confidence > 0.80;
      this.progress(opts, useFullHMM ? 'analyzing_full' : 'analyzing_simple', 0.5);
      
      const t4 = this.now();
      let timeline = this.trackChordsHMM(features, key, opts.bassMultiplier, useFullHMM);
      timings.hmmTracking = this.now() - t4;
      this.progress(opts, 'refining', 0.7);

      // 5. Finalize and validate
      timeline = this.finalizeTimeline(timeline, key, audioData.bpm, features);
      const validatedKey = this.validateKeyFromChords(timeline, key, features);

      // Rerun if key changed
      if (validatedKey.root !== key.root || validatedKey.minor !== key.minor) {
        key = validatedKey;
        timeline = this.trackChordsHMM(features, key, opts.bassMultiplier, true);
        timeline = this.finalizeTimeline(timeline, key, audioData.bpm, features);
      }

      this.progress(opts, 'decorating', 0.8);

      // 6. Add musical intelligence
      const t5 = this.now();
      timeline = this.applyMusicalIntelligence(timeline, key, features, audioData.bpm, opts);
      timings.musicalIntelligence = this.now() - t5;

      // 7. Detect tonic and rerun if needed
      ...
      this.progress(opts, 'complete', 1.0);

      // 8. Calculate statistics
      const stats = this.calculateStats(timeline, key);
      const modulations = this.detectModulations(timeline, key);
      ...
      timeline = timeline.filter(ev => 
        ...
        ev.label.trim() !== '' &&
        (ev.fi !== undefined || ev.frameIdx !== undefined)
      );

      // Ensure fi exists on all events
      timeline = timeline.map(ev => ({
        ...ev,
        fi: ev.fi !== undefined ? ev.fi : ev.frameIdx
      }));
  ...
  // ═══════════════════════════════════════════════════════════
  // 🔑 ENHANCED KEY DETECTION (Ultimate Minor Detection)
  // ═══════════════════════════════════════════════════════════
  ...
  detectKeyEnhanced(feats) {
    ...
    if (bassTonic.confidence > 0.25) {
      ...
      // 🎯 Ultimate Minor Detection: Analyze m3 vs M3, m6 vs M6, m7 vs M7
  ...
  detectTonicFromBass(feats) {
    ...
    // Analyze bass for minor/major hints
  ...
  trackChordsHMM(feats, key, bassMultiplier, useFullMode = true) {
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 MUSICAL INTELLIGENCE (All advanced features)
  // ═══════════════════════════════════════════════════════════
  
  applyMusicalIntelligence(timeline, key, feats, bpm, opts) {
    // 1. Enforce early diatonic
    timeline = this.enforceEarlyDiatonic(timeline, key, feats, bpm);
    
    // 2. Add qualities (7th, sus, dim, aug, etc)
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 KEY VALIDATION & TONIC DETECTION
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // 📊 STATISTICS & MODULATION DETECTION
  // ═══════════════════════════════════════════════════════════
  ...
  detectModulations(timeline, primaryKey) {
  ...
  progress(opts, stage, progress, extra = {}) {
    if (opts.progressCallback) {
      opts.progressCallback({ stage, progress, ...extra });
    }
  }
