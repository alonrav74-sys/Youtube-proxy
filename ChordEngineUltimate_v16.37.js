/**
 * ChordEngine v16.37 - Conservative Filtering
 * 
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log('🎵 ChordEngine v16.37 (Conservative - Fewer but Solid Chords)');
    ...
    // v16.37: TONIC VALIDATION after timeline (section 6)
    ...
    // v16.37: Enhanced slash chord detection (section 6)
    ...
    // v16.37: AUTOMATIC STYLE PROFILE DETECTION
    ...
    // v16.37: Pattern memory V2 (section 6 - fixes anomalies in repeated patterns)
    ...
    return {
      ...
      mode: modeResult,
      styleProfile,  // v16.37: easy_pop / colorful_pop / jazz_like
  ...
  processAudio(audioBuffer) {
    ...
    // v16.37: Clean signal before processing
  ...
  // v16.37: Clean signal - pre-emphasis + noise gate + smoothing
  cleanSignal(x, sr) {
  ...
  extractFeatures(audio) {
    ...
    for (let start = 0; start + win <= x.length; start += hop) {
      ...
      // v16.37: Improved chroma with harmonic weighting
      ...
      for (let bin = 1; bin < mags.length; bin++) {
        ...
        // v16.37: Weight by harmonic clarity
    ...
    // v16.37: Compute spectral flux for chord change detection
    ...
    return {
      ...
      flux, fluxP80, // v16.37: Added flux
  ...
  // v16.37: Spectral flux - detects changes in chroma
  ...
  // v16.37: Find candidate frames for chord changes
  ...
  detectBassNote(mags, sr, N) {
    const fmin = 40, fmax = 300; // v16.37: Extended range (was 250)
    ...
    for (let bin = 1; bin < mags.length; bin++) {
      ...
      if (freq >= fmin) {
        ...
        const weight = freq < 150 ? 1.5 : 1.0; // v16.37: Boost very low freqs
    ...
    // v16.37: Stricter threshold (was 0.25)
  ...
  // v16.37: Improved - also detect clear pitch without bass (guitar/piano intros)
  findMusicStart(features) {
    ...
    for (let i = 0; i < energy.length; i++) {
      ...
      // v16.37: Check for clear musical pitch in chroma (for guitar/piano intros)
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.37: SONG PROFILE ANALYSIS (Dynamic thresholds) - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.37: TONIC VALIDATION FROM CHORDS - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.37: BETTER SLASH CHORD DETECTION - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.37: IMPROVED PATTERN MEMORY (3+4 chord patterns) - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 v16.37: STABLE BASS CHORD BUILDING with Beat-Aware Detection
  // ═══════════════════════════════════════════════════════════
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    // v16.37: MUCH STRICTER thresholds to avoid noise
    ...
    // v16.37: Get chord change candidate frames (beat-aligned + flux + bass changes)
    const changeFrames = this.getChordChangeCandidates(features, bpm, startFrame);
    console.log(`🎵 v16.37: ${changeFrames.length} chord change candidates (beat-aligned)`);
  ...
  getBorrowedChords(key) {
    ...
    if (!key.minor) {
      ...
      // v16.37: REMOVED bVI (Ab/G#) - causes confusion with E
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    if (candidates.length === 0) {
      // v16.37: Chromatic fallback - only if VERY clear
      for (const isMinor of [false, true]) {
        const score = this.scoreChordCandidate(avg, bassNote, isMinor, bassNote, false);
        if (score > 60) { // v16.37: Higher threshold (was 40)
          ...
          if (isReasonable || score > 80) { // Only if reasonable OR super strong
            candidates.push({
              ...
              inversionBass: null,
              chordType: 'chromatic',
              score: score - 30, // v16.37: Bigger penalty (was -20)
              priority: 20        // v16.37: Lower priority (was 30)
            });
          }
        }
      }
    ...
    if (best.score < 45 && durSec < 0.40) return null; // v16.37: Stricter threshold
    
    // v16.37: Enharmonic substitution for unlikely chromatics
    ...
    // v16.37: AUTO-CORRECT quality based on actual third (use finalRoot!)
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale) {
    ...
    // v16.37: AUTO-DETECT which third is actually present!
    ...
    else if (bassNote === actualThird) score += 10; // v16.37: Slightly lower for first inversion
    else if (bassNote === fifth) score += 8;        // v16.37: Slightly lower for second inversion
  ...
  // v16.37: Check if chromatic chord makes harmonic sense
  isReasonableChromaticChord(root, key, prevChord) {
    ...
    if (!key.minor) {
      ...
      // v16.37: REMOVED bVI (Ab/G#) - too rare and often confused with E
  ...
  validateWithCircleOfFifths(timeline, key, features) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.37: STRICT chromatic filtering
  ...
  addExtensions(timeline, features, key, opts) {
    ...
    return timeline.map(ev => {
      ...
      // v16.37: Check all chord tones properly
      ...
      // 3. Check for 7th chords - v16.37: More precise detection
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
        ...
        if (!isMinor && M7 > 0.10 && M7 > b7 * 1.5 && M3 > 0.08) {
          // v16.37: Also check that M7 is significant relative to root
          if (M7 > r * 0.15) {
        ...
        // Dominant 7th (C7, G7) - only for major chords with clear b7
          ...
          // v16.37: b7 must be prominent enough
          if (b7 > r * 0.12) {
            label += '7';
          }
        }
      ...
      // 4. Check for sus chords - v16.37: More precise
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
      ...
      // 5. Add 6 if present (C6, Am6) - v16.37: Stricter to avoid melodic 6ths
  ...
  finalizeTimeline(timeline, bpm, features) {
    ...
    // v16.37: MUCH STRICTER filtering like v14.36
    ...
    // v16.37: Second pass - remove any remaining short chords after merging
    const final = [];
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.37: ENFORCE EARLY DIATONIC (from v14.36)
  ...
  // v16.37: Detect if song is "easy rock" (mostly diatonic)
  ...
  // v16.37: Force non-diatonic chords to nearest diatonic
  ...
  // v16.37: Remove isolated chromatic chords between identical neighbors
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.37: HMM ENGINE (parallel to Bass engine)
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.37: MERGE ENGINES - Compare and choose best
  // ═══════════════════════════════════════════════════════════
