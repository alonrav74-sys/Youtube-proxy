...
class ChordEngineUltimate {
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    // v16.15b: זיהוי ארפג'יו - מנתח כל פריים בנפרד לזיהוי מדויק יותר
    const arpeggioAnalysis = this.analyzeArpeggio(chroma, startFrame, endFrame, bassNote, key);
    
    for (const tc of theoryCandidates) {
      let score = this.scoreChordCandidate(avg, tc.root, tc.isMinor, bassNote, tc.chordType.startsWith('diatonic'));
      
      // v16.15b: בונוס אם הארפג'יו תומך באקורד הזה
      if (arpeggioAnalysis && arpeggioAnalysis.suggestedRoot === tc.root) {
        score += arpeggioAnalysis.confidence * 0.3;
        if (arpeggioAnalysis.isMinor === tc.isMinor) {
          score += 10; // התאמה מושלמת
        }
      }
    ...
    if (candidates.length === 0) {
      ...
      // v16.15b: BLOCKED in SAFE mode!
      const opts = this.currentOpts || {};
      if (opts.profile === 'safe') {
        // במצב SAFE אין אקורדים כרומטיים בכלל!
        return null;
      }
    ...
    if (bassNote >= 0) {
      ...
      // v16.15b: משקלות מאוזנים - 1 הכי חזק, 3 חזק, 5 משמעותי
      const majorTriadScore = bassRootEnergy * 1.4 + bassMajorThird * 1.2 + bassFifthEnergy * 1.0;
      const minorTriadScore = bassRootEnergy * 1.4 + bassMinorThird * 1.2 + bassFifthEnergy * 1.0;
    ...
    const opts = this.currentOpts || {};
    if (opts.profile === 'safe') {
      const rootInScale = diatonic.pcs.includes(best.root);
      const bassInScale = bassNote >= 0 ? diatonic.pcs.includes(bassNote) : true;
      
      // חישוב משך האקורד
      const durSec = (endFrame - startFrame) * (secPerFrame || 0.1);
      
      // אם הבאס לא בסולם
      if (!bassInScale) {
        // אקורד קצר עם באס לא דיאטוני = רעש, התעלם
        if (durSec < 0.8) {
          console.log(`🔒 SAFE: ignoring short chromatic bass ${this.NOTES_SHARP[bassNote]} (${durSec.toFixed(2)}s) - likely noise`);
          return null;
        }
        
        // באס ארוך לא דיאטוני - נסה למצוא אקורד הגיוני
        // G# → כנראה E (G# הוא שלישית של E)
        // F# → כנראה D/F# (F# הוא שלישית של D)
        // C# → כנראה A/C# (C# הוא שלישית של A)
        
        const possibleRoots = [
          { root: this.toPc(bassNote - 4), name: 'major 3rd below' },  // באס הוא שלישית מז'ורית
          { root: this.toPc(bassNote - 3), name: 'minor 3rd below' },  // באס הוא שלישית מינורית
          { root: this.toPc(bassNote - 7), name: '5th below' }         // באס הוא חמישית
        ];
        
        for (const pr of possibleRoots) {
          if (diatonic.pcs.includes(pr.root)) {
            // מצאנו שורש דיאטוני!
            const dc = diatonic.chords.find(c => c.root === pr.root);
            if (dc) {
              const chordName = this.getNoteName(pr.root, key) + (dc.minor ? 'm' : '');
              const bassName = this.getNoteName(bassNote, key);
              console.log(`🎵 SAFE: converting chromatic bass ${this.NOTES_SHARP[bassNote]} → ${chordName}/${bassName} (${pr.name})`);
              
              // אבל אם הבאס עצמו לא דיאטוני, פשוט נחזיר את האקורד בלי היפוך
              console.log(`🔒 SAFE: returning ${chordName} without chromatic bass`);
              return {
                root: pr.root,
                label: chordName,
                type: dc.minor ? 'minor' : 'major',
                inScale: true,
                confidence: Math.min(100, Math.round(best.score * 0.8)),
                chordType: 'diatonic_converted',
                isInversion: false,
                inversionBass: null
              };
            }
        ...
        // לא מצאנו שום אקורד הגיוני - התעלם
        console.log(`🔒 SAFE: no logical chord for bass ${this.NOTES_SHARP[bassNote]} - ignoring`);
        return null;
      }
      ...
      if (!rootInScale) {
        // אקורד קצר = רעש
        if (durSec < 0.8) {
          console.log(`🔒 SAFE: ignoring short chromatic root ${this.NOTES_SHARP[best.root]} (${durSec.toFixed(2)}s) - likely noise`);
          return null;
        }
        
        // אקורד ארוך לא דיאטוני - נסה להמיר
        // G# → E, F# → D או F, C# → A
        const conversions = {
          [this.toPc(key.root + 8)]: this.toPc(key.root + 4),  // G# → E (in C)
          [this.toPc(key.root + 6)]: this.toPc(key.root + 5),  // F# → F (in C)
          [this.toPc(key.root + 1)]: this.toPc(key.root + 9),  // C# → A (in C)
          [this.toPc(key.root + 10)]: this.toPc(key.root + 9), // A# → A or Bb (in C) - try A first
        };
        
        if (conversions[best.root] !== undefined) {
          const newRoot = conversions[best.root];
          if (diatonic.pcs.includes(newRoot)) {
            const dc = diatonic.chords.find(c => c.root === newRoot);
            if (dc) {
              const chordName = this.getNoteName(newRoot, key) + (dc.minor ? 'm' : '');
              console.log(`🎵 SAFE: converting ${this.NOTES_SHARP[best.root]} → ${chordName}`);
              return {
                root: newRoot,
                label: chordName,
                type: dc.minor ? 'minor' : 'major',
                inScale: true,
                confidence: Math.min(100, Math.round(best.score * 0.7)),
                chordType: 'diatonic_converted',
                isInversion: false,
                inversionBass: null
              };
        ...
        // לא הצלחנו להמיר - התעלם
        console.log(`🔒 SAFE: cannot convert ${this.NOTES_SHARP[best.root]} to diatonic - ignoring`);
        return null;
      }
    }
    
    // v16.14: Enharmonic substitution for unlikely chromatics (BALANCED/RICH only)
    // Example: G# in C major → FORCE to E (modal interchange)
    let finalRoot = best.root;
    let forcedToE = false; // v16.15b: track if we forced G# → E
    
    if (!key.minor && !diatonic.pcs.includes(best.root) && opts.profile !== 'safe') {
      ...
      const forcedSubstitutions = [
        { from: toPc(8),  to: toPc(4),  name: 'G#→E', forceMajor: true },   // G# → E MAJOR (very common!)
      ...
      for (const sub of forcedSubstitutions) {
        if (best.root === sub.from) {
          finalRoot = sub.to;
          if (sub.forceMajor) forcedToE = true;
    ...
    // v16.15b: אם המרנו G# ל-E, תמיד E מז'ור (לא Em)
    // כי E מז'ור הוא III או V/vi - נפוץ מאוד
    // opts כבר מוגדר למעלה
    if (forcedToE && opts.profile === 'safe') {
      finalIsMinor = false; // E major, not Em
      console.log(`🎵 SAFE: Forcing E major (not Em) after G#→E substitution`);
    } else if (m3 > M3 * 1.5) {
    ...
    // v16.16: במצב SAFE, לא מאפשרים inversions עם באס לא דיאטוני
    if (best.inversionBass !== null) {
      const bassInScale = diatonic.pcs.includes(best.inversionBass);
      
      if (opts.profile === 'safe' && !bassInScale) {
        // באס לא דיאטוני - מחזירים אקורד בסיסי בלי inversion
        console.log(`🔒 SAFE: removing non-diatonic bass /${this.NOTES_SHARP[best.inversionBass]} from ${label}`);
        best.inversionBass = null;
      } else {
    ...
    // v16.15b: במצב SAFE, לא מחזירים אקורדים שלא בסולם
    // opts כבר מוגדר למעלה
    if (opts.profile === 'safe' && !inScale) {
      console.log(`🔒 SAFE: blocking ${this.NOTES_SHARP[finalRoot]} (not in scale)`);
      return null;
    }
  ...
  analyzeArpeggio(chroma, startFrame, endFrame, bassNote, key) {
    if (endFrame - startFrame < 4) return null; // צריך לפחות 4 פריימים
    
    const notesDetected = [];
    const threshold = 0.15;
    
    // עוברים על הפריימים ומזהים איזה תו דומיננטי בכל פריים
    for (let i = startFrame; i < endFrame && i < chroma.length; i++) {
      ...
      if (maxNote >= 0) {
        // הוספה רק אם זה תו חדש (לא כפילות)
        if (notesDetected.length === 0 || notesDetected[notesDetected.length - 1].note !== maxNote) {
          notesDetected.push({ note: maxNote, strength: maxVal, frame: i });
        }
      }
    }
    
    if (notesDetected.length < 2) return null; // צריך לפחות 2 תווים שונים
    
    // מחפשים דפוס של טריאדה
    const uniqueNotes = [...new Set(notesDetected.map(n => n.note))];
    
    // בדיקה: האם זו טריאדה על הבאס?
    const bassRoot = bassNote;
    const bassM3 = this.toPc(bassRoot + 4);
    const bassm3 = this.toPc(bassRoot + 3);
    const bass5 = this.toPc(bassRoot + 7);
    const bassm7 = this.toPc(bassRoot + 10);
    const bassM7 = this.toPc(bassRoot + 11);
    
    // בדיקת טריאדה מז'ורית על הבאס
    const hasMajorTriad = uniqueNotes.includes(bassRoot) && uniqueNotes.includes(bassM3) && uniqueNotes.includes(bass5);
    const hasMinorTriad = uniqueNotes.includes(bassRoot) && uniqueNotes.includes(bassm3) && uniqueNotes.includes(bass5);
    
    // בדיקת שביעית
    const has7 = uniqueNotes.includes(bassm7);
    const hasMaj7 = uniqueNotes.includes(bassM7);
    
    if (hasMajorTriad || hasMinorTriad) {
      const isMinor = hasMinorTriad && !hasMajorTriad;
      let chordType = isMinor ? 'minor' : 'major';
      
      if (has7) chordType = isMinor ? 'm7' : '7';
      if (hasMaj7 && !isMinor) chordType = 'maj7';
      
      console.log(`🎵 Arpeggio detected: ${this.NOTES_SHARP[bassRoot]}${isMinor ? 'm' : ''}${has7 ? '7' : ''}${hasMaj7 ? 'maj7' : ''} (notes: ${uniqueNotes.map(n => this.NOTES_SHARP[n]).join(',')})`);
      
      return {
        suggestedRoot: bassRoot,
        isMinor: isMinor,
        chordType: chordType,
        confidence: Math.min(100, uniqueNotes.length * 25),
        has7: has7,
        hasMaj7: hasMaj7,
    ...
    // בדיקה: אולי זה היפוך? (הבאס הוא 3 או 5 של אקורד אחר)
    // בודקים אקורדים דיאטוניים
    const diatonic = this.getDiatonicInfo(key);
    
    for (const dc of diatonic.chords) {
      const root = dc.root;
      const third = this.toPc(root + (dc.minor ? 3 : 4));
      const fifth = this.toPc(root + 7);
      
      // האם הבאס הוא השלישית או החמישית?
      if (bassNote === third || bassNote === fifth) {
        // בדיקה אם כל התווים של הטריאדה קיימים
        const hasRoot = uniqueNotes.includes(root);
        const hasThird = uniqueNotes.includes(third);
        const hasFifth = uniqueNotes.includes(fifth);
        
        if (hasRoot && hasThird && hasFifth) {
          const inversionType = bassNote === third ? 'first' : 'second';
          console.log(`🎵 Arpeggio inversion detected: ${this.NOTES_SHARP[root]}${dc.minor ? 'm' : ''}/${this.NOTES_SHARP[bassNote]} (${inversionType} inversion)`);
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale) {
    ...
    const fifth = this.toPc(root + 7);
    const m7 = this.toPc(root + 10); // minor 7th (dominant 7, m7)
    const M7 = this.toPc(root + 11); // major 7th (maj7)
    
    // v16.14: AUTO-DETECT which third is actually present!
    const m3Strength = avg[m3];
    const M3Strength = avg[M3];
    const fifthStrength = avg[fifth];
    const m7Strength = avg[m7];
    const M7Strength = avg[M7];
    ...
    let score = rootStrength * 40 + avg[actualThird] * 32 + fifthStrength * 24;
    score -= avg[wrongThird] * 20;
    
    // v16.15b: בונוס אם כל 3 התווים קיימים (טריאדה מלאה)
    const triadThreshold = 0.06;
    if (rootStrength > triadThreshold && avg[actualThird] > triadThreshold && fifthStrength > triadThreshold) {
      score += 10; // Full triad bonus
    }
    
    // v16.15b: זיהוי שביעית לפרופיל RICH
    const opts = this.currentOpts || {};
    if (opts.profile === 'rich' || opts.allowExtensions) {
      // Check for 7th
      if (m7Strength > 0.08) {
        score += 5; // Likely a 7 chord
      }
      if (M7Strength > 0.08) {
        score += 5; // Likely a maj7 chord
      }
    }
  ...
  validateWithCircleOfFifths(timeline, key, features, opts = {}) {
    ...
    // v16.15b: במצב SAFE, האקורד האחרון חייב להיות בסולם
    if (opts.profile === 'safe' && validated.length > 0) {
      const last = validated[validated.length - 1];
      const diatonic = this.getDiatonicInfo(key);
      
      if (!diatonic.pcs.includes(last.root)) {
        // האקורד האחרון לא בסולם - מחפשים את הקרוב ביותר בסולם
        console.log(`🔒 SAFE: Last chord ${this.NOTES_SHARP[last.root]} not in scale, looking for substitute...`);
        
        // III מז'ור מקובל כסיום (E בסולם C)
        const III = this.toPc(key.root + 4);
        if (last.root === III && last.type !== 'minor') {
          // E major is acceptable as ending
          console.log(`   → Keeping III major as acceptable ending`);
        } else {
          // החלפה לטוניקה
          const tonicName = this.getNoteName(key.root, key);
          const newLabel = key.minor ? tonicName + 'm' : tonicName;
          console.log(`   → Replacing with tonic: ${newLabel}`);
  ...
  // v16.15b: ניתוח מיקום אקורדים ביחס לקצב
  // אקורדים בדרך כלל משתנים על הביט, לא באמצע
  analyzeBeatPositions(timeline, bpm, features) {
    const spb = 60 / Math.max(60, Math.min(200, bpm)); // seconds per beat
    const beatsPerBar = 4; // נניח 4/4
    const spBar = spb * beatsPerBar; // seconds per bar
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // מיקום בתוך הביט (0 = בדיוק על הביט)
      const beatPosition = time / spb;
      const beatFraction = beatPosition - Math.floor(beatPosition);
      
      // מיקום בתוך התיבה
      const barPosition = time / spBar;
      const barFraction = barPosition - Math.floor(barPosition);
      
      // בדיקות
      const tolerance = 0.15; // סובלנות של 15% מהביט
      
      const onBeat = beatFraction < tolerance || beatFraction > (1 - tolerance);
      const onHalfBeat = Math.abs(beatFraction - 0.5) < tolerance;
      const onDownbeat = barFraction < tolerance || barFraction > (1 - tolerance); // ביט 1 של התיבה
      const onStrongBeat = Math.abs(barFraction - 0.5) < tolerance; // ביט 3 (חצי תיבה)
      
      // ביטים חזקים: 1 ו-3 בתיבת 4/4
      const isStrongPosition = onDownbeat || onStrongBeat;
      ...
      beatPositions.set(i, {
        ...
        onBeat: onBeat,
        onHalfBeat: onHalfBeat,
        onDownbeat: onDownbeat,
        onStrongBeat: onStrongBeat,
        isStrongPosition: isStrongPosition
      });
    }
  ...
  finalizeTimeline(timeline, bpm, features) {
    ...
    // v16.15b: ניתוח קצב - אקורדים צריכים ליפול על הביט או על חצי ביט
    const beatPositions = this.analyzeBeatPositions(timeline, bpm, features);
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.15b: בדיקת קצב - האם האקורד נופל על ביט?
      const beatInfo = beatPositions.get(i);
      const isOnBeat = beatInfo && beatInfo.onBeat;
      const isOnHalfBeat = beatInfo && beatInfo.onHalfBeat;
      const isOnDownbeat = beatInfo && beatInfo.onDownbeat; // תחילת תיבה
      
      // אקורד שלא על הביט צריך להיות מאוד חזק כדי להישאר
      if (!isOnBeat && !isOnHalfBeat && !isOnDownbeat) {
        if (isShort || !isStrong) {
          console.log(`🥁 Filtered off-beat chord: ${ev.label} at ${ev.t.toFixed(2)}s`);
          continue;
        }
      }
