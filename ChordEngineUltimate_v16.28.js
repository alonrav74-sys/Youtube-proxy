/**
 * ChordEngine v16.28 - Stricter Filtering (v14.36 lessons)
 * 
 * Based on v16.25 + lessons from v14.36:
 * - calcEmitScore: Hard threshold (matchScore < 0.30 = reject)
 * - Stronger energy penalty in emit scores
 * - findMusicStart: Accepts chroma without bass (for acoustic intros)
 * - MIN_DURATION: 0.7s (was 0.6s)
 * - MIN_DURATION_NO_BASS: 1.5s (was 1.2s)
 * - Energy threshold: 0.4 * P70 (was 0.3)
 * - Auto-clean for passing chords
 */
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log('🎵 ChordEngine v16.25 (Conservative - Fewer but Solid Chords)');
    ...
    const modeResult = this.detectModeFromThird(features, tonicResult.root, 0); // v16.28: Start from 0!
    ...
    // v16.28: START FROM FRAME 0 (like v14.36) - don't skip intro!
    let timeline = this.buildChordsStableBass(features, key, 0, audio.bpm);
    console.log(`🎸 Bass engine: ${timeline.length} chords`);
    
    // v16.25: Run HMM engine in parallel and compare
    const hmmTimeline = this.buildChordsHMM(features, key, 0); // v16.28: Start from 0!
    ...
    // v16.25: Auto-detect "easy rock" songs and clean up
    const stats = this.buildStats(timeline, key);
    const isEasy = this.isEasyRockSong(stats, key);
    
    if (isEasy) {
      console.log(`🎸 v16.25: Easy rock song detected - enforcing diatonic`);
      timeline = this.enforceEasyDiatonic(timeline, key);
      
      // v16.25: Apply pattern memory for easy rock songs
      timeline = this.applyPatternMemory(timeline, key);
    }
    
    // v16.25: Always run outlier smoothing (removes isolated chromatic chords)
    timeline = this.smoothOutliers(timeline, key, this.getDiatonicInfo(key));

    // 🧠 v16.28: Auto-clean invented passing chords using harmonic memory
  ...
  processAudio(audioBuffer) {
    ...
    // v16.25: Clean signal before processing
  ...
  // v16.25: Clean signal - pre-emphasis + noise gate + smoothing
  cleanSignal(x, sr) {
  ...
  extractFeatures(audio) {
    ...
    for (let start = 0; start + win <= x.length; start += hop) {
      ...
      // v16.25: Improved chroma with harmonic weighting
      ...
      for (let bin = 1; bin < mags.length; bin++) {
        ...
        // v16.25: Weight by harmonic clarity
    ...
    // v16.25: Compute spectral flux for chord change detection
    ...
    return {
      ...
      flux, fluxP80, // v16.25: Added flux
  ...
  // v16.25: Spectral flux - detects changes in chroma
  ...
  // v16.25: Find candidate frames for chord changes
  ...
  detectBassNote(mags, sr, N) {
    const fmin = 40, fmax = 300; // v16.25: Extended range (was 250)
    ...
    for (let bin = 1; bin < mags.length; bin++) {
      ...
      if (freq >= fmin) {
        ...
        const weight = freq < 150 ? 1.5 : 1.0; // v16.25: Boost very low freqs
    ...
    // v16.25: Stricter threshold (was 0.25)
  ...
  // v16.28: Fixed - accept clear pitch even without bass (for acoustic intros)
  findMusicStart(features) {
    ...
    for (let i = 0; i < energy.length; i++) {
      ...
      // v16.28: Check for clear musical pitch in chroma (for guitar/piano intros)
    ...
    // v16.28: If still no music found, start from frame 0
    if (musicFrame === 0 && stableCount < 2) {
      console.log('⚠️ v16.28: No clear music start found, starting from 0');
    }
  ...
  // v16.25: getFirstStrongChord and detectTriadFromChroma removed - not used in new tonic detection
  ...
  // v16.25: detectModeHybrid removed - using detectModeFromThird directly
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 v16.25: STABLE BASS CHORD BUILDING - Bass is King
  // ═══════════════════════════════════════════════════════════
  
  // v16.28: Start from frame 0 like v14.36 (not from startFrame)
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    // v16.28: Stricter thresholds based on v14.36 lessons
    const MIN_DURATION_SEC = 0.7;              // Was 0.6
    const MIN_DURATION_NO_BASS_CHANGE = 1.5;   // Was 1.2
    const CHROMA_CHANGE_THRESHOLD = 0.40;      // Was 0.35
    const SIGNIFICANT_THIRD_FIFTH_CHANGE = 0.30; // Was 0.25
    
    let currentBass = -1;
    let currentStart = 0;  // v16.28: Start from 0, not startFrame!
    let currentChroma = null;
    
    // v16.28: Get candidates from frame 0, not startFrame
    const changeFrames = this.getChordChangeCandidates(features, bpm, 0);
    console.log(`🎵 v16.28: ${changeFrames.length} chord change candidates (from frame 0)`);
    
    for (const i of changeFrames) {
      if (i >= bass.length) continue;
      
      const bp = bass[i];
      const hasEnergy = energy[i] >= energyP70 * 0.4; // v16.28: Stricter (was 0.3)
      
      if (!hasEnergy || bp < 0) continue;
      
      if (currentBass === -1) {
        // First bass note found - CREATE FIRST CHORD IMMEDIATELY!
        currentBass = bp;
        currentStart = i;
        currentChroma = this.avgChroma(chroma, i, Math.min(i + 3, chroma.length));
        
        // v16.28: Add FIRST chord RIGHT NOW (don't wait for change)
        const firstChord = this.determineChordTheoryAware(
          chroma, i, Math.min(i + 5, chroma.length), key, diatonic, bp, secPerFrame
        );
        if (firstChord) {
          console.log(`🎸 Bass: First chord ${firstChord.label} at t=${(i * secPerFrame).toFixed(2)}s`);
          timeline.push({
            t: i * secPerFrame,
            fi: i,
            label: firstChord.label,
            root: firstChord.root,
            type: firstChord.type,
            bassNote: bp,
            inScale: firstChord.inScale,
      ...
      if (currentChroma) {
        ...
        // v16.25: Check specifically for 3rd and 5th changes
        thirdFifthChange = this.calculateThirdFifthChange(currentChroma, chroma[i], currentBass);
      }
      
      // v16.25: Decision logic - Bass is King
      ...
      if (bassChanged) {
        ...
        if (durationSec >= MIN_DURATION_NO_BASS_CHANGE && chromaChange >= CHROMA_CHANGE_THRESHOLD) {
          ...
          shouldCommit = true;
          console.log(`  🎹 v16.25: Harmonic change detected (3rd/5th) without bass change at ${(i * secPerFrame).toFixed(2)}s`);
        }
  ...
  // v16.25: Check for significant change in 3rd and 5th intervals
  ...
  getChordsWithBassNote(bassNote, key) {
    ...
    // v16.25: Check if bass note is diatonic scale degree
    const bassIsDiatonic = diatonic.pcs.includes(bassNote);
    
    // v16.25: HIGHEST PRIORITY - Bass is root of diatonic chord
    ...
    for (const dc of diatonic.chords) {
      if (dc.root === bassNote) {
        candidates.push({
          ...
          priority: bassIsDiatonic ? 120 : 100 // v16.25: Extra boost if diatonic bass
        });
      }
    }
    
    // v16.25: LOWER PRIORITY for inversions when bass is diatonic
    ...
    for (const dc of diatonic.chords) {
      ...
      if (third === bassNote) {
        candidates.push({
          ...
          priority: bassIsDiatonic ? 70 : 85 // v16.25: Lower if bass is diatonic (prefer root position)
        });
      }
    }
  ...
  getBorrowedChords(key) {
    ...
    if (!key.minor) {
      ...
      // v16.25: REMOVED bVI (Ab/G#) - causes confusion with E
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    for (const tc of theoryCandidates) {
      // v16.25: Pass inversionBass to scoreChordCandidate
        ...
        if (score > 60) { // v16.25: Higher threshold (was 40)
          ...
          if (isReasonable || score > 80) { // Only if reasonable OR super strong
            candidates.push({
              ...
              inversionBass: null,
              chordType: 'chromatic',
              score: score - 30, // v16.25: Bigger penalty (was -20)
              priority: 20        // v16.25: Lower priority (was 30)
            });
          }
        }
      }
    ...
    if (best.score < 45 && durSec < 0.40) return null; // v16.25: Stricter threshold
    
    // v16.25: Enharmonic substitution for unlikely chromatics
    ...
    // v16.25: AUTO-CORRECT quality based on actual third (use finalRoot!)
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale, inversionBass = null) {
    ...
    // v16.25: AUTO-DETECT which third is actually present!
    ...
    else if (bassNote === actualThird) score += 10; // v16.25: Slightly lower for first inversion
    else if (bassNote === fifth) score += 8;        // v16.25: Slightly lower for second inversion
    
    // v16.25: Extra validation for inversions - the bass note must be STRONG in the audio
  ...
  // v16.25: Check if chromatic chord makes harmonic sense
  isReasonableChromaticChord(root, key, prevChord) {
    ...
    if (!key.minor) {
      ...
      // v16.25: REMOVED bVI (Ab/G#) - too rare and often confused with E
  ...
  validateWithCircleOfFifths(timeline, key, features) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.25: STRICT chromatic filtering
  ...
  addExtensions(timeline, features, key, opts) {
    ...
    return timeline.map(ev => {
      ...
      // v16.25: Check all chord tones properly
      ...
      // 3. Check for 7th chords - v16.25: More precise detection
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
        ...
        if (!isMinor && M7 > 0.10 && M7 > b7 * 1.5 && M3 > 0.08) {
          // v16.25: Also check that M7 is significant relative to root
          if (M7 > r * 0.15) {
        ...
        // Dominant 7th (C7, G7) - only for major chords with clear b7
          ...
          // v16.25: b7 must be prominent enough
          if (b7 > r * 0.12) {
            label += '7';
          }
        }
      ...
      // 4. Check for sus chords - v16.25: More precise
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
      ...
      // 5. Add 6 if present (C6, Am6) - v16.25: Stricter to avoid melodic 6ths
  ...
  // v16.28: Stricter finalization based on v14.36
  finalizeTimeline(timeline, bpm, features) {
    if (!timeline.length) return [];
    
    const spb = 60 / Math.max(60, Math.min(200, bpm));
    const minDuration = 0.55 * spb;
    const { energy, energyP50 } = features;
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.28: ALWAYS KEEP FIRST CHORD (index 0) - critical for song start!
      if (i === 0) {
        filtered.push(ev);
        continue;
      }
  ...
  // v16.25: Detect if song is "easy rock" (mostly diatonic)
  ...
  // v16.25: Force non-diatonic chords to nearest diatonic
  ...
  // ═══════════════════════════════════════════════════════════
  // 🧠 v16.28: AUTO REFINEMENT – זיכרון הרמוני נגד אקורדים מומצאים
  // ═══════════════════════════════════════════════════════════
  
  autoRefineTimeline(timeline, key, totalDuration) {
    if (!timeline || timeline.length === 0) return timeline;

    // 1. משך לכל אקורד (נשתמש בזה לסינון קצרצרים)
    this.computeSegmentDurations(timeline, totalDuration);

    // 2. סטטיסטיקות קיימות + פרופיל מורכבות
    const stats = this.buildStats(timeline, key);
    const profile = this.autoComplexityProfileFromStats(stats);
    const isSimple = (profile === 'simple');
    
    console.log(`🧠 v16.28 AUTO-CLEAN: profile=${profile}, isSimple=${isSimple}`);

    // 3. מידע דיאטוני וזיכרון הרמוני (כמה פעמים כל אקורד הופיע)
    ...
    // סף משך – יותר אגרסיבי בשירים פשוטים
    const maxShortNonDiatonic = isSimple ? 0.70 : 0.45;
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      if (candidateForDrop) {
        // מדלגים – לא מוסיפים ל-cleaned → האקורד "נעלם"
        console.log(`  🧹 AUTO: dropped passing chord ${ev.label} (non-diatonic, short, single, between ${prev.label})`);
        continue;
      }
    ...
    // 4. מיזוג כפולים רצופים שנוצרו אחרי הניקוי
    const merged = [];
    for (const ev of cleaned) {
      const last = merged[merged.length - 1];
      if (last && last.label === ev.label) {
        // שומרים את הראשון, מתעלמים מהכפול
        continue;
      }
      merged.push(ev);
    }
    
    console.log(`  🧹 AUTO-CLEAN: ${timeline.length} → ${merged.length} chords`);

    return merged;
  }

  // מחשב משך לכל אקורד לפי הזמן של הבא אחריו / סוף השיר
  ...
  // פרופיל מורכבות על בסיס הסטטיסטיקות הקיימות
  autoComplexityProfileFromStats(stats) {
    ...
    // שירים כמו Hallelujah: כמעט הכול דיאטוני, מעט מאוד חריגים
  ...
  // זיכרון הרמוני – כמה פעמים כל אקורד (בלי סלאש) הופיע
  ...
  // שורש האקורד לצורכי זיהוי תבניות – בלי maj7, sus, /bass וכו'
  baseLabelForPattern(label) {
    if (!label) return '';
    // מוריד הרחבות וסלאשים, משאיר רק root + m אם יש
    return label
      .replace(/7|maj7|sus[24]|dim|aug|6|9|11|13|add[0-9]+|\/.*$/g, '')
      .trim()
      .toUpperCase();
  }

  // v16.25: Remove isolated chromatic chords between identical neighbors
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.25: PATTERN MEMORY - Learn and apply recurring progressions
  // ═══════════════════════════════════════════════════════════
  ...
  // v16.25: mixStereo removed - stereo mixing is in processAudio
  // v16.25: resampleLinear removed - use resample directly
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.25: HMM ENGINE (parallel to Bass engine)
  // ═══════════════════════════════════════════════════════════
  ...
  buildChordsHMM(features, key, startFrame) {
    ...
    // v16.28: START FROM FRAME 0 like v14.36 (not from startFrame!)
    // This ensures we detect chords even in acoustic intros
    ...
    // Init from frame 0 (like v14.36)
    for (let c = 0; c < N; c++) {
      dp[c] = emitScores[0][c];
    }
    
    // Forward from frame 1
    for (let i = 1; i < M; i++) {
    ...
    console.log(`🎹 HMM: path[0]=${cur}, first chord=${candidates[cur]?.label}`);
    
    for (let i = 1; i < M; i++) {
      if (path[i] !== cur) {
        const cand = candidates[cur];
        const t = start * secPerFrame;
        console.log(`🎹 HMM: Adding ${cand.label} at t=${t.toFixed(2)}s (frame ${start})`);
        timeline.push({
          t,
          fi: start,
          label: cand.label,
    ...
    // Final
    const cand = candidates[cur];
    const finalT = start * secPerFrame;
    console.log(`🎹 HMM: Adding final ${cand.label} at t=${finalT.toFixed(2)}s`);
    timeline.push({
      t: finalT,
      fi: start,
      label: cand.label,
  ...
  // v16.28: Stricter emit score based on v14.36 lessons
  calcEmitScore(frame, bassNote, energy, energyP70, candidate) {
    if (!frame) return -Infinity;
    
    // Calculate chord template match (like v14.36)
    let templateScore = 0;
    let templateNorm = 0;
    ...
    const matchScore = templateScore / (templateNorm * frameNorm);
    
    // v16.28: Threshold - but not too strict for acoustic intros
    if (matchScore < 0.20) return -Infinity;
    ...
    // v16.28: Low energy penalty (but not too harsh for quiet intros)
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.25: MERGE ENGINES - Compare and choose best
  // ═══════════════════════════════════════════════════════════
