...
<head>
  ...
  <title>ChordFinderPro v16.15 🎵 Ultimate Precision</title>
  
  <!-- PWA Meta Tags -->
  <meta name="description" content="🎵 זיהוי אקורדים מתקדם עם AI - תמיכה ב-YouTube, inversions, extensions, וסנכרון lyrics" />
  ...
  <div class="wrap">
    ...
    <section class="panel">
      <div class="tabs">
        ...
        <div class="tab" data-tab="report">📊 דוח</div>
      </div>
      ...
      <div id="reportTab" class="tab-content">
        <h3>📊 דוח השוואת אקורדים</h3>
        <div class="info-box">
          השווה את תוצאות המנוע לאקורדים נכונים (Ground Truth)
        </div>
        
        <!-- חיפוש במאגרים -->
        <div style="margin-top:15px;padding:12px;background:#0a1324;border-radius:8px;border:1px solid #2a3d55;">
          <label style="font-weight:700;color:#38bdf8;">🔍 חיפוש במאגרים אקדמיים</label>
          <p style="font-size:12px;color:#94a3b8;margin:4px 0 8px;">Beatles, Queen, Carole King מ-Isophonics Dataset</p>
          <div class="row">
            <input id="songSearchInput" type="text" placeholder="שם השיר באנגלית (למשל: Yesterday)" style="flex:1" />
            <button id="btnSearchChords">🔍 חפש</button>
          </div>
        ...
        <div style="margin-top:12px;padding:12px;background:#0a1324;border-radius:8px;border:1px solid #2a3d55;">
          <label style="font-weight:700;color:#f472b6;">🖼️ חילוץ אקורדים מתמונה</label>
          <p style="font-size:12px;color:#94a3b8;margin:4px 0 8px;">
            העלה צילום מסך של דף אקורדים
          </p>
        ...
        <!-- הזנה ידנית -->
        <div style="margin-top:12px">
          <label style="display:block;margin-bottom:4px;font-weight:700;color:#22c55e;">✏️ אקורדים נכונים (Ground Truth)</label>
          <p style="font-size:12px;color:#94a3b8;margin:0 0 8px;">האקורדים יופיעו כאן אחרי חיפוש/העלאה, או הדבק ידנית</p>
          <textarea id="truthChordsInput" 
                    placeholder="פורמטים נתמכים:
...
5.00,F"
        ...
        <!-- כפתורי פעולה -->
        <div class="row" style="margin-top:12px">
          <button id="btnGenerateReport" style="background:linear-gradient(135deg,#22c55e,#16a34a);">📊 השווה עכשיו</button>
          <button id="btnExportCsv" style="background:#1f2a40;">💾 הורד CSV של המנוע</button>
        </div>
        
        <p style="font-size:11px;color:#64748b;margin-top:8px;">
          💡 <b>טיפ:</b> ההשוואה בודקת התאמת רצף האקורדים (בלי זמנים). מתאים ל-Ultimate Guitar, תמונות, או כל מקור אחר.
        </p>
        
        <!-- דוח תוצאות -->
        <pre id="reportOutput" class="report-output" style="margin-top:15px;">ממתין לניתוח שיר ואקורדים להשוואה...</pre>
      </div>
      ...
      <div class="row" style="margin-top:8px">
        ...
        <button onclick="refreshKeyDisplay()" style="padding:4px 10px;font-size:12px" title="רענן סולם">🔄</button>
        ...
        <span class="badge" id="profileBadge" style="display:none">פרופיל: <b id="profileValue">—</b></span>
      </div>
    ...
    <section class="panel">
      ...
      <div class="analysis-grid">
        <div class="analysis-card">
          <h3>סה"כ אקורדים</h3>
          <div class="value" id="totalChordsCount">—</div>
        </div>
        <div class="analysis-card">
          <h3>בסולם</h3>
          <div class="value" id="inScaleCount">—</div>
        </div>
        <div class="analysis-card">
          <h3>Borrowed</h3>
          <div class="value" id="borrowedCount">—</div>
        </div>
        <div class="analysis-card">
          <h3>Inversions</h3>
          <div class="value" id="inversionsCount">—</div>
        </div>
        <div class="analysis-card">
          <h3>Extensions</h3>
          <div class="value" id="extensionsCount">—</div>
        </div>
      </div>
...
<script src="ChordEngineUltimate_v16.25.js"></script>
...
window.addEventListener('DOMContentLoaded', () => {
  if (typeof ChordEngineUltimate === 'undefined') {
    alert('❌ ChordEngineUltimate_v16.25.js לא נטען!');
    throw new Error('Engine not loaded');
  }
  try {
    engine = new ChordEngineUltimate();
    console.log('✅ Engine v16.15 with AI Profiles initialized');
  } catch(e) {
    alert('❌ שגיאה ביצירת Engine: ' + e.message);
    throw e;
  }
});
...
// ═══════════════════════════════════════════════════════════
// 📊 דוח השוואה - חיפוש אקורדים ממאגרים
// ═══════════════════════════════════════════════════════════

// מאגר Isophonics - Beatles, Queen, Carole King
const ISOPHONICS_BASE = 'https://raw.githubusercontent.com/tmc323/Isophonics/master';

// פונקציה לחיפוש אקורדים
async function searchChords(songName) {
  ...
  searchStatus.innerHTML = '<span class="spin"></span> מחפש...';
  
  try {
    // חיפוש ב-GitHub Isophonics
    ...
    if (searchResponse.ok) {
      ...
      if (data.items && data.items.length > 0) {
        ...
        if (fileResponse.ok) {
          ...
          searchStatus.textContent = `✅ נמצא! מאגר Isophonics (${firstResult.name})`;
          return;
        }
      }
    }
    ...
    searchStatus.className = 'pill warn';
    searchStatus.textContent = '⚠️ לא נמצא. הדבק ידנית או חפש ב-Ultimate Guitar';
    
  } catch (err) {
    console.error('Search error:', err);
    searchStatus.className = 'pill err';
    searchStatus.textContent = '❌ שגיאה: ' + err.message;
  }
}

// פרסור קובץ .lab (פורמט Isophonics)
...
document.getElementById('btnSearchChords').addEventListener('click', () => {
  ...
  if (!songName) { alert('הזן שם שיר'); return; }
...
document.getElementById('btnExportCsv').addEventListener('click', () => {
  if (!window.__lastChordResult) {
    alert('אין תוצאה אחרונה של זיהוי. תריץ שיר קודם.');
    return;
  }
  if (typeof ChordEvalSimple === 'undefined') {
    alert('ChordEvalSimple.js לא נטען!');
    return;
  }
...
document.getElementById('btnGenerateReport').addEventListener('click', () => {
  ...
  if (!window.__lastChordResult || !window.__lastChordResult.chords) {
    alert('תריץ ניתוח קודם.');
    return;
  }
  if (!truthText.trim()) {
    alert('הדבק או חפש אקורדים נכונים קודם.');
    return;
  }
  ...
  if (truthChords.length === 0) {
    alert('לא הצלחתי לפרסר את האקורדים.');
    return;
  }
  ...
  let report = `🎼 דו"ח השוואת אקורדים\n`;
  report += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  report += `מנוע: ${engineChords.length} אקורדים | GT: ${truthChords.length}\n`;
  report += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  
  report += `אקורדים במנוע: ${engineUnique.join(', ')}\n`;
  report += `אקורדים ב-GT: ${truthUnique.join(', ')}\n\n`;
  
  if (missingInEngine.length > 0) report += `❌ חסרים: ${missingInEngine.join(', ')}\n`;
  if (extraInEngine.length > 0) report += `⚠️ מיותרים: ${extraInEngine.join(', ')}\n`;
  ...
  for (let i = 0; i < maxCompare; i++) {
    const eng = engineLabels[i] || '—';
    const tru = truthLabels[i] || '—';
    const match = eng === tru;
    if (match) matches++;
    report += `${(i+1).toString().padStart(2)}. ${match ? '✅' : '❌'} ${eng.padEnd(6)} | ${tru}\n`;
  }
  
  const accuracy = maxCompare > 0 ? (matches / maxCompare * 100).toFixed(1) : 0;
  report += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  report += `🎯 דיוק: ${matches}/${maxCompare} (${accuracy}%)\n`;
...
async function correctHebrewWords(words, fullText) {
  ...
  if (corrections > 0) console.log(`✅ Fixed ${corrections} Hebrew words`);
  return corrected;
}
...
analyzeBtn.onclick=async()=>{
  ...
  totalChordsCountEl.textContent='—'; 
  inScaleCountEl.textContent='—'; 
  borrowedCountEl.textContent='—'; 
  inversionsCountEl.textContent='—';
  extensionsCountEl.textContent='—';
  ...
  try{
    ...
    showAnalyzing('שלב 2/3: זיהוי אקורדים (v16.15 AI Profiles)');
    ...
    if (!engine || !engine.NOTES_SHARP) {
      ...
      err('שגיאה: Engine לא הוגדר נכון');
      return;
    }
    ...
    ok(`✅ v16.15 AI [${detectedProfile.toUpperCase()}] - ${timeline.length} אקורדים`);
    hookPlaybackLive(); 
    refreshSheetTabView();
...
function buildFullLyricsSheet(isRTL) {
  ...
  if (isHebrew) {
    ...
    html += '<div style="color:#38bdf8;font-weight:700;font-size:18px;margin-bottom:15px;text-align:center">🎸 אקורדים בלבד</div>';
