/**
 * ChordEngine v16.26 - Auto-Clean with Harmonic Memory
 * 
 * Based on v16.25 + automatic refinement:
 * - Detects song complexity (simple/medium/complex) from stats
 * - For simple songs: removes invented passing chords
 * - Criteria: non-diatonic + short + single occurrence + sandwiched
 * - No manual modes - everything is automatic
 */
...
class ChordEngineUltimate {
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    const MIN_DURATION_SEC = 0.6;
    const MIN_DURATION_NO_BASS_CHANGE = 1.2; // v16.25: If bass doesn't change, need longer duration
    const CHROMA_CHANGE_THRESHOLD = 0.35;
    const SIGNIFICANT_THIRD_FIFTH_CHANGE = 0.25; // v16.25: Threshold for 3rd/5th change
    ...
    console.log(`🎵 v16.25: ${changeFrames.length} chord change candidates`);
  ...
  finalizeTimeline(timeline, bpm, features) {
    ...
    const spb = 60 / Math.max(60, Math.min(200, bpm));
    const minDuration = 0.5 * spb; // v16.25: Longer minimum (was 0.4)
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      const isStrong = ev.confidence >= 90; // v16.25: Higher threshold (was 85)
