/**
 * ChordEngine v16.26 - Stricter Filtering (v14.36 lessons)
 * 
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // 🧠 v16.26: Auto-clean invented passing chords using harmonic memory
  ...
  // v16.26: Fixed - accept clear pitch even without bass (for acoustic intros)
  findMusicStart(features) {
    ...
    for (let i = 0; i < energy.length; i++) {
      ...
      // v16.26: Check for clear musical pitch in chroma (for guitar/piano intros)
    ...
    // v16.26: If still no music found, start from frame 0
    if (musicFrame === 0 && stableCount < 2) {
      console.log('⚠️ v16.26: No clear music start found, starting from 0');
    }
  ...
  // v16.26: Start from frame 0 like v14.36 (not from startFrame)
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    // v16.26: Stricter thresholds based on v14.36 lessons
    ...
    let currentBass = -1;
    let currentStart = 0;  // v16.26: Start from 0, not startFrame!
    let currentChroma = null;
    
    // v16.26: Get candidates from frame 0, not startFrame
    const changeFrames = this.getChordChangeCandidates(features, bpm, 0);
    console.log(`🎵 v16.26: ${changeFrames.length} chord change candidates (from frame 0)`);
    
    for (const i of changeFrames) {
      if (i >= bass.length) continue;
      
      const bp = bass[i];
      const hasEnergy = energy[i] >= energyP70 * 0.4; // v16.26: Stricter (was 0.3)
      ...
      if (currentBass === -1) {
        ...
        // v16.26: Add FIRST chord RIGHT NOW (don't wait for change)
  ...
  // v16.26: Stricter finalization based on v14.36
  finalizeTimeline(timeline, bpm, features) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.26: ALWAYS KEEP FIRST CHORD (index 0) - critical for song start!
      if (i === 0) {
        filtered.push(ev);
        continue;
      }
  ...
  // ═══════════════════════════════════════════════════════════
  // 🧠 v16.26: AUTO REFINEMENT – זיכרון הרמוני נגד אקורדים מומצאים
  // ═══════════════════════════════════════════════════════════
  ...
  autoRefineTimeline(timeline, key, totalDuration) {
    ...
    console.log(`🧠 v16.26 AUTO-CLEAN: profile=${profile}, isSimple=${isSimple}`);
  ...
  // v16.26: Stricter emit score based on v14.36 lessons
  calcEmitScore(frame, bassNote, energy, energyP70, candidate) {
    ...
    // v16.26: Threshold - but not too strict for acoustic intros
    ...
    // v16.26: Low energy penalty (but not too harsh for quiet intros)
