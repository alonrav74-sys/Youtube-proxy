// =====================================================
// 🎵 SyncEngine v6.3 - Improved RTL Positioning
// =====================================================
  ...
  refreshSheetTabView: function(state, whisperWords, whisperText, detectedLanguage, capo, sanitizeLabel, applyCapoToLabel, fullSheetEl) {
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
      ...
      if (lineChords.length > 0) {
        if (isRTL) {
          // 🔧 RTL: Position chords above words (right to left)
          const chordPositions = [];
          
          // Calculate total line length for RTL positioning
          ...
          for (const chord of lineChords) {
            ...
            for (let i = 0; i < line.length; i++) {
              ...
              if (chordTime >= word.time && chordTime < word.end) {
                // Chord DURING word
                // In RTL, calculate position from the RIGHT
                const afterText = line.slice(i + 1).map(w => w.text).join(' ');
                const afterLength = afterText.length + (afterText.length > 0 ? 1 : 0);
                charPos = lineLength - afterLength - word.text.length;
                foundWord = true;
                break;
              }
            }
            ...
            if (!foundWord) {
              // In RTL, this means position 0 (far right)
              charPos = 0;
            }
          ...
          // Sort by position (right to left in RTL)
      ...
      if (chordLine.trim()) {
        // Use unicode-bidi: isolate to prevent chord labels from flipping
        html += `<div style="color:#38bdf8;font-weight:700;white-space:pre;unicode-bidi:isolate">${this.escapeHtml(chordLine)}</div>`;
      }
