...
class ChordEngineEnhanced {
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    // ✅ CRITICAL FIX: Mark which are TRULY diatonic (root + quality!)
    // In C major: Am is diatonic, but A is NOT!
    ...
    for (const cand of candidates) {
      ...
      if (rootIsDiatonic) {
        // Root is in scale, but is the QUALITY correct?
        ...
        if (idx >= 0) {
          ...
          // ONLY mark as diatonic if quality matches!
          cand.diatonic = (actualIsMinor === expectedIsMinor);
        } else {
          cand.diatonic = false;
        }
      } else {
    ...
    const emitScore = (i, cand) => {
      ...
      // ✅ CRITICAL FIX: Diatonic bonus (now cand.diatonic is correct!)
      // In C major: Am gets +0.30, A gets nothing!
      if (cand.diatonic) {
        score += 0.30; // Strong preference for in-key chords with correct quality
      }
