...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    let tonic;
    if (humanEarResult && humanEarResult.confidence >= 50) {
      tonic = humanEarResult;
      // Also update key if human ear is confident
      if (humanEarResult.confidence >= 65 && 
          (humanEarResult.root !== key.root || humanEarResult.minor !== key.minor)) {
        key = { 
          root: humanEarResult.root, 
          minor: humanEarResult.minor, 
          confidence: humanEarResult.confidence / 100 
        };
        ...
        timeline = this.finalizeTimeline(timeline, key, audioData.bpm, feats);
        timeline = this.applyPostProcessing(timeline, key, feats, audioData.bpm, opts);
        timeline = this.harmonicRefinement(timeline, key); // Refine again
        timings.rerunHumanEar = now() - tRe2;
      }
    } else {
      // Fallback to old method
      tonic = this.detectTonicMusically(timeline, key, audioData.duration, feats);
  ...
  findCycleOfLength(roots, len) {
    ...
    for (let start = 0; start < Math.min(4, roots.length - len * 2); start++) {
      const pattern = roots.slice(start, start + len).map(r => r.root);
      
      // Count how many times this pattern repeats
      let matches = 0;
      for (let i = start; i + len <= roots.length; i += len) {
      ...
      // Need at least 2 repetitions (pattern appears 3+ times total)
      if (matches >= 3) {
        return {
          cycleLength: len,
          pattern: pattern,
