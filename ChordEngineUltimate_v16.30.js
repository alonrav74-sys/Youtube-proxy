/**
 * ChordEngine v16.30 - Conservative Filtering + Auto Harmonic Memory Cleanup
 *
 * Based on v14.36 with improvements:
 * - Uses harmonic "memory" (frequency of each chord)
 * - Identifies rare, very short, non-diatonic passing chords
 * - Removes only those that are clearly noise (esp. between identical neighbors)
 */
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log('🎵 ChordEngine v16.30 (Conservative + Auto Harmonic Memory)');
    ...
    // 🧠 v16.26: Auto-refine using harmonic memory (remove invented passing chords)
  ...
  extractFeatures(audio) {
    ...
    for (let start = 0; start + win <= x.length; start += hop) {
      ...
      for (let bin = 1; bin < mags.length; bin++) {
        ...
        // Weight by harmonic clarity (octave zone)
        const octave = Math.floor(midi / 12);
        const harmonicWeight = octave >= 3 && octave <= 6 ? 1.2 :
                              octave < 3 ? 0.8 :
                              0.7;

        const magWeight = Math.sqrt(mags[bin]);
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 STABLE BASS CHORD BUILDING - Bass is King
  // ═══════════════════════════════════════════════════════════
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    const MIN_DURATION_NO_BASS_CHANGE = 1.2;
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    for (const tc of theoryCandidates) {
      const score = this.scoreChordCandidate(
        ...
        tc.inversionBass !== undefined ? tc.inversionBass : null
      );
      if (score > 0) {
        candidates.push({
          ...tc,
    ...
    if (best.score < 45 && durSec < 0.40) return null;
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.26: PATTERN MEMORY & AUTO REFINEMENT
  // ═══════════════════════════════════════════════════════════
  ...
  // 🧠 NEW: Auto refinement using harmonic memory
