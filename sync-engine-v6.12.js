// =====================================================
// 🎵 SyncEngine v6.12 - Fix Intro (like English!)
// =====================================================
// 
// Intro detection: Chords BEFORE first lyric time
// (not first lyric index in cells array!)
...
const SyncEngine = {
  ...
  buildHebrewTableSheet: function(state, whisperWords, whisperText, capo, sanitizeLabel, applyCapoToLabel, fullSheetEl) {
    ...
    // 🔧 v6.12: Find first LYRIC TIME (not index!)
    ...
    for (const w of whisperWords) {
      ...
      if (text) {
        ...
        if (text) {
          firstLyricTime = Math.min(firstLyricTime, w.start + gateOffset);
        }
      }
    }
    ...
    // 🔧 v6.12: Intro = chords BEFORE firstLyricTime (like English!)
    const introChords = state.timeline.filter(ch => {
      const chordTime = ch.t + gateOffset;
      return chordTime < firstLyricTime;
    }).map(ch => sanitizeLabel(applyCapoToLabel(ch.label, capoVal)));
    
    // Main cells = everything from firstLyricTime onwards
    ...
    if (introChords.length > 0) {
      ...
      for (let i = 0; i < introChords.length; i++) {
        if (i > 0) html += '&nbsp;&nbsp;&nbsp;&nbsp;';
        html += `<span style="font-size:16px">${this.escapeHtml(introChords[i])}</span>`;
      }
      html += '</div></div>';
    }
