...
class ChordEngineUltimate {
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 v16.19: DUAL ENGINE - Bass tracking + HMM with Consensus
  // ═══════════════════════════════════════════════════════════
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    // Run both engines
    const bassTimeline = this.trackByBass(features, key, startFrame, diatonic);
    const hmmTimeline = this.trackByHMM(features, key, startFrame, diatonic);
    
    console.log(`🎸 Bass engine: ${bassTimeline.length} chords`);
    console.log(`🎹 HMM engine: ${hmmTimeline.length} chords`);
    
    // Merge with consensus
    const merged = this.mergeWithConsensus(bassTimeline, hmmTimeline, features, key, diatonic);
    console.log(`🤝 Consensus: ${merged.length} chords`);
    
    return merged;
  }
  
  // ─────────────────────────────────────────────────────────────
  // Engine 1: Bass-based tracking
  // ─────────────────────────────────────────────────────────────
  trackByBass(features, key, startFrame, diatonic) {
    ...
    for (let i = startFrame; i < bass.length; i++) {
      const bp = bass[i];
      const hasEnergy = energy[i] >= energyP70 * 0.25;
      
      if (!hasEnergy) continue;
      
      if (bp >= 0 && bp !== currentBass) {
        const durSec = (i - currentStart) * secPerFrame;
        
        if (currentBass >= 0 && durSec >= MIN_DURATION_SEC) {
          const chord = this.scoreChordAtSegment(chroma, currentStart, i, currentBass, key, diatonic);
          if (chord && chord.score > 40) {
            timeline.push({
    ...
    // Final
    if (currentBass >= 0) {
      const durSec = (chroma.length - currentStart) * secPerFrame;
      if (durSec >= MIN_DURATION_SEC) {
        const chord = this.scoreChordAtSegment(chroma, currentStart, chroma.length, currentBass, key, diatonic);
        if (chord && chord.score > 40) {
          timeline.push({
            t: currentStart * secPerFrame,
            fi: currentStart,
  ...
  // ─────────────────────────────────────────────────────────────
  // Engine 2: HMM-based tracking (from v14.36)
  // ─────────────────────────────────────────────────────────────
  trackByHMM(features, key, startFrame, diatonic) {
    const { chroma, bass, energy, energyP70, secPerFrame } = features;
    
    // Build candidates (diatonic only)
    const candidates = [];
    for (const dc of diatonic.chords) {
      const noteName = this.getNoteName(dc.root, key);
      candidates.push({ 
        root: dc.root, 
        label: noteName + (dc.minor ? 'm' : ''), 
    ...
    // Add V in minor (harmonic minor)
    if (key.minor) {
      const V = this.toPc(key.root + 7);
      if (!candidates.find(c => c.root === V && c.type === 'major')) {
        candidates.push({ root: V, label: this.getNoteName(V, key), type: 'major', borrowed: true, intervals: [0, 4, 7] });
      }
    }
    
    // Templates
    const templates = candidates.map(cand => {
      const mask = new Float32Array(12);
      for (const iv of cand.intervals) mask[this.toPc(cand.root + iv)] = 1;
      const sum = mask.reduce((a, b) => a + b, 0);
      for (let p = 0; p < 12; p++) mask[p] /= sum;
      return { ...cand, mask };
    });
    
    const lowE = energyP70 * 0.25;
    const N = candidates.length;
    const M = chroma.length;
    
    // Emit score
    const emitScore = (i, candIdx) => {
      if (i < startFrame || !chroma[i]) return -Infinity;
      ...
      for (let p = 0; p < 12; p++) {
        dot += c[p] * tmpl.mask[p];
        normC += c[p] * c[p];
        normT += tmpl.mask[p] * tmpl.mask[p];
      }
      let score = dot / (Math.sqrt(normC) * Math.sqrt(normT) + 0.001);
      
      if (score < 0.35) return -Infinity;
      
      if (!tmpl.borrowed) score += 0.20;
      else score -= 0.15;
      
      if (bass[i] >= 0 && bass[i] === tmpl.root) score += 0.20;
    ...
    // Transition cost
    const transitionCost = (aIdx, bIdx) => {
      if (aIdx === bIdx) return 0.0;
      const a = candidates[aIdx], b = candidates[bIdx];
      
      const circle = [0, 7, 2, 9, 4, 11, 6, 1, 8, 3, 10, 5];
      let dist = Math.abs(circle.indexOf(a.root) - circle.indexOf(b.root));
      if (dist > 6) dist = 12 - dist;
      
      let cost = 0.25 + 0.05 * dist;
      if (a.borrowed || b.borrowed) cost += 0.15;
      
      const iv = this.toPc(b.root - a.root);
      if (iv === 7 || iv === 5) cost -= 0.10; // V-I or IV-I
    ...
    for (let s = 0; s < N; s++) dp[s] = emitScore(startFrame, s);
    
    for (let i = startFrame + 1; i < M; i++) {
      const newdp = new Array(N).fill(-Infinity);
      const beam = dp.map((sc, idx) => ({ sc, idx }))
        .filter(x => x.sc > -Infinity)
        .sort((a, b) => b.sc - a.sc)
        .slice(0, BEAM);
      
      if (!beam.length) { dp = newdp; continue; }
      
      for (let s = 0; s < N; s++) {
        let best = -Infinity, bestJ = -1;
        for (const { sc, idx: j } of beam) {
          const val = sc - transitionCost(j, s);
          if (val > best) { best = val; bestJ = j; }
        }
        newdp[s] = best + emitScore(i, s);
        backptr[i][s] = bestJ;
      }
      dp = newdp;
    }
    ...
    // Backtrack
    let bestS = dp.indexOf(Math.max(...dp));
    if (bestS < 0 || dp[bestS] === -Infinity) return [];
    
    const states = new Array(M).fill(bestS);
    for (let i = M - 1; i > startFrame; i--) {
      states[i - 1] = backptr[i][states[i]] >= 0 ? backptr[i][states[i]] : states[i];
    }
    
    // Convert to timeline
    const timeline = [];
    let cur = states[startFrame], start = startFrame;
    
    for (let i = startFrame + 1; i < M; i++) {
      if (states[i] !== cur) {
        const cand = candidates[cur];
        const durSec = (i - start) * secPerFrame;
        if (durSec >= 0.4 && cand) {
          // Calculate score for this segment
          let segScore = 0, count = 0;
          for (let j = start; j < i; j++) {
            const es = emitScore(j, cur);
            if (es > -Infinity) { segScore += es; count++; }
          }
          segScore = count > 0 ? (segScore / count) * 100 : 50;
          ...
          timeline.push({
            ...
            score: segScore,
            confidence: Math.round(segScore),
    ...
    // Final
    if (cur >= 0 && cur < candidates.length) {
      const cand = candidates[cur];
      const durSec = (M - start) * secPerFrame;
      if (durSec >= 0.4) {
        timeline.push({
          t: start * secPerFrame,
          fi: start,
  ...
  // ─────────────────────────────────────────────────────────────
  // Consensus: Merge Bass + HMM results
  // ─────────────────────────────────────────────────────────────
  mergeWithConsensus(bassTimeline, hmmTimeline, features, key, diatonic) {
    const { secPerFrame, chroma } = features;
    const merged = [];
    
    // Create time segments from both timelines
    const allTimes = new Set();
    for (const ev of bassTimeline) allTimes.add(ev.t);
    for (const ev of hmmTimeline) allTimes.add(ev.t);
    const sortedTimes = [...allTimes].sort((a, b) => a - b);
    
    // For each time point, find what each engine says
    for (let i = 0; i < sortedTimes.length; i++) {
      const t = sortedTimes[i];
      const nextT = sortedTimes[i + 1] || Infinity;
      
      const bassChord = bassTimeline.find(ev => ev.t <= t && (ev.t + 0.01 >= t || !bassTimeline.find(e2 => e2.t > ev.t && e2.t <= t)));
      const hmmChord = hmmTimeline.find(ev => ev.t <= t && (ev.t + 0.01 >= t || !hmmTimeline.find(e2 => e2.t > ev.t && e2.t <= t)));
      
      // Find active chord at this time
      const activeBass = bassTimeline.filter(ev => ev.t <= t).pop();
      const activeHMM = hmmTimeline.filter(ev => ev.t <= t).pop();
      
      if (!activeBass && !activeHMM) continue;
      
      let chosen = null;
      let consensusType = '';
      
      if (activeBass && activeHMM) {
        if (activeBass.root === activeHMM.root) {
          // 🎯 AGREEMENT! Both engines agree on root
          consensusType = 'agree';
          chosen = activeBass.score >= activeHMM.score ? activeBass : activeHMM;
          chosen.confidence = Math.round((activeBass.score + activeHMM.score) / 2);
        } else {
          // Disagreement - check scores
          const bassScore = activeBass.score || 50;
          const hmmScore = activeHMM.score || 50;
          
          // Prefer diatonic
          const bassIsDiatonic = diatonic.pcs.includes(activeBass.root);
          const hmmIsDiatonic = diatonic.pcs.includes(activeHMM.root);
          
          if (bassIsDiatonic && !hmmIsDiatonic) {
            chosen = activeBass;
            consensusType = 'bass_diatonic';
          } else if (hmmIsDiatonic && !bassIsDiatonic) {
            chosen = activeHMM;
            consensusType = 'hmm_diatonic';
          } else if (bassScore > hmmScore + 10) {
            chosen = activeBass;
            consensusType = 'bass_stronger';
          } else if (hmmScore > bassScore + 10) {
            chosen = activeHMM;
            consensusType = 'hmm_stronger';
          } else {
            // Close call - prefer HMM (more stable)
            chosen = activeHMM;
            consensusType = 'hmm_tiebreak';
          }
        }
      } else {
        chosen = activeBass || activeHMM;
        consensusType = activeBass ? 'bass_only' : 'hmm_only';
      }
      
      if (chosen) {
        // Check if this is a new chord or same as last
        const last = merged[merged.length - 1];
        if (!last || last.label !== chosen.label) {
          merged.push({
            t: t,
            fi: chosen.fi,
            label: chosen.label,
            root: chosen.root,
            type: chosen.type,
            inScale: chosen.inScale,
            confidence: chosen.confidence || 70,
  ...
  // ─────────────────────────────────────────────────────────────
  // Helper: Score chord at a segment
  // ─────────────────────────────────────────────────────────────
  scoreChordAtSegment(chroma, startFi, endFi, bassNote, key, diatonic) {
    const count = endFi - startFi;
    if (count <= 0) return null;
    
    const avg = new Float32Array(12);
    for (let i = startFi; i < endFi && i < chroma.length; i++) {
    ...
    for (const dc of diatonic.chords) {
      const intervals = dc.minor ? [0, 3, 7] : [0, 4, 7];
      let score = 0;
      for (const iv of intervals) {
        score += avg[this.toPc(dc.root + iv)] * 30;
      }
      
      // Bonus if bass matches
      if (bassNote === dc.root) score += 25;
      else if (bassNote === this.toPc(dc.root + (dc.minor ? 3 : 4))) score += 15; // third
      else if (bassNote === this.toPc(dc.root + 7)) score += 15; // fifth
      
      // Penalize wrong third
      const wrongThird = avg[this.toPc(dc.root + (dc.minor ? 4 : 3))];
      score -= wrongThird * 20;
      ...
      if (score > bestScore) {
        ...
        bestChord = {
          ...
          inScale: true,
          score: score,
          confidence: Math.round(Math.min(100, score)),
          chordType: 'diatonic'
        };
      }
    }
