/**
 * ChordEngine v16.24 - Conservative Filtering
 * 
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log('🎵 ChordEngine v16.24 (Conservative - Fewer but Solid Chords)');
    ...
    // v16.24: Run HMM engine in parallel and compare
    ...
    // v16.24: Auto-detect "easy rock" songs and clean up
    const stats = this.buildStats(timeline, key);
    const isEasy = this.isEasyRockSong(stats, key);
    
    if (isEasy) {
      console.log(`🎸 v16.24: Easy rock song detected - enforcing diatonic`);
      timeline = this.enforceEasyDiatonic(timeline, key);
      
      // v16.24: Apply pattern memory for easy rock songs
      timeline = this.applyPatternMemory(timeline, key);
    }
    
    // v16.24: Always run outlier smoothing (removes isolated chromatic chords)
  ...
  processAudio(audioBuffer) {
    ...
    // v16.24: Clean signal before processing
  ...
  // v16.24: Clean signal - pre-emphasis + noise gate + smoothing
  cleanSignal(x, sr) {
  ...
  extractFeatures(audio) {
    ...
    for (let start = 0; start + win <= x.length; start += hop) {
      ...
      // v16.24: Improved chroma with harmonic weighting
      ...
      for (let bin = 1; bin < mags.length; bin++) {
        ...
        // v16.24: Weight by harmonic clarity
    ...
    // v16.24: Compute spectral flux for chord change detection
    ...
    return {
      ...
      flux, fluxP80, // v16.24: Added flux
  ...
  // v16.24: Spectral flux - detects changes in chroma
  ...
  // v16.24: Find candidate frames for chord changes
  ...
  detectBassNote(mags, sr, N) {
    const fmin = 40, fmax = 300; // v16.24: Extended range (was 250)
    ...
    for (let bin = 1; bin < mags.length; bin++) {
      ...
      if (freq >= fmin) {
        ...
        const weight = freq < 150 ? 1.5 : 1.0; // v16.24: Boost very low freqs
    ...
    // v16.24: Stricter threshold (was 0.25)
  ...
  // v16.24: getFirstStrongChord and detectTriadFromChroma removed - not used in new tonic detection
  ...
  // v16.24: detectModeHybrid removed - using detectModeFromThird directly
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 v16.24: STABLE BASS CHORD BUILDING with Beat-Aware Detection
  // ═══════════════════════════════════════════════════════════
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    // v16.24: Get chord change candidate frames (beat-aligned + flux + bass changes)
    const changeFrames = this.getChordChangeCandidates(features, bpm, startFrame);
    console.log(`🎵 v16.24: ${changeFrames.length} chord change candidates (beat-aligned)`);
  ...
  getBorrowedChords(key) {
    ...
    if (!key.minor) {
      ...
      // v16.24: REMOVED bVI (Ab/G#) - causes confusion with E
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    for (const tc of theoryCandidates) {
      // v16.24: Pass inversionBass to scoreChordCandidate
        ...
        if (score > 60) { // v16.24: Higher threshold (was 40)
          ...
          if (isReasonable || score > 80) { // Only if reasonable OR super strong
            candidates.push({
              ...
              inversionBass: null,
              chordType: 'chromatic',
              score: score - 30, // v16.24: Bigger penalty (was -20)
              priority: 20        // v16.24: Lower priority (was 30)
            });
          }
        }
      }
    ...
    if (best.score < 45 && durSec < 0.40) return null; // v16.24: Stricter threshold
    
    // v16.24: Enharmonic substitution for unlikely chromatics
    ...
    // v16.24: AUTO-CORRECT quality based on actual third (use finalRoot!)
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale, inversionBass = null) {
    ...
    // v16.24: AUTO-DETECT which third is actually present!
    ...
    else if (bassNote === actualThird) score += 10; // v16.24: Slightly lower for first inversion
    else if (bassNote === fifth) score += 8;        // v16.24: Slightly lower for second inversion
    
    // v16.24: Extra validation for inversions - the bass note must be STRONG in the audio
  ...
  // v16.24: Check if chromatic chord makes harmonic sense
  isReasonableChromaticChord(root, key, prevChord) {
    ...
    if (!key.minor) {
      ...
      // v16.24: REMOVED bVI (Ab/G#) - too rare and often confused with E
  ...
  validateWithCircleOfFifths(timeline, key, features) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.24: STRICT chromatic filtering
  ...
  addExtensions(timeline, features, key, opts) {
    ...
    return timeline.map(ev => {
      ...
      // v16.24: Check all chord tones properly
      ...
      // 3. Check for 7th chords - v16.24: More precise detection
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
        ...
        if (!isMinor && M7 > 0.10 && M7 > b7 * 1.5 && M3 > 0.08) {
          // v16.24: Also check that M7 is significant relative to root
          if (M7 > r * 0.15) {
        ...
        // Dominant 7th (C7, G7) - only for major chords with clear b7
          ...
          // v16.24: b7 must be prominent enough
          if (b7 > r * 0.12) {
            label += '7';
          }
        }
      ...
      // 4. Check for sus chords - v16.24: More precise
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
      ...
      // 5. Add 6 if present (C6, Am6) - v16.24: Stricter to avoid melodic 6ths
  ...
  finalizeTimeline(timeline, bpm, features) {
    ...
    const spb = 60 / Math.max(60, Math.min(200, bpm));
    const minDuration = 0.5 * spb; // v16.24: Longer minimum (was 0.4)
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      const isStrong = ev.confidence >= 90; // v16.24: Higher threshold (was 85)
  ...
  // v16.24: Detect if song is "easy rock" (mostly diatonic)
  ...
  // v16.24: Force non-diatonic chords to nearest diatonic
  ...
  // v16.24: Remove isolated chromatic chords between identical neighbors
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.24: PATTERN MEMORY - Learn and apply recurring progressions
  // ═══════════════════════════════════════════════════════════
  ...
  // v16.24: mixStereo removed - stereo mixing is in processAudio
  // v16.24: resampleLinear removed - use resample directly
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.24: HMM ENGINE (parallel to Bass engine)
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.24: MERGE ENGINES - Compare and choose best
  // ═══════════════════════════════════════════════════════════
