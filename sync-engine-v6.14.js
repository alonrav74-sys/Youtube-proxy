...
const SyncEngine = {
  ...
  buildEnglishSheet: function(state, whisperWords, whisperText, capo, sanitizeLabel, applyCapoToLabel, fullSheetEl, isHebrewOverride) {
    ...
    if (lines.length > 0) {
      ...
      // Skip first chord if:
      // 1. More than 2 seconds before first lyric, OR
      // 2. Within first 2 seconds of song (likely noise)
      const firstChordTime = (state.timeline[0] ? state.timeline[0].t + gateOffset : 0);
      const skipFirstChord = (firstChordTime < firstLineStart - 2.0) || (firstChordTime < 2.0);
      
      const introChords = state.timeline.filter((ch, idx) => {
        const chordTime = ch.t + gateOffset;
        
        // Skip first chord if it's noise
