...
class ChordEngineEnhanced {
  constructor() {
    console.log('🎸 ChordEngine v14.22 - KEY FIX WITH PROPER FLAG!');
  ...
  async detect(audioBuffer, options = {}) {
    ...
    const originalKey = { root: key.root, minor: key.minor }; // Save original before validation
    const validatedKey = this.validateKeyFromChords(timeline, key, feats);
    
    let wasKeyCorrected = false;
    
    if (validatedKey.root !== key.root || validatedKey.minor !== key.minor) {
      console.log(`🔄 KEY CHANGED: ${this.NOTES_SHARP[key.root]}${key.minor?'m':''} → ${this.NOTES_SHARP[validatedKey.root]}${validatedKey.minor?'m':''}`);
      key = validatedKey;
      wasKeyCorrected = true;
      
      // 🆕 v14.16: Update UI with corrected key!
      if (opts.progressCallback) {
        opts.progressCallback({ 
          stage: 'key_corrected', 
          progress: 0.75,
    ...
    // 🆕 v14.21: Don't override key if it was already corrected by validateKeyFromChords
    console.log(`🔍 Was key corrected? ${wasKeyCorrected}, Tonic says: ${this.NOTES_SHARP[tonic.root]}, Key is: ${this.NOTES_SHARP[key.root]}`);
    
    if (!wasKeyCorrected && tonic.root !== key.root && tonic.confidence >= tonicThreshold) {
      t0 = performance.now();
      key.root = tonic.root;
    ...
    console.log(`🎯 FINAL KEY: ${this.NOTES_SHARP[key.root]}${key.minor ? 'm' : ''}`);
  ...
  extractFeatures(audioData) {
    ...
    const chromaSum = [], chromaPeak = []; // 🆕 v14.12: Track harmonic content
    ...
    for (let i = 0; i < frames.length; i++) {
      ...
      // 🆕 Store PRE-NORMALIZED sum (harmonic content indicator)
      ...
      chromaPeak.push(maxVal); // 🆕 How clear is the dominant pitch
    ...
    return { chroma, bassPc, frameE, hop, sr, chromaSum, chromaPeak };
  }
  ...
  // 🆕 v14.15: Smart intro skip - check both energy AND harmonic content
  computeDynamicIntroSkip(frameE, hop, sr, chromaSum = null) {
    const thr = this.percentile(frameE, 75);
    let stable = 0, i = 0;
    let maxEnergy = Math.max(...frameE);
    
    // 🆕 v14.15: Calculate harmonic threshold if chromaSum available
    let harmonicThr = 0;
    if (chromaSum && chromaSum.length > 0) {
      const validSums = chromaSum.filter(s => s > 0);
      if (validSums.length > 0) {
        validSums.sort((a, b) => a - b);
        const medianSum = validSums[Math.floor(validSums.length / 2)];
        harmonicThr = medianSum * 0.15; // 15% of median
        console.log(`🎵 Harmonic threshold: ${harmonicThr.toFixed(3)} (median: ${medianSum.toFixed(3)})`);
      }
    }
    
    for (; i < frameE.length; i++) {
      // 🆕 v14.15: Check BOTH energy and harmonic content
      const hasEnergy = frameE[i] >= thr;
      const hasHarmony = !chromaSum || chromaSum[i] >= harmonicThr;
      
      if (hasEnergy && hasHarmony) {
        stable++;
      } else {
        stable = 0;
      }
      ...
      // Require stability
      const requiredStability = (i < 50 && frameE[i] < maxEnergy * 0.5) ? 1.0 : 0.5;
      
      if (stable * hop / sr >= requiredStability) break;
    }
    
    // Minimum skip - always skip first 1 second if energy is low OR no harmony
    const minSkipSec = 1.0;
    const minSkipFrames = Math.floor(minSkipSec * sr / hop);
    
    // Check if first second is weak OR has no harmonic content
    const firstSecFrames = Math.min(minSkipFrames, frameE.length);
    const avgFirstSec = frameE.slice(0, firstSecFrames).reduce((a, b) => a + b, 0) / firstSecFrames;
    const avgRest = frameE.slice(firstSecFrames).reduce((a, b) => a + b, 0) / Math.max(1, frameE.length - firstSecFrames);
    
    // 🆕 v14.15: Also check harmonic content in first second
    let firstSecHasMusic = true;
    if (chromaSum && chromaSum.length >= firstSecFrames) {
      const avgFirstHarmony = chromaSum.slice(0, firstSecFrames).reduce((a, b) => a + b, 0) / firstSecFrames;
      if (avgFirstHarmony < harmonicThr * 2) {
        firstSecHasMusic = false;
        console.log(`🥁 First second has no harmonic content (${avgFirstHarmony.toFixed(3)} < ${(harmonicThr * 2).toFixed(3)})`);
      }
    }
    
    // Skip if weak OR no music
    if (avgFirstSec < avgRest * 0.6 || !firstSecHasMusic) {
      i = Math.max(i, minSkipFrames);
      console.log(`⚠️ Skipping intro: weak=${avgFirstSec < avgRest * 0.6}, noMusic=${!firstSecHasMusic}`);
    }
    
    // Hard cap at 10 seconds
    const hardCapSec = 10.0;
    const hardCapFrames = Math.floor(hardCapSec * sr / hop);
    
    const finalSkip = Math.min(i, hardCapFrames);
    console.log(`🎸 Final intro skip: ${finalSkip} frames (${(finalSkip * hop / sr).toFixed(1)}s)`);
    
    return finalSkip;
  }

  detectTonicFromBass(feats) {
    const { bassPc, frameE, hop, sr, chromaSum } = feats;
    ...
    // 🆕 v14.15: Use chromaSum (same as chord filtering!)
    let harmonicThr = 0;
    if (chromaSum && chromaSum.length > 0) {
      const validSums = chromaSum.filter(s => s > 0);
      if (validSums.length > 0) {
        validSums.sort((a, b) => a - b);
        const medianSum = validSums[Math.floor(validSums.length / 2)];
        harmonicThr = medianSum * 0.15; // Same threshold as finalizeTimeline!
      }
    }
    
    // 🆕 v14.5: Dynamic intro skip instead of hard 15%
    const skipFrames = this.computeDynamicIntroSkip(frameE, hop, sr, feats.chromaSum);
    console.log(`🎸 Bass analysis: skip=${skipFrames}, E_thr=${threshold.toFixed(4)}, H_thr=${harmonicThr.toFixed(3)}`);
    
    for (let i = skipFrames; i < bassPc.length; i++) {
      const bp = bassPc[i];
      // 🆕 v14.15: Use chromaSum - same logic as chord filtering!
      const hasEnergy = frameE[i] >= threshold;
      const hasHarmony = !chromaSum || chromaSum[i] >= harmonicThr;
      
      if (bp >= 0 && hasEnergy && hasHarmony) {
  ...
  detectKeyEnhanced(feats, duration) {
    ...
    if (bassTonic.confidence > 0.30) {
      ...
      // 🆕 v14.15: Dynamic intro skip with harmonic check
      const skipFrames = this.computeDynamicIntroSkip(feats.frameE, feats.hop, feats.sr, feats.chromaSum);
      
      // 🔧 FIX: Only use strong frames for third analysis (ignore weak background)
      const threshold = this.percentile(feats.frameE, 80);
      
      // 🆕 v14.15: Use chromaSum (same as chord filtering!)
      let harmonicThr = 0;
      if (feats.chromaSum && feats.chromaSum.length > 0) {
        const validSums = feats.chromaSum.filter(s => s > 0);
        if (validSums.length > 0) {
          validSums.sort((a, b) => a - b);
          const medianSum = validSums[Math.floor(validSums.length / 2)];
          harmonicThr = medianSum * 0.15; // Same as finalizeTimeline!
        }
      }
      
      console.log(`🎵 Key detection: E_thr=${threshold.toFixed(4)}, H_thr=${harmonicThr.toFixed(3)}`);
      
      // Aggregate chroma from STRONG+HARMONIC frames only
      const agg = new Array(12).fill(0);
      let totalWeight = 0;
      
      for (let i = skipFrames; i < chroma.length; i++) {
        // 🆕 v14.15: Use chromaSum (consistent with chord filtering!)
        const hasEnergy = feats.frameE[i] >= threshold;
        const hasHarmony = !feats.chromaSum || feats.chromaSum[i] >= harmonicThr;
  ...
  estimateFirstChordPc(feats) {
    const { chroma, frameE, sr, hop, chromaSum } = feats;
    if (!chroma || !chroma.length) return -1;

    // 🆕 v14.15: Use dynamic skip with harmonic check
    const skipFrames = this.computeDynamicIntroSkip(frameE, hop, sr, chromaSum);
  ...
  validateKeyFromChords(timeline, currentKey, feats) {
    // 🆕 v14.9: Skip first 20% of timeline for validation (ignore noisy intro)
    if (!timeline || timeline.length < 3) {
      return currentKey;
    }
    
    const skipFirst = Math.floor(timeline.length * 0.2); // Skip first 20%
    const validationTimeline = timeline.slice(skipFirst);
    
    console.log(`🔍 Key validation: using ${validationTimeline.length}/${timeline.length} chords (skipped first 20%)`);
    
    if (validationTimeline.length < 3) {
      return currentKey; // Not enough chords after skip
    }
    
    const chordRoots = [];
    validationTimeline.forEach(chord => {
      const root = this.parseRoot(chord.label);
      if (root >= 0) {
  ...
  finalizeTimeline(timeline, key, bpm, feats) {
    ...
    // 🆕 v14.17: Filter out percussion-only frames FIRST
    console.log('🎵 Filtering percussion-only chords...');
    const preFiltered = [];
    ...
    if (feats.chromaSum && feats.chromaSum.length > 0) {
      ...
      if (validSums.length > 0) {
        validSums.sort((a, b) => a - b);
        const medianSum = validSums[Math.floor(validSums.length / 2)];
        const minHarmonicThreshold = medianSum * 0.15;
        
        console.log(`🎵 Median chromaSum: ${medianSum.toFixed(3)}, threshold: ${minHarmonicThreshold.toFixed(3)}`);
        
        for (const ev of timeline) {
          const frameIdx = ev.fi || 0;
          const chromaSum = feats.chromaSum[frameIdx] || 0;
          
          if (chromaSum < minHarmonicThreshold) {
            console.log(`🥁 Removing percussion at ${ev.t.toFixed(1)}s: ${ev.label} (chromaSum: ${chromaSum.toFixed(3)})`);
            continue;
          }
        ...
        console.log(`✅ Filtered ${timeline.length} → ${preFiltered.length} chords`);
      } else {
        preFiltered.push(...timeline);
      }
    } else {
      preFiltered.push(...timeline);
    }
    ...
    if (preFiltered.length === 0) {
      console.warn('⚠️ No chords after filtering!');
      return [];
    }
    ...
    // Apply voting window to pre-filtered timeline
    let processed = applyVotingWindow(preFiltered);
