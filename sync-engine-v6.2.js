...
const SyncEngine = {
  ...
  refreshSheetTabView: function(state, whisperWords, whisperText, detectedLanguage, capo, sanitizeLabel, applyCapoToLabel, fullSheetEl) {
    ...
    const lines = [];
    let currentLine = [];
    const maxWords = isRTL ? 5 : 6;  // Reduced to create shorter lines
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
      ...
      const lineChords = state.timeline.filter((ch, idx) => {
        ...
        // Chord is in the line if it's between start and up to 3 seconds after end
        const inLineRange = chordTime >= lineStart && chordTime <= (lineEnd + 3.0);
        
        // But check if it's closer to the next line
        if (inLineRange && lineIdx < lines.length - 1) {
          const nextLineStart = lines[lineIdx + 1][0].time;
          const distToThisLine = Math.abs(chordTime - lineEnd);
          const distToNextLine = Math.abs(chordTime - nextLineStart);
          
          // If chord is closer to next line (within 0.5s of next line start), skip it
          if (distToNextLine < 0.5 && distToNextLine < distToThisLine) {
            return false;
          }
        }
      ...
      if (lineIdx < lines.length - 1) {
        ...
        const interludeChords = state.timeline.filter((ch, idx) => {
          const chordTime = ch.t + gateOffset;
          // Interlude = chords more than 3 seconds after line end and more than 0.5s before next line
          if (chordTime > (lineEnd + 3.0) && chordTime < (nextLineStart - 0.5) && !usedChordIndices.has(idx)) {
            usedChordIndices.add(idx);
            return true;
          }
          return false;
    ...
    const outroChords = state.timeline.filter((ch, idx) => {
      const chordTime = ch.t + gateOffset;
      // Outro = chords more than 3 seconds after last line
      if (chordTime > (lastLineEnd + 3.0) && !usedChordIndices.has(idx)) {
        return true;
      }
      return false;
    });
