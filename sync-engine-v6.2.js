...
const SyncEngine = {
  ...
  refreshSheetTabView: function(state, whisperWords, whisperText, detectedLanguage, capo, sanitizeLabel, applyCapoToLabel, fullSheetEl) {
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
      ...
      // Find chords for THIS line (include chords shortly after line end)
      const lineChords = state.timeline.filter((ch, idx) => {
        const chordTime = ch.t + gateOffset;
        
        // Include chords from line start until 1 second after line end
        const threshold = 1.0;
        const inRange = chordTime >= lineStart && chordTime <= (lineEnd + threshold);
      ...
      if (lineIdx < lines.length - 1) {
        ...
        const interludeChords = state.timeline.filter((ch, idx) => {
          const chordTime = ch.t + gateOffset;
          if (chordTime > (lineEnd + 1.0) && chordTime < nextLineStart && !usedChordIndices.has(idx)) {
            usedChordIndices.add(idx);
            return true;
          }
          return false;
    ...
    const outroChords = state.timeline.filter((ch, idx) => {
      const chordTime = ch.t + gateOffset;
      if (chordTime > (lastLineEnd + 1.0) && !usedChordIndices.has(idx)) {
        return true;
      }
      return false;
    });
