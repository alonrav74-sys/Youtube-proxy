...
const ChordDebugger = {
  ...
  buildDebugData(result, capoOffset = 0) {
    ...
    for (let i = 0; i < chords.length; i++) {
      ...
      // Determine what changed (BEFORE capo)
      ...
      // NOW apply capo to ALL stages
      // applyCapo function already SUBTRACTS internally!
      if (capoOffset > 0) {
        ...
        // Also apply capo to bass note
        if (bassDetected && bassDetected !== 'NO_BASS') {
          bassDetected = this.applyCapo(bassDetected, capoOffset);
        }
      }
      
      const finalLabel = bassChord; // Final is always the bass stage result
      ...
      debugData.push({
        ...
        refinerAnalysis: chord.refinerAnalysis || null,  // ✅ ADD THIS!
  ...
  reconstructPreBass(chord, allChords) {
    ...
    // If we have originalLabel from Refiner, return label before bass
    return chord.label;
  },
  ...
  reconstructPreRefiner(chord, allChords) {
    // Use stored originalLabel if available
    if (chord.originalLabel) {
      return chord.originalLabel;
    }
    
    // Fallback: try to reverse the change
  ...
  renderTable(debugData, filter = 'all') {
    ...
    for (const row of filtered) {
      ...
      // Build refiner cell - ALWAYS show what Refiner thinks!
      let refinerContent = '';
      if (row.refinerApplied) {
        ...
        `;
      } else if (row.refinerAnalysis) {
        // Didn't change, but show what Refiner detected
        const analysis = row.refinerAnalysis;
        const qualityIcon = analysis.detectedQuality === 'major' ? '▲' : 
                           analysis.detectedQuality === 'minor' ? '▼' : '?';
        refinerContent = `
          <span style="color:#94a3b8">
            ${row.refinerChord}<br>
            <small style="color:#64748b">${qualityIcon} ${(analysis.qualityConfidence * 100).toFixed(0)}%</small>
          </span>
        `;
      } else {
      ...
      // Build bass cell - ALWAYS show what Bass detected!
      let bassContent = '';
      if (row.bassDetected === 'NO_BASS') {
        ...
        // Didn't change, but show what Bass detected
        bassContent = `
          <span style="color:#94a3b8">
            ${row.bassChord}<br>
  ...
  /**
   * Apply capo transposition (ORIGINAL LOGIC - SUBTRACT!)
   * Engine detects what we HEAR (e.g., Gm)
   * We show what we PLAY (e.g., Em with capo 3)
   */
  applyCapo(chord, capo) {
    ...
    // ORIGINAL FORMULA: SUBTRACT capo!
    const newIdx = ((idx - capo) % 12 + 12) % 12;
    const newRoot = flats[newIdx];  // Use flats for display
