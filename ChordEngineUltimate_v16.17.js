...
class ChordEngineUltimate {
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    // 🎯 v16.17: תיקון שורש "שלישית" - עובד לכל סולם!
    ...
    if (!diatonic.pcs.includes(best.root)) {
      for (const dc of diatonic.chords) {
        ...
        if (best.root === majorThird || best.root === minorThird) {
          ...
          if (triadDiatonic >= triadChromatic * 0.80 && triadDiatonic > 0.06) {
            console.log(`🎵 Re-rooting ${this.NOTES_SHARP[best.root]} → ${this.NOTES_SHARP[dc.root]} (triad score: ${triadDiatonic.toFixed(3)} vs ${triadChromatic.toFixed(3)})`);
    ...
    // v16.17: סינון אקורדים קצרים לא דיאטוניים (רעש)
    if (!diatonic.pcs.includes(best.root)) {
      if (durSec < 0.6) {
        console.log(`🔒 Ignoring short non-diatonic ${this.NOTES_SHARP[best.root]} (${durSec.toFixed(2)}s)`);
        return null;
      }
    }
    ...
    // v16.17: הקוד הישן של forcedSubstitutions הוסר - ה-re-rooting למעלה מטפל בזה טוב יותר
    // ועובד לכל סולם, לא רק C major!
  ...
  // v16.17: חישוב חוזק טריאדה על שורש נתון
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale) {
    ...
    // v16.17: העדפה חזקה יותר לאקורדים דיאטוניים
    if (inScale) {
      score += 15;  // בונוס גדול לדיאטוני
    } else {
      score -= 10;  // קנס לא דיאטוני
    }
