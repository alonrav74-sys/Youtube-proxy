...
class MajorMinorRefiner {
  ...
  /**
   * NEW: Refine chord timeline - required by ChordEngineEnhanced v14.36
   */
  async refineChordTimeline(audioBuffer, timeline, options = {}) {
    const chordResults = this.analyze(timeline, audioBuffer);
    
    const refinedTimeline = timeline.map((chord, i) => {
      const analysis = chordResults[i] || {};
      
      return {
        ...chord,
        detectedQuality: analysis.quality || 'unclear',
        qualityConfidence: analysis.confidence || 0,
        thirdRatio: 1.0,
