// =====================================================
// 🎵 SyncEngine v6.7 - Clean Lyrics (no nbsp)
// =====================================================
// 
...
const SyncEngine = {
  ...
  buildHebrewTableSheet: function(state, whisperWords, whisperText, capo, sanitizeLabel, applyCapoToLabel, fullSheetEl) {
    ...
    whisperWords.forEach(w => {
      ...
      // 🔧 Clean up artifacts
      text = text.replace(/&nbsp;?/g, '');  // Remove &nbsp; or nbsp;
      text = text.replace(/&amp;/g, '');    // Remove &amp;
      text = text.trim();
      
      if (!text) return;  // Skip if empty after cleanup
    ...
    // Build table (RTL, 2 rows per line, 12 cells per row!)
    let html = `<div dir="rtl" style="padding:20px;font-family:'David',serif;font-size:16px;line-height:1.8">`;
    html += '<table style="border-collapse:collapse;width:100%;direction:rtl;table-layout:auto">';
    
    // 🔧 FIX: 12 cells per row (looks like real song sheet!)
    ...
    for (let i = 0; i < cells.length; i += cellsPerRow) {
      ...
      while (rowCells.length < cellsPerRow) {
        rowCells.push({ time: 999999, chord: null, lyric: null });
      }
      ...
      for (const cell of rowCells) {
        ...
        if (chord) {
          html += `<td style="padding:4px 8px;color:#38bdf8;font-weight:700;text-align:center;vertical-align:bottom;font-size:14px;min-width:40px">${this.escapeHtml(chord)}</td>`;
        } else {
          html += `<td style="padding:4px 8px;vertical-align:bottom;min-width:40px"></td>`;
        }
      }
      html += '</tr>';
      ...
      for (const cell of rowCells) {
        ...
        if (lyric) {
          html += `<td style="padding:4px 8px;color:#ffffff;text-align:center;vertical-align:top;font-size:16px;min-width:40px">${this.escapeHtml(lyric)}</td>`;
        } else {
          html += `<td style="padding:4px 8px;vertical-align:top;min-width:40px"></td>`;
        }
      }
      html += '</tr>';
