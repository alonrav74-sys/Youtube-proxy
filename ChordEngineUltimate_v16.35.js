/**
 * ChordEngine v16.35 - Conservative Filtering
 * 
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log('🎵 ChordEngine v16.35 (Conservative - Fewer but Solid Chords)');
    ...
    // v16.35: TONIC VALIDATION after timeline (section 6)
    ...
    // v16.35: Enhanced slash chord detection (section 6)
    ...
    // v16.35: Auto-detect "easy rock" songs and clean up
    const stats = this.buildStats(timeline, key);
    const isEasy = this.isEasyRockSong(stats, key);
    
    if (isEasy) {
      console.log(`🎸 v16.35: Easy rock song detected - enforcing diatonic`);
      timeline = this.enforceEasyDiatonic(timeline, key);
    }
    
    // v16.35: Always run outlier smoothing (removes isolated chromatic chords)
    timeline = this.smoothOutliers(timeline, key, this.getDiatonicInfo(key));
    
    // v16.35: Pattern memory V2 (section 6 - fixes anomalies in repeated patterns)
  ...
  processAudio(audioBuffer) {
    ...
    // v16.35: Clean signal before processing
  ...
  // v16.35: Clean signal - pre-emphasis + noise gate + smoothing
  cleanSignal(x, sr) {
  ...
  extractFeatures(audio) {
    ...
    for (let start = 0; start + win <= x.length; start += hop) {
      ...
      // v16.35: Improved chroma with harmonic weighting
      ...
      for (let bin = 1; bin < mags.length; bin++) {
        ...
        // v16.35: Weight by harmonic clarity
    ...
    // v16.35: Compute spectral flux for chord change detection
    ...
    return {
      ...
      flux, fluxP80, // v16.35: Added flux
  ...
  // v16.35: Spectral flux - detects changes in chroma
  ...
  // v16.35: Find candidate frames for chord changes
  ...
  detectBassNote(mags, sr, N) {
    const fmin = 40, fmax = 300; // v16.35: Extended range (was 250)
    ...
    for (let bin = 1; bin < mags.length; bin++) {
      ...
      if (freq >= fmin) {
        ...
        const weight = freq < 150 ? 1.5 : 1.0; // v16.35: Boost very low freqs
    ...
    // v16.35: Stricter threshold (was 0.25)
  ...
  // v16.35: Improved - also detect clear pitch without bass (guitar/piano intros)
  findMusicStart(features) {
    ...
    for (let i = 0; i < energy.length; i++) {
      ...
      // v16.35: Check for clear musical pitch in chroma (for guitar/piano intros)
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.35: SONG PROFILE ANALYSIS (Dynamic thresholds) - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.35: TONIC VALIDATION FROM CHORDS - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  validateTonicFromChords(timeline, initialKey, features) {
    ...
    for (let i = 0; i < timeline.length - 1; i++) {
      ...
      if (interval === 5) cadenceTargets[next.root] += 15;
      if (interval === 7) cadenceTargets[next.root] += 10;
    ...
    for (let tonic = 0; tonic < 12; tonic++) {
      ...
      for (const isMinor of [false, true]) {
        ...
        if (fitRatio >= 0.7) {
          candidates.push({ root: tonic, minor: isMinor, score: score + fitRatio * 30 });
        }
      }
    }
    ...
    if (best.score > initialScore + 20) {
      console.log(`🔄 Tonic revalidated: ${this.NOTES_SHARP[initialKey.root]}${initialKey.minor ? 'm' : ''} → ${this.NOTES_SHARP[best.root]}${best.minor ? 'm' : ''}`);
      return { root: best.root, minor: best.minor, confidence: Math.min(0.95, initialKey.confidence + 0.1) };
    }
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.35: BETTER SLASH CHORD DETECTION - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.35: IMPROVED PATTERN MEMORY (3+4 chord patterns) - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 v16.35: STABLE BASS CHORD BUILDING with Beat-Aware Detection
  // ═══════════════════════════════════════════════════════════
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    // v16.35: Get chord change candidate frames (beat-aligned + flux + bass changes)
    const changeFrames = this.getChordChangeCandidates(features, bpm, startFrame);
    console.log(`🎵 v16.35: ${changeFrames.length} chord change candidates (beat-aligned)`);
  ...
  getBorrowedChords(key) {
    ...
    if (!key.minor) {
      ...
      // v16.35: REMOVED bVI (Ab/G#) - causes confusion with E
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    if (candidates.length === 0) {
      // v16.35: Chromatic fallback - only if VERY clear
      for (const isMinor of [false, true]) {
        const score = this.scoreChordCandidate(avg, bassNote, isMinor, bassNote, false);
        if (score > 60) { // v16.35: Higher threshold (was 40)
          ...
          if (isReasonable || score > 80) { // Only if reasonable OR super strong
            candidates.push({
              ...
              inversionBass: null,
              chordType: 'chromatic',
              score: score - 30, // v16.35: Bigger penalty (was -20)
              priority: 20        // v16.35: Lower priority (was 30)
            });
          }
        }
      }
    ...
    if (best.score < 45 && durSec < 0.40) return null; // v16.35: Stricter threshold
    
    // v16.35: Enharmonic substitution for unlikely chromatics
    ...
    // v16.35: AUTO-CORRECT quality based on actual third (use finalRoot!)
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale) {
    ...
    // v16.35: AUTO-DETECT which third is actually present!
    ...
    else if (bassNote === actualThird) score += 10; // v16.35: Slightly lower for first inversion
    else if (bassNote === fifth) score += 8;        // v16.35: Slightly lower for second inversion
    
    // v16.35: Extra validation for inversions - the bass note must be STRONG in the audio
  ...
  // v16.35: Check if chromatic chord makes harmonic sense
  isReasonableChromaticChord(root, key, prevChord) {
    ...
    if (!key.minor) {
      ...
      // v16.35: REMOVED bVI (Ab/G#) - too rare and often confused with E
  ...
  validateWithCircleOfFifths(timeline, key, features) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.35: STRICT chromatic filtering
  ...
  addExtensions(timeline, features, key, opts) {
    ...
    return timeline.map(ev => {
      ...
      // v16.35: Check all chord tones properly
      ...
      // 3. Check for 7th chords - v16.35: More precise detection
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
        ...
        if (!isMinor && M7 > 0.10 && M7 > b7 * 1.5 && M3 > 0.08) {
          // v16.35: Also check that M7 is significant relative to root
          if (M7 > r * 0.15) {
        ...
        // Dominant 7th (C7, G7) - only for major chords with clear b7
          ...
          // v16.35: b7 must be prominent enough
          if (b7 > r * 0.12) {
            label += '7';
          }
        }
      ...
      // 4. Check for sus chords - v16.35: More precise
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
      ...
      // 5. Add 6 if present (C6, Am6) - v16.35: Stricter to avoid melodic 6ths
  ...
  finalizeTimeline(timeline, bpm, features) {
    ...
    const spb = 60 / Math.max(60, Math.min(200, bpm));
    const minDuration = 0.5 * spb; // v16.35: Longer minimum (was 0.4)
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      const isStrong = ev.confidence >= 90; // v16.35: Higher threshold (was 85)
  ...
  // v16.35: Detect if song is "easy rock" (mostly diatonic)
  ...
  // v16.35: Force non-diatonic chords to nearest diatonic
  ...
  // v16.35: Remove isolated chromatic chords between identical neighbors
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.35: HMM ENGINE (parallel to Bass engine)
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.35: MERGE ENGINES - Compare and choose best
  // ═══════════════════════════════════════════════════════════
