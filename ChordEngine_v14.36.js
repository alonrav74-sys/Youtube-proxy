...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // Store audioBuffer for optional modules
      ...
      try {
        ...
        for (let i = 0; i < timeline.length; i++) {
          ...
          if (refined?.shouldOverride && refined.refinedLabel !== timeline[i].label) {
            ...
            timeline[i].originalLabel = oldLabel;
            ...
            timeline[i].reason = refined.reason;
      ...
      try {
        ...
        const refinedTimeline = await bassEngine.refineBassInTimeline(
          ...
          key,
          {
            ...
            minInversionConfidence: 0.45,
            minChordChangeConfidence: 0.55,
            debug: false
          }
        );
        ...
        for (let i = 0; i < timeline.length; i++) {
          const bassData = refinedTimeline[i];
          
          // ✅ ALWAYS store what BassEngine detected!
          if (bassData) {
            ...
            timeline[i].changedByBass = bassData.changedByBass || false;
          }
          
          if (bassData?.label && bassData.label !== timeline[i].label) {
            if (bassData.changedByBass) {
              chordChangeCount++;
              console.log(`🎸 Bass Override: ${timeline[i].label} → ${bassData.label} (${(bassData.bassConfidence * 100).toFixed(0)}%)`);
            } else if (bassData.label.includes('/')) {
              inversionCount++;
            }
            timeline[i].label = bassData.label;
          }
        }
  ...
  // ... (REST OF THE CODE IS TOO LONG - I'LL CONTINUE IN THE FILE)
