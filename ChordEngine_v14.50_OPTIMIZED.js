...
class ChordEngineEnhanced {
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode) {
    ...
    for (const cand of candidates) {
      ...
      // ✅ Set borrowed flag for scoring
      // borrowed = NOT perfectly diatonic
    ...
    const emitScore = (i, cand) => {
      ...
      // ═══════════════════════════════════════════════════════════════════════════
      // 🎯 DIATONIC BOOST - Simple and effective (from v14.36)
      // ═══════════════════════════════════════════════════════════════════════════
      // v14.36 uses simple borrowed flag: +0.20 if diatonic, -0.25 if borrowed
      // This keeps chroma as PRIMARY signal!
      
      const isBorrowed = cand.borrowed || false;
      if (!isBorrowed) {
        score += 0.20;  // Simple diatonic boost
      } else {
        score -= 0.25;  // Penalty for borrowed chords
      }
      
      // ═══════════════════════════════════════════════════════════════════════════
      // 🎸 BASS BOOST - Simple (from v14.36)
      // ═══════════════════════════════════════════════════════════════════════════
