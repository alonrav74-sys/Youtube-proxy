...
class ChordEngineEnhanced {
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode) {
    ...
    const emitScore = (i, cand) => {
      ...
      // ✅ BALANCED: Strong but not overwhelming diatonic preference
      // Allows legitimate borrowed chords with strong chroma evidence
      if (cand.diatonic) score += 0.45;  // ✅ Was 0.30, now 0.45 (moderate increase)
      
      // Bass match gets bonus
    ...
    const transitionCost = (a, b) => {
      ...
      // ✅ BALANCED: Discourage non-diatonic but don't block legitimate borrowed chords
      if (!a.diatonic && !b.diatonic) cost += 0.30;  // ✅ Was 0.15, now 0.30 (moderate)
      
      // ✅ Reward for diatonic-to-diatonic
      if (a.diatonic && b.diatonic) cost -= 0.10;  // ✅ Was 0.05, now 0.10
      
      // ✅ Small penalty for leaving key (but not huge)
      if (a.diatonic && !b.diatonic) cost += 0.15;  // ✅ Was 0.30, now 0.15

      // Common progressions
      if (toPc(b.root - a.root) === 5) cost -= 0.15;  // P4 up
      if (toPc(b.root - a.root) === 7) cost -= 0.10;  // P5 up
