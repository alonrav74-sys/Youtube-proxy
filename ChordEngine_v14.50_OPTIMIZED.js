...
class ChordEngineEnhanced {
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode) {
    ...
    const emitScore = (i, cand) => {
      ...
      // ✅ CRITICAL FIX: Require strong evidence - 0.35 threshold (from v14.36)
      if (score < 0.35) return -Infinity; // Reject weak/noisy chords
      ...
      // ═══════════════════════════════════════════════════════════════════════════
      // 🎸 BASS BOOST - With chroma validation and correction!
      // ═══════════════════════════════════════════════════════════════════════════
      // Problem: Bass detection can have ±1 semitone errors (F# instead of G)
      // Solution: Use chroma to validate and correct bass pitch class
      
      if (bassPc[i] >= 0 && cand.root === bassPc[i]) {
        // Direct bass match
        score += 0.15 * bassMultiplier;
      } else if (bassPc[i] >= 0) {
        // Bass doesn't match - but check if it's off by 1 semitone
        // If chroma strongly supports this chord root, maybe bass is wrong!
        const chromaAtRoot = c[cand.root] || 0;
        const chromaAtBass = c[bassPc[i]] || 0;
        
        const semitoneDiff = Math.min(
          Math.abs(cand.root - bassPc[i]),
          12 - Math.abs(cand.root - bassPc[i])
        );
        
        // If bass is off by just 1 semitone AND chroma strongly supports this root:
        if (semitoneDiff === 1 && chromaAtRoot > chromaAtBass * 1.3) {
          // Chroma says this is the right root, bass is probably wrong by ±1
          score += 0.12 * bassMultiplier;  // Almost full bass boost
        }
      } else {
        // No reliable bass - rely MORE on chroma!
        // Give extra boost if chroma is very strong for this root
        const chromaAtRoot = c[cand.root] || 0;
        if (chromaAtRoot > 0.50) {
          score += 0.10;  // Compensate for missing bass with strong chroma
        }
      }
  ...
  detectTonicMusically(timeline, key, duration) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // ✅ FIX: Skip first 1.5 seconds (intro noise) - from v14.36
    let realStart = 0;
    for (let i = 0; i < timeline.length; i++) {
      if (timeline[i].t >= 1.5) {
        realStart = i;
    ...
    if (realStart < timeline.length && timeline[realStart]?.label) {
      const root = this.parseRoot(timeline[realStart].label);
  ...
  enforceEarlyDiatonic(timeline, key, feats, bpm) {
    ...
    // ✅ FIX: More aggressive intro - from v14.36
    ...
    for (const ev of timeline) {
      ...
      if (ev.t <= earlyWindow) {
        ...
        if (r >= 0 && !this.inKey(r, key.root, key.minor)) {
          // ✅ CRITICAL FIX: Force tonic for first 3 seconds (from v14.36)
