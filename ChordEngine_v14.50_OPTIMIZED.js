...
class ChordEngineEnhanced {
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode) {
    ...
    const emitScore = (i, cand) => {
      ...
      // ═══════════════════════════════════════════════════════════════════════════
      // 🎯 DIATONIC BOOST - Smart application with contradiction detection
      // ═══════════════════════════════════════════════════════════════════════════
      // Check if audio strongly supports opposite mode (contradiction!)
      ...
      if (oppositeTmpl) {
        ...
        // If opposite mode scores MUCH better (>25% better), there's a contradiction!
        // This catches: V major in minor keys, borrowed chords, secondary dominants
        if (oppositeScore > score * 1.25) {
          hasContradiction = true;
        }
      }
      
      // Apply diatonic boost ONLY if no strong contradiction
      if (!hasContradiction) {
        if (cand.perfectDiatonic) {
          score += 0.65;  // Strong boost - audio + theory agree!
        } else if (cand.diatonic) {
          score += 0.18;  // Gentle nudge - root is diatonic
        }
      } else {
        // There's a contradiction - audio strongly prefers opposite mode!
        // Examples: V major in minor, III major in major (Hallelujah!), borrowed chords
        // Just give small boost for diatonic root, let audio quality decide the winner
        if (cand.diatonic) {
          score += 0.08;  // Root is correct, mode is debatable
        }
      }
