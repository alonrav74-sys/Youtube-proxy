...
class ChordEngineEnhanced {
  ...
  detectTonicFromBass(feats) {
    ...
    console.log(`🎸 Bass tonic: ${this.getNoteName(tonicPc, key)} (${(confidence * 100).toFixed(1)}% confidence)`);
  ...
  detectKeyEnhanced(feats, duration) {
    ...
    console.log(`🎯 Bass tonic: ${this.getNoteName(bassTonic.root, key)} (${(bassTonic.confidence * 100).toFixed(1)}%)`);
    ...
    if (bassTonic.confidence > 0.30) {
      ...
      console.log(`   Minor 3rd (${this.getNoteName(minorThird, key)}): ${(m3Strength * 100).toFixed(1)}%`);
      console.log(`   Major 3rd (${this.getNoteName(majorThird, key)}): ${(M3Strength * 100).toFixed(1)}%`);
      ...
      console.log(`🏆 Key (from third): ${this.getNoteName(root, key)}${isMinor ? 'm' : ''} (${isMinor ? 'minor' : 'major'} third)`);
      ...
      if (harmonicAnalysis.score >= 20.0) {
        ...
        console.log(`⚠️ Weak cadences, this might not be ${this.getNoteName(root, key)}${isMinor?'m':''}`);
        ...
        console.log(`   🔄 Testing relative key: ${this.getNoteName(relativeRoot, key)}${relativeMinor?'m':''}...`);
        ...
        if (relativeAnalysis.score > harmonicAnalysis.score + 10.0) {
          console.log(`   🎯 Relative key has MUCH stronger cadences! Switching to ${this.getNoteName(relativeRoot, key)}${relativeMinor?'m':''}`);
          root = relativeRoot;
      ...
      if (result.confidence < 0.50 && firstPc >= 0) {
        if (this.inKey(firstPc, root, isMinor)) {
          console.log(`✅ First chord ${this.getNoteName(firstPc, key)} coherent - boosting confidence`);
    ...
    if (ksResult.confidence < 0.60) {
      ...
      if (firstPc >= 0) {
        if (this.inKey(firstPc, ksResult.root, ksResult.minor)) {
          console.log(`✅ KS + First chord ${this.getNoteName(firstPc, key)} coherent - boosting`);
          ksResult.confidence = Math.min(1.0, ksResult.confidence + 0.15);
        } else {
          console.log(`⚠️ KS + First chord ${this.getNoteName(firstPc, key)} not coherent`);
  ...
  analyzeHarmonicProgressions(feats, candidateRoot, candidateMinor) {
    ...
    console.log(`🎼 Analyzing ${bassTimeline.length} bass segments for key ${this.getNoteName(candidateRoot, key)}${candidateMinor?'m':''}`);
