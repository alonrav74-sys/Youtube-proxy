// =====================================================
// 🎵 SyncEngine v6.4 - RTL Fixed (Mirror of LTR)
// =====================================================
  ...
  refreshSheetTabView: function(state, whisperWords, whisperText, detectedLanguage, capo, sanitizeLabel, applyCapoToLabel, fullSheetEl) {
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
      ...
      if (lineChords.length > 0) {
        if (isRTL) {
          // 🎯 RTL: Mirror of LTR logic - position chords precisely
          ...
          for (const chord of lineChords) {
            ...
            for (let i = 0; i < line.length; i++) {
              ...
              if (chordTime < word.time) {
                if (i === 0) {
                  // Before first word
                  charPos = lyricText.length; // Far right in RTL
                  foundWord = true;
                }
                break;
              }
              ...
              if (chordTime >= word.time && chordTime < word.end) {
                ...
                // Sum up all text AFTER this word
                const wordsAfter = line.slice(i + 1);
                const textAfter = wordsAfter.map(w => w.text).join(' ');
                const lengthAfter = textAfter.length + (textAfter.length > 0 ? 1 : 0);
                
                // Position = total length - text after - current word
                charPos = lyricText.length - lengthAfter - word.text.length;
                foundWord = true;
                break;
              }
            }
            
            // After all words? (end of line)
            if (!foundWord) {
              charPos = 0; // Far left in display (end of line in RTL)
            }
          ...
          // Sort RIGHT to LEFT (high position first)
          chordPositions.sort((a, b) => b.pos - a.pos);
          
          // Build string from RIGHT to LEFT
          let currentPos = lyricText.length;
          
          for (const cp of chordPositions) {
            // Calculate spaces before this chord
            const targetPos = cp.pos;
            const spaces = currentPos - targetPos - cp.label.length;
          ...
          if (currentPos > 0) {
            chordLine = ' '.repeat(currentPos) + chordLine;
          }
        } else {
      ...
      if (chordLine.trim()) {
        // For RTL text, chords stay LTR but align right
        const chordStyle = isRTL 
          ? 'color:#38bdf8;font-weight:700;white-space:pre;direction:ltr;text-align:right;unicode-bidi:bidi-override'
          : 'color:#38bdf8;font-weight:700;white-space:pre';
        html += `<div style="${chordStyle}">${this.escapeHtml(chordLine)}</div>`;
      }
