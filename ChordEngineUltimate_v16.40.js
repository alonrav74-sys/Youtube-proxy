/**
 * ChordEngine v16.40 - Conservative Filtering
 * 
...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    console.log('🎵 ChordEngine v16.40 (Conservative - Fewer but Solid Chords)');
    ...
    // v16.40: TONIC VALIDATION after timeline (section 6)
    ...
    if (validatedKey.root !== key.root || validatedKey.minor !== key.minor) {
      ...
      timeline = this.buildChordsStableBass(features, key, 0, audio.bpm);
      const hmmTimeline2 = this.buildChordsHMM(features, key, 0);
    ...
    // v16.40: Enhanced slash chord detection (section 6)
    ...
    // v16.40: AUTOMATIC STYLE PROFILE DETECTION
    ...
    // v16.40: Pattern memory V2 (section 6 - fixes anomalies in repeated patterns)
    ...
    return {
      ...
      mode: modeResult,
      styleProfile,  // v16.40: easy_pop / colorful_pop / jazz_like
  ...
  processAudio(audioBuffer) {
    ...
    // v16.40: Clean signal before processing
  ...
  // v16.40: Clean signal - pre-emphasis + noise gate + smoothing
  cleanSignal(x, sr) {
  ...
  extractFeatures(audio) {
    ...
    for (let start = 0; start + win <= x.length; start += hop) {
      ...
      // v16.40: Improved chroma with harmonic weighting
      ...
      for (let bin = 1; bin < mags.length; bin++) {
        ...
        // v16.40: Weight by harmonic clarity
    ...
    // v16.40: Compute spectral flux for chord change detection
    ...
    // v16.40: Bass histogram for debug
    ...
    return {
      ...
      flux, fluxP80, bassHist, // v16.40: Added bassHist
  ...
  // v16.40: Spectral flux - detects changes in chroma
  ...
  // v16.40: Find candidate frames for chord changes
  ...
  detectBassNote(mags, sr, N) {
    // v16.40: Find the LOWEST audible note, not the strongest!
    // Bass is defined by pitch, not by amplitude
    
    const fmin = 30;   // Lowest bass frequency to detect
    const fmax = 250;  // Upper limit for bass
    const minBin = Math.max(1, Math.floor(fmin * N / sr));
    const maxBin = Math.min(mags.length - 1, Math.ceil(fmax * N / sr));
    
    // Step 1: Find all peaks in the bass spectrum
    const peaks = [];
    for (let bin = minBin + 1; bin < maxBin; bin++) {
      const freq = bin * sr / N;
      const mag = mags[bin];
      
      // Must be a local maximum with minimum magnitude
      if (mag > mags[bin - 1] && mag > mags[bin + 1] && mag > 0.005) {
        peaks.push({ bin, freq, mag });
      }
    }
    
    if (peaks.length === 0) return -1;
    
    // Step 2: Find minimum magnitude threshold (10% of max peak)
    const maxMag = Math.max(...peaks.map(p => p.mag));
    const threshold = maxMag * 0.10;  // Must be at least 10% of strongest
    
    // Step 3: Find the LOWEST frequency peak that's above threshold
    // This is the key change - we want the lowest, not the strongest!
    const validPeaks = peaks.filter(p => p.mag >= threshold);
    ...
    // Take the lowest valid peak
    const bassPeak = validPeaks[0];
    
    // Step 4: Check for subharmonic - maybe we caught a harmonic
    // If there's a peak at half the frequency, prefer that
    const halfFreq = bassPeak.freq / 2;
    if (halfFreq >= fmin) {
      const subharmonic = peaks.find(p => 
        Math.abs(p.freq - halfFreq) < halfFreq * 0.08 && // Within 8%
        p.mag >= threshold * 0.5  // Can be weaker
      );
      if (subharmonic) {
        // Found a subharmonic - use it instead
        const f0 = subharmonic.freq;
        return this.toPc(Math.round(69 + 12 * Math.log2(f0 / 440)));
      }
    }
    
    // Convert frequency to pitch class
    const f0 = bassPeak.freq;
    if (f0 < fmin || f0 > fmax) return -1;
  ...
  // v16.40: Improved - also detect clear pitch without bass (guitar/piano intros)
  findMusicStart(features) {
    ...
    for (let i = 0; i < energy.length; i++) {
      ...
      // v16.40: Check for clear musical pitch in chroma (for guitar/piano intros)
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.40: SONG PROFILE ANALYSIS (Dynamic thresholds) - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.40: TONIC VALIDATION FROM CHORDS - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.40: BETTER SLASH CHORD DETECTION - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.40: IMPROVED PATTERN MEMORY (3+4 chord patterns) - section 6
  // ═══════════════════════════════════════════════════════════
  ...
  detectTonicHybrid(features, startFrame) {
    // 🔬 v16.40: Capture all detection steps
    ...
    // 🔬 v16.40: Save all sources + bass histogram
    ...
    // v16.40: Bass histogram - shows all bass notes detected
    ...
    // 🔧 v16.40: הבס - משקל גבוה כשה-confidence גבוה מאוד!
    ...
    // 🔧 v16.40: אקורד ראשון - פחות משקל אם הבס לא מסכים!
  ...
  // ═══════════════════════════════════════════════════════════
  // 🎯 v16.40: STABLE BASS CHORD BUILDING with Beat-Aware Detection
  // ═══════════════════════════════════════════════════════════
  ...
  buildChordsStableBass(features, key, startFrame, bpm) {
    ...
    // v16.40: MUCH STRICTER thresholds to avoid noise
    ...
    // v16.40: Get chord change candidate frames (beat-aligned + flux + bass changes)
    const changeFrames = this.getChordChangeCandidates(features, bpm, startFrame);
    console.log(`🎵 v16.40: ${changeFrames.length} chord change candidates (beat-aligned)`);
  ...
  getChordsWithBassNote(bassNote, key) {
    ...
    for (const bc of borrowed) {
      // v16.40: III major gets higher priority (very common in pop!)
  ...
  getBorrowedChords(key) {
    ...
    if (!key.minor) {
      // v16.40: III major (E in C) - VERY common in pop! (Hallelujah, etc.)
  ...
  determineChordTheoryAware(chroma, startFrame, endFrame, key, diatonic, bassNote, secPerFrame) {
    ...
    if (candidates.length === 0) {
      // v16.40: Chromatic fallback - only if VERY clear
      for (const isMinor of [false, true]) {
        const score = this.scoreChordCandidate(avg, bassNote, isMinor, bassNote, false);
        if (score > 60) { // v16.40: Higher threshold (was 40)
          ...
          if (isReasonable || score > 80) { // Only if reasonable OR super strong
            candidates.push({
              ...
              inversionBass: null,
              chordType: 'chromatic',
              score: score - 30, // v16.40: Bigger penalty (was -20)
              priority: 20        // v16.40: Lower priority (was 30)
            });
          }
        }
      }
    ...
    if (best.score < 45 && durSec < 0.40) return null; // v16.40: Stricter threshold
    
    // v16.40: Enharmonic substitution for unlikely chromatics
    ...
    // v16.40: AUTO-CORRECT quality based on actual third (use finalRoot!)
    const m3 = avg[this.toPc(finalRoot + 3)];
    const M3 = avg[this.toPc(finalRoot + 4)];
    
    let finalIsMinor = best.isMinor;
    
    // v16.40: For III borrowed chord, FORCE major (E in C major is ALWAYS E major, not Em)
  ...
  scoreChordCandidate(avg, root, isMinor, bassNote, inScale) {
    ...
    // v16.40: AUTO-DETECT which third is actually present!
    ...
    else if (bassNote === actualThird) score += 10; // v16.40: Slightly lower for first inversion
    else if (bassNote === fifth) score += 8;        // v16.40: Slightly lower for second inversion
  ...
  // v16.40: Check if chromatic chord makes harmonic sense
  isReasonableChromaticChord(root, key, prevChord) {
    ...
    if (!key.minor) {
      ...
      // v16.40: REMOVED bVI (Ab/G#) - too rare and often confused with E
  ...
  validateWithCircleOfFifths(timeline, key, features) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.40: STRICT chromatic filtering
  ...
  addExtensions(timeline, features, key, opts) {
    ...
    return timeline.map(ev => {
      ...
      // v16.40: Check all chord tones properly
      ...
      // 3. Check for 7th chords - v16.40: More precise detection
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
        ...
        if (!isMinor && M7 > 0.10 && M7 > b7 * 1.5 && M3 > 0.08) {
          // v16.40: Also check that M7 is significant relative to root
          if (M7 > r * 0.15) {
        ...
        // Dominant 7th (C7, G7) - only for major chords with clear b7
          ...
          // v16.40: b7 must be prominent enough
          if (b7 > r * 0.12) {
            label += '7';
          }
        }
      ...
      // 4. Check for sus chords - v16.40: More precise
      if (!label.includes('7') && !label.includes('dim') && !label.includes('aug')) {
      ...
      // 5. Add 6 if present (C6, Am6) - v16.40: Stricter to avoid melodic 6ths
  ...
  // v16.40: Enhanced filtering from v14.36 with KEY parameter
  finalizeTimeline(timeline, bpm, features, key) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      // v16.40: Use KEY to determine if diatonic (not relying on ev.inScale)
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.40: ENFORCE EARLY DIATONIC (from v14.36)
  ...
  // v16.40: Check if pitch class is in key (from v14.36)
  ...
  // v16.40: Detect if song is "easy rock" (mostly diatonic)
  ...
  // v16.40: Force non-diatonic chords to nearest diatonic
  ...
  // v16.40: Remove isolated chromatic chords between identical neighbors
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.40: HMM ENGINE (parallel to Bass engine)
  // ═══════════════════════════════════════════════════════════
    ...
    for (let i = startFrame + 1; i < M; i++) {
      if (path[i] !== cur) {
        ...
        timeline.push({
          ...
          bassNote: bass[start] >= 0 ? bass[start] : cand.root, // v16.40: Add bass
          inScale: true,
          confidence: 75,
          chordType: 'hmm',
          engine: 'hmm'
  ...
  calcEmitScore(frame, bassNote, energy, energyP70, candidate) {
    ...
    // v14.36 style: HARD THRESHOLD - reject weak/noisy frames
    if (score < 0.35) return -Infinity;
    
    // Bass match bonus
    if (bassNote >= 0 && bassNote === candidate.root) score += 0.3;
    
    // v14.36 style: HEAVY penalty for low energy (was -2, now -Infinity for very low)
    if (energy < energyP70 * 0.2) return -Infinity;
    if (energy < energyP70 * 0.4) score -= 0.5;
  ...
  // ═══════════════════════════════════════════════════════════
  // v16.40: MERGE ENGINES - Compare and choose best
  // ═══════════════════════════════════════════════════════════
