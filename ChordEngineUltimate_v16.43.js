...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // v16.43: Use Bass engine for PRIMARY timeline (better timing like v14.36)
    // HMM validates chord choices
    let timeline = this.buildChordsStableBass(features, key, 0, audio.bpm);
    console.log(`🎸 Bass engine: ${timeline.length} chords`);
    
    // Run HMM for validation
    const hmmTimeline = this.buildChordsHMM(features, key, 0);
    console.log(`🎹 HMM engine: ${hmmTimeline.length} chords`);
    
    // v16.43: Use HMM to validate Bass choices (but keep Bass timing!)
    timeline = this.validateBassWithHMM(timeline, hmmTimeline, features, key);
  ...
  // v16.43: Use HMM to validate Bass timeline (Bass is primary for timing)
  validateBassWithHMM(bassTimeline, hmmTimeline, features, key) {
    ...
    for (const bassChord of bassTimeline) {
      // Find corresponding HMM chord at this time
      const hmmChord = hmmTimeline.find(h => 
        Math.abs(h.t - bassChord.t) < 0.5 && h.root !== undefined
      );
      
      let finalChord = { ...bassChord };
      
      if (hmmChord && hmmChord.root === bassChord.root) {
        // Both agree - great!
        finalChord.consensus = 'agree';
      } else if (hmmChord && hmmChord.root !== bassChord.root) {
        ...
        // If HMM is diatonic and Bass is chromatic, use HMM chord but keep Bass timing
        if (hmmInScale && !bassInScale && !bassBorrowed) {
          console.log(`🔧 HMM corrects Bass: ${this.NOTES_SHARP[bassChord.root]} → ${this.NOTES_SHARP[hmmChord.root]} at ${bassChord.t.toFixed(2)}s`);
          finalChord = {
            ...bassChord,  // Keep Bass timing!
            root: hmmChord.root,
            label: hmmChord.label,
            type: hmmChord.type,
            hmmValidated: true
          };
        }
  ...
  validateWithBass(hmmTimeline, bassTimeline, features, key) {
    return this.validateBassWithHMM(bassTimeline, hmmTimeline, features, key);
  }
