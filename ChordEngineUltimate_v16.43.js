...
class ChordEngineUltimate {
  ...
  async detect(audioBuffer, options = {}) {
    ...
    // v16.43: Use ONLY HMM engine (like v14.36) for better timing accuracy
    // The merge of two engines was causing timing issues
    let timeline = this.buildChordsHMM(features, key, 0);
    console.log(`🎹 HMM engine: ${timeline.length} chords`);
    
    // Keep bass engine for validation - correct HMM mistakes using bass info
    const bassTimeline = this.buildChordsStableBass(features, key, 0, audio.bpm);
    console.log(`🎸 Bass engine (validation): ${bassTimeline.length} chords`);
    
    // v16.43: Use bass to validate/correct HMM timeline
    timeline = this.validateWithBass(timeline, bassTimeline, features, key);
  ...
  // v16.43: Use bass engine to validate/correct HMM timeline
  // Keeps HMM timing but can correct chord quality based on bass
  validateWithBass(hmmTimeline, bassTimeline, features, key) {
    if (!hmmTimeline.length || !bassTimeline.length) return hmmTimeline;
    ...
    for (const hmmChord of hmmTimeline) {
      ...
      if (bassChord && bassChord.root !== hmmChord.root) {
        // Bass disagrees with HMM - check if bass makes more sense
        ...
        // If bass is diatonic/borrowed and HMM is chromatic, prefer bass
